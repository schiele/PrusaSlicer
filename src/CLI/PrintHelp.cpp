...
static void print_help(
    const ConfigDef &config_def, bool show_defaults,
    ...
    for (const std::string &category : categories)
    {
        ...
        for (const auto &opt : config_def.options)
        {
            ...
            for (auto &arg : cli_args)
            {
                ...
                if (def.type == coFloat || def.type == coInt || def.type == coFloatOrPercent || def.type == coFloats ||
                    def.type == coInts)
                {
                    arg += " N";
                }
...
void print_help(bool include_print_options /* = false*/, PrinterTechnology printer_technology /* = ptAny*/)
{
    ...
    boost::nowide::cout << std::endl << "Input:" << std::endl;
    ...
    boost::nowide::cout << std::endl << "Actions:" << std::endl;
