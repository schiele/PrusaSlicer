...
void TCPConsole::transmit_next_command()
{
    ...
    BOOST_LOG_TRIVIAL(debug) << boost::format("TCPConsole: transmitting '%3%' to %1%:%2%") % m_host_name % m_port_name %
                                    cmd;
    ...
    boost::asio::async_write(m_socket, boost::asio::buffer(m_send_buffer),
                             boost::bind(&TCPConsole::handle_write, this, boost::placeholders::_1,
...
void TCPConsole::wait_next_line()
{
    ...
    boost::asio::async_read_until(m_socket, m_recv_buffer, m_newline,
                                  boost::bind(&TCPConsole::handle_read, this, boost::placeholders::_1,
                                              boost::placeholders::_2));
}
...
void TCPConsole::handle_read(const boost::system::error_code &ec, std::size_t bytes_transferred)
{
    ...
    if (ec)
    {
        BOOST_LOG_TRIVIAL(error) << boost::format("TCPConsole: Can't read from %1%:%2%: %3%") % m_host_name %
                                        m_port_name % ec.message();
    ...
    else
    {
        ...
        BOOST_LOG_TRIVIAL(debug) << boost::format("TCPConsole: received '%3%' from %1%:%2%") % m_host_name %
                                        m_port_name % line;
...
void TCPConsole::handle_write(const boost::system::error_code &ec, std::size_t)
{
    ...
    if (ec)
    {
        BOOST_LOG_TRIVIAL(error) << boost::format("TCPConsole: Can't write to %1%:%2%: %3%") % m_host_name %
                                        m_port_name % ec.message();
...
void TCPConsole::handle_connect(const boost::system::error_code &ec)
{
    ...
    if (ec)
    {
        BOOST_LOG_TRIVIAL(error) << boost::format("TCPConsole: Can't connect to %1%:%2%: %3%") % m_host_name %
                                        m_port_name % ec.message();
...
bool TCPConsole::run_queue()
{
    ...
    catch (std::exception &e)
    {
        BOOST_LOG_TRIVIAL(error) << boost::format("TCPConsole: Exception while talking with %1%:%2%: %3%") %
                                        m_host_name % m_port_name % e.what();
