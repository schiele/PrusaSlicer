...
enum class NotificationType
{
    ...
    //	Mouse3dConnected,
    //	NewPresetsAviable,
    // Notification on the start of preFlight, when a new preFlight version is published.
    // Contains a hyperlink to open a web browser pointing to the preFlight download location.
    NewAppAvailable,
    // Like NewAppAvailable but with text and link for alpha / bet release
    NewAlphaAvailable,
    NewBetaAvailable,
    NoNewReleaseAvailable,
    // Notification on the start of preFlight, when updates of system profiles are detected.
    ...
    ExportOngoing,
    // Progressbar of download from preflight:// url
    URLDownload,
    ...
    // Info about successful login or logout
    UserAccountID,
...
public:
    ...
    void push_notification(NotificationType type, NotificationLevel level, const std::string &text,
                           ...
                           const std::string &text_after = "", int timestamp = 0, bool multiline = false);
    // Pushes basic_notification with delay. See push_delayed_notification_data.
    void push_delayed_notification(const NotificationType type, std::function<bool(void)> condition_callback,
    ...
    void push_simplify_suggestion_notification(
    ...
    void push_version_notification(NotificationType type, NotificationLevel level, const std::string &text,
    ...
    // print host upload
    void push_upload_job_notification(int id, float filesize, const std::string &filename, const std::string &host,
                                      float percentage = 0);
    void set_upload_job_notification_percentage(int id, const std::string &filename, const std::string &host,
                                                float percentage);
    ...
    void push_download_URL_progress_notification(size_t id, const std::string &text,
                                                 std::function<bool(DownloaderUserAction, int)> user_action_callback);
    void push_download_URL_progress_notification_with_printables_link(
    ...
    // Transition to rendering state (generation complete, rendering preview)
    void set_slicing_progress_rendering();
    // Update rendering progress while staying in rendering state
    void set_rendering_progress(float percentage);
...
private:
    ...
    protected:
        ...
        virtual void render_text(const float win_size_x, const float win_size_y, const float win_pos_x,
                                 const float win_pos_y);
        virtual void render_close_button(const float win_size_x, const float win_size_y, const float win_pos_x,
                                         const float win_pos_y);
        virtual void render_hypertext(const float text_x, const float text_y, const std::string text,
                                      bool more = false);
    ...
    {
    public:
        PlaterWarningNotification(const NotificationData &n, NotificationIDProvider &id_provider,
    ...
    {
    public:
        ProgressBarNotification(const NotificationData &n, NotificationIDProvider &id_provider,
                                wxEvtHandler *evt_handler)
            : PopNotification(n, id_provider, evt_handler)
        {
        }
    ...
    protected:
        ...
        virtual void render_text(const float win_size_x, const float win_size_y, const float win_pos_x,
                                 const float win_pos_y) override;
        virtual void render_bar(const float win_size_x, const float win_size_y, const float win_pos_x,
                                const float win_pos_y);
        virtual void render_cancel_button(const float win_size_x, const float win_size_y, const float win_pos_x,
                                          const float win_pos_y)
        {
        }
    ...
    {
    public:
        ProgressBarWithCancelNotification(const NotificationData &n, NotificationIDProvider &id_provider,
                                          wxEvtHandler *evt_handler, std::function<bool()> cancel_callback)
            : ProgressBarNotification(n, id_provider, evt_handler), m_cancel_callback(cancel_callback)
        {
        }
    ...
    protected:
        void render_close_button(const float win_size_x, const float win_size_y, const float win_pos_x,
                                 const float win_pos_y) override;
        void render_close_button_inner(const float win_size_x, const float win_size_y, const float win_pos_x,
                                       const float win_pos_y);
        void render_cancel_button_inner(const float win_size_x, const float win_size_y, const float win_pos_x,
                                        const float win_pos_y);
        void render_bar(const float win_size_x, const float win_size_y, const float win_pos_x,
                        const float win_pos_y) override;
    ...
    {
    public:
        URLDownloadNotification(const NotificationData &n, NotificationIDProvider &id_provider,
                                wxEvtHandler *evt_handler, size_t download_id,
                                std::function<bool(DownloaderUserAction, int)> user_action_callback)
    ...
    protected:
        ...
        void render_close_button_inner(const float win_size_x, const float win_size_y, const float win_pos_x,
                                       const float win_pos_y);
        void render_pause_cancel_buttons_inner(const float win_size_x, const float win_size_y, const float win_pos_x,
                                               const float win_pos_y);
        void render_open_button_inner(const float win_size_x, const float win_size_y, const float win_pos_x,
                                      const float win_pos_y);
        void render_cancel_button_inner(const float win_size_x, const float win_size_y, const float win_pos_x,
                                        const float win_pos_y);
        void render_pause_button_inner(const float win_size_x, const float win_size_y, const float win_pos_x,
                                       const float win_pos_y);
        void render_bar(const float win_size_x, const float win_size_y, const float win_pos_x,
                        const float win_pos_y) override;
    ...
    {
    public:
        URLDownloadWithPrintablesLinkNotification(const NotificationData &n, NotificationIDProvider &id_provider,
                                                  wxEvtHandler *evt_handler, size_t download_id,
                                                  std::function<bool(DownloaderUserAction, int)> user_action_callback,
                                                  std::function<void(std::string)> hypertext_callback)
    ...
    protected:
        void render_text(const float win_size_x, const float win_size_y, const float win_pos_x,
                         const float win_pos_y) override;
        void init() override;
    ...
    public:
        ...
        PrintHostUploadNotification(const NotificationData &n, NotificationIDProvider &id_provider,
                                    wxEvtHandler *evt_handler, float percentage, int job_id, float filesize,
                                    const std::string &filename, const std::string &host)
            : ProgressBarNotification(n, id_provider, evt_handler)
            , m_job_id(job_id)
    ...
    protected:
        ...
        virtual void render_close_button(const float win_size_x, const float win_size_y, const float win_pos_x,
                                         const float win_pos_y) override;
        void render_cancel_button(const float win_size_x, const float win_size_y, const float win_pos_x,
                                  const float win_pos_y) override;
        ...
        std::string m_host;
        std::string
            m_original_host; // when hostname is resolved into ip address, we can still display original hostname (that user inserted)
    ...
    public:
        ...
        enum class SlicingProgressState
        {
            ...
            SP_RENDERING, // generation complete at 100%, rendering preview (no cancel, shows progress bar at 100%)
        ...
        SlicingProgressNotification(const NotificationData &n, NotificationIDProvider &id_provider,
                                    wxEvtHandler *evt_handler, std::function<bool()> callback)
            : ProgressBarNotification(n, id_provider, evt_handler), m_cancel_callback(callback)
        {
    ...
    protected:
        ...
        void render_close_button(const float win_size_x, const float win_size_y, const float win_pos_x,
                                 const float win_pos_y) override;
        void render_hypertext(const float text_x, const float text_y, const std::string text,
    ...
    public:
        ...
        ProgressIndicatorNotification(const NotificationData &n, NotificationIDProvider &id_provider,
                                      wxEvtHandler *evt_handler)
            : ProgressBarNotification(n, id_provider, evt_handler)
        {
            m_render_percentage = true;
    ...
    {
    public:
        ExportFinishedNotification(const NotificationData &n, NotificationIDProvider &id_provider,
                                   wxEvtHandler *evt_handler, bool to_removable, const std::string &export_dir_path)
            : PopNotification(n, id_provider, evt_handler)
    ...
    protected:
        ...
        void render_close_button(const float win_size_x, const float win_size_y, const float win_pos_x,
                                 const float win_pos_y) override;
        void render_eject_button(const float win_size_x, const float win_size_y, const float win_pos_x,
                                 const float win_pos_y);
        void render_minimize_button(const float win_pos_x, const float win_pos_y) override
        {
    ...
    {
    public:
        UpdatedItemsInfoNotification(const NotificationData &n, NotificationIDProvider &id_provider,
                                     wxEvtHandler *evt_handler, InfoItemType info_item_type)
            : PopNotification(n, id_provider, evt_handler)
        {
    ...
    const std::vector<NotificationType> m_multiple_types = {
        NotificationType::CustomNotification, NotificationType::PlaterWarning,      NotificationType::ProgressBar,
        NotificationType::PrintHostUpload,    NotificationType::SimplifySuggestion, NotificationType::URLDownload};
    //prepared (basic) notifications
    // non-static so its not loaded too early. If static, the translations wont load correctly.
    const std::vector<NotificationData> basic_notifications = {
        {NotificationType::Mouse3dDisconnected, NotificationLevel::RegularNotificationLevel, 10,
         _u8L("3D Mouse disconnected.")},
        {NotificationType::PresetUpdateAvailable, NotificationLevel::ImportantNotificationLevel, 20,
         _u8L("Configuration update is available."), _u8L("See more."),
        ...
        {NotificationType::PresetUpdateAvailableNewPrinter, NotificationLevel::ImportantNotificationLevel, 20,
         _u8L("Configuration update is available. Update contains new printer releases."), _u8L("See more."),
         [](wxEvtHandler *evnthndlr)
         {
             if (evnthndlr != nullptr)
                 wxPostEvent(evnthndlr, PresetUpdateAvailableClickedEvent(EVT_PRESET_UPDATE_AVAILABLE_CLICKED));
        ...
        {NotificationType::URLNotRegistered, NotificationLevel::RegularNotificationLevel,
         10
         // TRN: The text is followed by a hyperlink saying "here." It is necessary to split it in two phrases, sorry.
         ,
         _u8L("preFlight received a download request from supported website, but it's not allowed. You can allow it"),
         _u8L("here."),
         [](wxEvtHandler *evnthndlr)
         {
        ...
        // New version notification removed
