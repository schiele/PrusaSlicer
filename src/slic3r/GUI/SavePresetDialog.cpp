    ...
    auto add_names_from_collection = [this](const PresetCollection &presets)
    {
        for (const Preset &preset : presets)
            if (!preset.is_default)
                m_casei_preset_names.emplace_back(
                    PresetName({boost::to_lower_copy<std::string>(preset.name), preset.name}));
    };
...
void SavePresetDialog::Item::init_input_name_ctrl(wxBoxSizer *input_name_sizer, const std::string preset_name)
{
    if (m_use_text_ctrl)
    {
        ...
        long style = 0L;
#endif
        m_text_ctrl = new wxTextCtrl(m_parent, wxID_ANY, from_u8(preset_name), wxDefaultPosition,
                                     wxSize(35 * wxGetApp().em_unit(), -1), style);
    ...
    else
    {
        ...
        m_combo = new wxComboBox(m_parent, wxID_ANY, from_u8(preset_name), wxDefaultPosition,
                                 wxSize(35 * wxGetApp().em_unit(), -1));
    ...
    , m_valid_label(new wxStaticText(m_parent, wxID_ANY, ""))
{
    ...
    wxStaticText *label_top = is_for_multiple_save
                                  ? new wxStaticText(m_parent, wxID_ANY, _(TOP_LABELS.at(m_type)) + ":")
                                  : nullptr;
...
SavePresetDialog::Item::Item(wxWindow *parent, wxBoxSizer *sizer, const std::string &def_name,
                             PresetBundle *preset_bundle, PrinterTechnology pt /*= ptFFF*/)
...
static std::string get_conflict_name(const std::vector<SavePresetDialog::Item::PresetName> &casei_names,
...
void SavePresetDialog::Item::update()
{
    ...
    if (m_valid_type == ValidationType::Valid && existing)
    {
        ...
        else
        {
            if (existing->is_compatible)
                info_line = from_u8(
                    (boost::format(_u8L("Preset with name \"%1%\" already exists.")) % existing->name).str());
            else
                info_line = from_u8(
                    (boost::format(
                         _u8L("Preset with name \"%1%\" already exists and is incompatible with selected printer.")) %
                     existing->name)
                        .str());
            info_line += "\n" + (m_use_text_ctrl
                                     ? _L("Note: This preset will be replaced after renaming")
    ...
    if (m_valid_type == ValidationType::Valid && m_presets &&
        m_presets->path_from_name(m_preset_name).length() >= max_path_length)
    {
    ...
    if (m_valid_type == ValidationType::Valid && m_presets &&
        m_presets->get_preset_name_by_alias(m_preset_name) != m_preset_name)
    {
...
void SavePresetDialog::Item::update_valid_bmp()
{
    std::string bmp_name = m_valid_type == ValidationType::Warning   ? "exclamation_manifold"
                           : m_valid_type == ValidationType::NoValid ? "exclamation"
                                                                     : "tick_mark";
...
SavePresetDialog::SavePresetDialog(wxWindow *parent, std::vector<Preset::Type> types, std::string suffix,
                                   bool template_filament /* =false*/, PresetBundle *preset_bundle /* = nullptr*/)
    : DPIDialog(parent, wxID_ANY, types.size() == 1 ? _L("Save preset") : _L("Save presets"), wxDefaultPosition,
                wxSize(45 * wxGetApp().em_unit(), 5 * wxGetApp().em_unit()), wxDEFAULT_DIALOG_STYLE | wxICON_WARNING)
    , m_preset_bundle(preset_bundle)
{
...
SavePresetDialog::SavePresetDialog(wxWindow *parent, Preset::Type type, const wxString &info_line_extention)
    : DPIDialog(parent, wxID_ANY, _L("Rename preset"), wxDefaultPosition,
                wxSize(45 * wxGetApp().em_unit(), 5 * wxGetApp().em_unit()), wxDEFAULT_DIALOG_STYLE | wxICON_WARNING)
...
//    this->SetFont(wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT));
    ...
    {
        m_template_filament_checkbox = new wxCheckBox(this, wxID_ANY,
...
void SavePresetDialog::add_info_for_edit_ph_printer(wxBoxSizer *sizer)
{
    ...
    {
        wxRadioButton *btn = new wxRadioButton(this, wxID_ANY, "", wxDefaultPosition, wxDefaultSize,
                                               id == 0 ? wxRB_GROUP : 0);
...
void SavePresetDialog::update_info_for_edit_ph_printer(const std::string &preset_name)
{
    ...
    if (wxSizerItem *sizer_item = m_radio_sizer->GetItem(size_t(0)))
    {
        ...
        {
            wxString msg_text = format_wxstr(_L("What would you like to do with \"%1%\" preset after saving?"),
                                             preset_name);
            stb_sizer->GetStaticBox()->SetLabel(msg_text);

            wxString choices[] = {format_wxstr(_L("Change \"%1%\" to \"%2%\" for this physical printer \"%3%\""),
                                               m_old_preset_name, preset_name, m_ph_printer_name),
                                  format_wxstr(_L("Add \"%1%\" as a next preset for the the physical printer \"%2%\""),
                                               preset_name, m_ph_printer_name),
