...
void glAssertRecentCallImpl(const char *file_name, unsigned int line, const char *function_name)
{
    ...
    BOOST_LOG_TRIVIAL(error) << "OpenGL error in " << file_name << ":" << line << ", function " << function_name
                             << "() : " << (int) err << " - " << sErr;
    assert(false);
}
#endif // HAS_GLSAFE
...
void GLVolume::SinkingContours::update()
{
    ...
    for (const ExPolygon &expoly :
         diff_ex(expand(polygons, float(scale_(HalfWidth))), shrink(polygons, float(scale_(HalfWidth)))))
    {
        ...
        for (const Vec3d &v : triangulation)
        {
            init_data.add_vertex(
                (Vec3f) (v.cast<float>() + 0.015f * Vec3f::UnitZ())); // add a small positive z to avoid z-fighting
            ++vertices_counter;
...
void GLVolume::NonManifoldEdges::update()
{
    ...
    if (0 <= object_idx && object_idx < int(model.objects.size()))
    {
        ...
        if (0 <= volume_idx && volume_idx < int(model_object->volumes.size()))
        {
            ...
            {
                GUI::GLModel::Geometry init_data;
                init_data.format = {GUI::GLModel::Geometry::EPrimitiveType::Lines,
                                    GUI::GLModel::Geometry::EVertexLayout::P3};
...
const std::array<ColorRGBA, 4> GLVolume::MODEL_COLOR = {
    {ColorRGBA::YELLOW(), {1.0f, 0.5f, 0.5f, 1.0f}, {0.5f, 1.0f, 0.5f, 1.0f}, {0.5f, 0.5f, 1.0f, 1.0f}}};
    ...
    else
    {
        ...
        else if (hover == HS_Select || selected)
        {
            ...
            if (color == NEGATIVE_VOLUME_COLOR || color == PARAMETER_MODIFIER_COLOR || color == SUPPORT_BLOCKER_COLOR ||
                ...
                set_render_color(
                    ColorRGBA(rc.r() * color.r(), rc.g() * color.g(), rc.b() * color.b(), rc.a() * color.a()));
            else
                set_render_color(rc);
        }
...
const BoundingBoxf3 &GLVolume::transformed_bounding_box() const
{
    if (!m_transformed_bounding_box.has_value())
    {
        ...
        std::optional<BoundingBoxf3> *trans_box = const_cast<std::optional<BoundingBoxf3> *>(
...
{
    return (m_convex_hull && !m_convex_hull->empty()) ? m_convex_hull->transformed_bounding_box(trafo)
...
bool GLVolume::is_sla_support() const
{
    return false;
}
bool GLVolume::is_sla_pad() const
{
    return false;
}
...
std::vector<int> GLVolumeCollection::load_object(const ModelObject *model_object, int obj_idx,
                                                 const std::vector<int> &instance_idxs)
{
    std::vector<int> volumes_idx;
    for (int volume_idx = 0; volume_idx < int(model_object->volumes.size()); ++volume_idx)
        for (int instance_idx : instance_idxs)
            volumes_idx.emplace_back(
                this->GLVolumeCollection::load_object_volume(model_object, obj_idx, volume_idx, instance_idx));
    return volumes_idx;
}

int GLVolumeCollection::load_object_volume(const ModelObject *model_object, int obj_idx, int volume_idx,
                                           int instance_idx)
{
    const ModelVolume *model_volume = model_object->volumes[volume_idx];
...
#endif // SLIC3R_OPENGL_ES
{
    ...
    if (size_unknown)
    {
        ...
        float out_points_idx[][3] = {{0, -depth, 0},  {0, 0, 0},           {38.453f, 0, 0},       {61.547f, 0, 0},
                                     {100.0f, 0, 0},  {100.0f, -depth, 0}, {55.7735f, -10.0f, 0}, {44.2265f, 10.0f, 0},
                                     {38.453f, 0, 1}, {0, 0, 1},           {0, -depth, 1},        {100.0f, -depth, 1},
                                     {100.0f, 0, 1},  {61.547f, 0, 1},     {55.7735f, -10.0f, 1}, {44.2265f, 10.0f, 1}};
        static constexpr const int out_facets_idx[][3] = {{0, 1, 2},    {3, 4, 5},    {6, 5, 0},   {3, 5, 6},
                                                          {6, 2, 7},    {6, 0, 2},    {8, 9, 10},  {11, 12, 13},
                                                          {10, 11, 14}, {14, 11, 13}, {15, 8, 14}, {8, 10, 14},
                                                          {3, 12, 4},   {3, 13, 12},  {6, 13, 3},  {6, 14, 13},
                                                          {7, 14, 6},   {7, 15, 14},  {2, 15, 7},  {2, 8, 15},
                                                          {1, 8, 2},    {1, 9, 8},    {0, 9, 1},   {0, 10, 9},
                                                          {5, 10, 0},   {5, 11, 10},  {4, 11, 5},  {4, 12, 11}};
        indexed_triangle_set its;
        for (int i = 0; i < 16; ++i)
            its.vertices.emplace_back(out_points_idx[i][0] / (100.f / min_width), out_points_idx[i][1] + depth,
                                      out_points_idx[i][2]);
        its.indices.reserve(28);
        for (const int *face : out_facets_idx)
            its.indices.emplace_back(coord_t(face[0]), coord_t(face[1]), coord_t(face[2]));
    ...
    else
    {
        ...
        {
            TriangleMesh m = make_cube(width, z_and_depth_pairs[i - 1].second,
                                       z_and_depth_pairs[i].first - z_and_depth_pairs[i - 1].first);
            m.translate(0.f, -z_and_depth_pairs[i - 1].second / 2.f + z_and_depth_pairs[0].second / 2.f,
                        z_and_depth_pairs[i - 1].first);
            mesh.merge(m);
        }
    }
    ...
    if (R > 0.)
    {
        ...
        disk_mesh.scale(
            Vec3f(1.f + scale_x * brim_width / R, 1.f + brim_width / R, 1.f)); // Scale so the brim is not deformed.
...
// This function was used for loading SLA auxiliary GLVolumes (support trees, pad)
...
GLVolumeWithIdAndZList volumes_to_render(const GLVolumePtrs &volumes, GLVolumeCollection::ERenderType type,
                                         ...
                                         std::function<bool(const GLVolume &)> filter_func)
{
    ...
    if (type == GLVolumeCollection::ERenderType::Transparent && list.size() > 1)
    {
        ...
        {
            volume.second.second =
                volume.first->bounding_box().transformed(view_matrix * volume.first->world_matrix()).max(2);
        }
                                ...
                                const GLRenderContext &ctx, std::function<bool(const GLVolume &)> filter_func) const
{
    ...
    for (GLVolumeWithIdAndZ &volume : to_render)
    {
        ...
        else
        {
            ...
            shader->set_uniform("volume_world_matrix", world_matrix);
            shader->set_uniform("slope.actived",
                                m_slope.active && !volume.first->is_modifier && !volume.first->is_wipe_tower());
            shader->set_uniform("slope.volume_world_normal_matrix",
                                static_cast<Matrix3f>(world_matrix_inv_transp.cast<float>()));
...
void GLVolumeCollection::update_colors_by_extruder(const DynamicPrintConfig *config)
{
    ...
    const ConfigOptionStrings *extruders_opt = dynamic_cast<const ConfigOptionStrings *>(
        config->option("extruder_colour"));
    if (extruders_opt == nullptr)
        return;

    const ConfigOptionStrings *filamemts_opt = dynamic_cast<const ConfigOptionStrings *>(
        config->option("filament_colour"));
    ...
    {
        if (volume == nullptr || volume->is_modifier || volume->is_wipe_tower() || volume->is_sla_pad() ||
            volume->is_sla_support())
            continue;
...
std::string GLVolumeCollection::log_memory_info() const
{
    return " (GLVolumeCollection RAM: " + format_memsize_MB(this->cpu_memory_used()) +
           " GPU: " + format_memsize_MB(this->gpu_memory_used()) +
           " Both: " + format_memsize_MB(this->gpu_memory_used()) + ")";
}

static void thick_lines_to_geometry(const Lines &lines, const std::vector<double> &widths,
                                    const std::vector<double> &heights, bool closed, double top_z,
                                    GUI::GLModel::Geometry &geometry)
{
    ...
    for (size_t ii = 0; ii < lines_end; ++ii)
    {
        ...
        if (is_first || bottom_z_different)
        {
            ...
            idx_a[Left] = idx_last++;
            geometry.add_vertex(Vec3f(a2.x(), a2.y(), middle_z),
                                Vec3f(-xy_right_normal.x(), -xy_right_normal.y(), 0.0f));
        ...
        else
        {
            ...
            if (sharp)
            {
                if (!bottom_z_different)
                {
                    ...
                    idx_a[Right] = idx_last++;
                    geometry.add_vertex(Vec3f(a1.x(), a1.y(), middle_z),
                                        Vec3f(xy_right_normal.x(), xy_right_normal.y(), 0.0f));
                    idx_a[Left] = idx_last++;
                    geometry.add_vertex(Vec3f(a2.x(), a2.y(), middle_z),
                                        Vec3f(-xy_right_normal.x(), -xy_right_normal.y(), 0.0f));
...
static void thick_lines_to_geometry(const Lines3 &lines, const std::vector<double> &widths,
                                    const std::vector<double> &heights, bool closed, GUI::GLModel::Geometry &geometry)
{
    assert(!lines.empty());
    if (lines.empty())
        return;
...
void _3DScene::thick_lines_to_verts(const Lines &lines, const std::vector<double> &widths,
                                    const std::vector<double> &heights, bool closed, double top_z,
                                    GUI::GLModel::Geometry &geometry)
{
...
void _3DScene::thick_lines_to_verts(const Lines3 &lines, const std::vector<double> &widths,
                                    const std::vector<double> &heights, bool closed, GUI::GLModel::Geometry &geometry)
{
...
void _3DScene::extrusionentity_to_verts(const ExtrusionPath &extrusion_path, float print_z, const Point &copy,
                                        GUI::GLModel::Geometry &geometry)
{
...
void _3DScene::extrusionentity_to_verts(const ExtrusionLoop &extrusion_loop, float print_z, const Point &copy,
                                        GUI::GLModel::Geometry &geometry)
{
    Lines lines;
...
void _3DScene::extrusionentity_to_verts(const ExtrusionMultiPath &extrusion_multi_path, float print_z,
                                        const Point &copy, GUI::GLModel::Geometry &geometry)
{
    Lines lines;
    std::vector<double> widths;
    std::vector<double> heights;
...
void _3DScene::extrusionentity_to_verts(const ExtrusionEntityCollection &extrusion_entity_collection, float print_z,
                                        const Point &copy, GUI::GLModel::Geometry &geometry)
{
...
void _3DScene::extrusionentity_to_verts(const ExtrusionEntity *extrusion_entity, float print_z, const Point &copy,
                                        GUI::GLModel::Geometry &geometry)
{
    if (extrusion_entity != nullptr)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    auto *extrusion_entity_collection = dynamic_cast<const ExtrusionEntityCollection *>(
                        extrusion_entity);
