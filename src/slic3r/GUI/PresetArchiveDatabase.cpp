...
bool unzip_repository(const fs::path &source_path, const fs::path &target_path)
{
    ...
    for (size_t i = 0; i < num_files; ++i)
    {
        ...
        {
            BOOST_LOG_TRIVIAL(error) << "Failed to get file stat for file #" << i
                                     << " in the zip archive. Ending Unzipping.";
...
{
    // No authorization header needed - account services removed
    return true;
}

} // namespace

bool OnlineArchiveRepository::get_file_inner(const std::string &url, const fs::path &target_path,
                                             PresetUpdaterUIStatus *ui_status) const
{
    bool res = false;
    fs::path tmp_path = target_path;
    tmp_path += format(".%1%%2%", get_current_pid(), TMP_EXTENSION);
    BOOST_LOG_TRIVIAL(info) << format("Get: `%1%`\n\t-> `%2%`\n\tvia tmp path `%3%`", url, target_path.string(),
                                      tmp_path.string());
    ...
    http.timeout_max(30)
        ...
        .on_retry([&](int attempt, unsigned delay) { return !ui_status->on_attempt(attempt, delay); })
...
bool OnlineArchiveRepository::get_file(const std::string &source_subpath, const fs::path &target_path,
                                       const std::string &repository_id, PresetUpdaterUIStatus *ui_status) const
{
...
bool OnlineArchiveRepository::get_ini_no_id(const std::string &source_subpath, const fs::path &target_path,
                                            PresetUpdaterUIStatus *ui_status) const
{
    ui_status->set_target(target_path.filename().string());
...
bool LocalArchiveRepository::get_file_inner(const fs::path &source_path, const fs::path &target_path) const
{
    ...
    {
        BOOST_LOG_TRIVIAL(error) << "Copying of " << source_path << " to " << target_path << " has failed (" << cfr
                                 << "): " << error_message;
...
bool LocalArchiveRepository::get_file(const std::string &source_subpath, const fs::path &target_path,
                                      const std::string &repository_id, PresetUpdaterUIStatus *ui_status) const
{
    if (repository_id != m_data.id)
    {
...
bool LocalArchiveRepository::get_ini_no_id(const std::string &source_subpath, const fs::path &target_path,
                                           PresetUpdaterUIStatus *ui_status) const
{
    return get_file_inner(m_data.tmp_path / source_subpath, target_path);
}
    ...
    for (const std::string &uuid : selected_uuids)
    {
        ...
        for (const auto &archive : m_archive_repositories)
        {
            ...
            if (!archive->is_extracted())
            {
                ...
                msg = GUI::format(_L("Cannot select local source from path: %1%. It was not extracted."),
...
bool PresetArchiveDatabase::extract_archives_with_check(std::string &msg)
{
    ...
    for (const auto &pair : m_selected_repositories_uuid)
    {
        ...
        const auto &archives_it = std::find_if(m_archive_repositories.begin(), m_archive_repositories.end(),
                                               compare_repo);
...
{
    if (auto it = std::find_if(m_archive_repositories.begin(), m_archive_repositories.end(),
                               [path](const std::unique_ptr<ArchiveRepository> &ptr)
    ...
    try
    {
        ...
        for (const auto &subtree : ptree)
        {
            ...
            if (const auto source_path = subtree.second.get_optional<std::string>("source_path"); source_path)
            {
                ...
                m_archive_repositories.emplace_back(
                    std::make_unique<LocalArchiveRepository>(std::move(uuid), std::move(manifest), extracted));
            ...
            m_archive_repositories.emplace_back(
                std::make_unique<OnlineArchiveRepository>(std::move(uuid), std::move(manifest)));
        }
    }
    catch (const std::exception &e)
    {
	...
	*/
    ...
    for (const auto &archive : m_archive_repositories)
    {
        ...
        line += GUI::format("{\"name\": \"%1%\","
                            ...
                            "}",
                            man.name, man.description, man.visibility, man.id, man.url, man.index_url,
...
void PresetArchiveDatabase::read_server_manifest(const std::string &json_body)
{
    ...
    for (const auto &repo_ptr : m_archive_repositories)
    {
        ...
        if (it->second)
        {
            ...
            secret_online_used_repos_cache.emplace_back(
                std::make_unique<OnlineArchiveRepository>(repo_ptr->get_uuid(), std::move(manifest)));
        }
    }
    ...
    for (const auto &repo_ptr : secret_online_used_repos_cache)
    {
        ...
        if (std::find_if(m_archive_repositories.begin(), m_archive_repositories.end(),
                         [uuid](const std::unique_ptr<ArchiveRepository> &ptr)
                         { return ptr->get_uuid() == uuid; }) == m_archive_repositories.end())
        {
            ArchiveRepository::RepositoryManifest manifest(repo_ptr->get_manifest());
...
{
    // Online preset sync disabled - external services removed
    return false;
}
} // namespace
