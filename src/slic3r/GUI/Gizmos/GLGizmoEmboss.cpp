...
std::unique_ptr<DataBase> create_emboss_data_base(const std::string &text, StyleManager &style_manager,
                                                  TextLinesModel &text_lines, const Selection &selection,
                                                  ModelVolumeType type, std::shared_ptr<std::atomic<bool>> &cancel);
...
void init_face_names(Facenames &facenames, Worker &worker);
...
void init_text_lines(TextLinesModel &text_lines, const Selection &selection, /* const*/ StyleManager &style_manager,
...
CreateVolumeParams GLGizmoEmboss::create_input(ModelVolumeType volume_type)
{
    ...
    DataBasePtr base = create_emboss_data_base(m_text, m_style_manager, m_text_lines, m_parent.get_selection(),
                                               volume_type, m_job_cancel);
...
bool GLGizmoEmboss::re_emboss(const ModelVolume &text_volume, GLCanvas3D &canvas,
                              std::shared_ptr<std::atomic<bool>> job_cancel)
{
    ...
    DataBasePtr base = create_emboss_data_base(tc.text, style_manager, text_lines, selection, text_volume.type(),
                                               job_cancel);
...
bool GLGizmoEmboss::do_mirror(size_t axis)
{
    ...
    if (!use_surface && !is_per_glyph)
    {
        ...
        {
            selection.mirror((Axis) axis, get_drag_transformation_type(selection));
        };
        selection_transform(selection, selection_mirror_fnc);

        m_parent.do_mirror(L("Set Mirror"));
        m_parent.event_poster()->postEvent(CanvasEventType::ManipulationUpdateAndShow);
        return true;
    }
...
bool GLGizmoEmboss::init_create(ModelVolumeType volume_type)
{
    ...
    if (m_parent.has_selected_cut_object())
    {
...
bool GLGizmoEmboss::on_mouse_for_rotation(const MouseInput &mouse)
{
...
bool GLGizmoEmboss::on_mouse_for_translate(const MouseInput &mouse)
{
    if (m_volume == nullptr)
        return false;
...
void GLGizmoEmboss::on_mouse_change_selection(const MouseInput &mouse)
{
    static bool was_dragging = true;
    if ((mouse.type == MouseEventType::LeftUp || mouse.type == MouseEventType::RightUp) && !was_dragging)
    {
    ...
    if (mouse.type == MouseEventType::RightDown)
    {
        // is hovered volume closest hovered?
        int hovered_idx = m_parent.get_first_hover_volume_idx();
        if (hovered_idx < 0)
...
bool GLGizmoEmboss::on_mouse(const MouseInput &mouse)
{
    if (m_volume == nullptr ||
    ...
    if (on_mouse_for_translate(mouse))
        return true;
    on_mouse_change_selection(mouse);
    return false;
}
...
bool GLGizmoEmboss::wants_enter_leave_snapshots() const
{
    return true;
}
std::string GLGizmoEmboss::get_gizmo_entering_text() const
{
    return _u8L("Enter emboss gizmo");
}
std::string GLGizmoEmboss::get_gizmo_leaving_text() const
{
    return _u8L("Leave emboss gizmo");
}
std::string GLGizmoEmboss::get_action_snapshot_name() const
{
    return _u8L("Embossing actions");
}
...
bool GLGizmoEmboss::on_init()
{
    ...
    m_style_manager.init(m_parent.app_config());
...
static void draw_mouse_offset(const std::optional<Vec2d> &offset)
{
    ...
    float thickness = 3.f * m_imgui->get_style_scaling();
...
void GLGizmoEmboss::on_set_state()
{
    ...
    else if (m_state == GLGizmoBase::On)
    {
        ...
        {
            m_set_window_offset = calc_fine_position(m_parent.get_selection(), get_minimal_window_size(),
                                                     m_parent.get_canvas_size());
        }
        else
        {
            m_set_window_offset = (m_gui_cfg != nullptr)
                                      ? ImGuiPureWrap::change_window_position(on_get_name().c_str(), false)
                                      : ImVec2(-1, -1);
        }
    }
}
...
EmbossStyles GLGizmoEmboss::create_default_styles()
{
    ...
    EmbossStyles styles = {
        WxFontUtils::create_emboss_style(wx_font_normal,
                                         _u8L("NORMAL")), // wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT)
        WxFontUtils::create_emboss_style(
            *wxSMALL_FONT,
            _u8L("SMALL")), // A font using the wxFONTFAMILY_SWISS family and 2 points smaller than wxNORMAL_FONT.
        WxFontUtils::create_emboss_style(
            *wxITALIC_FONT,
        ...
        WxFontUtils::create_emboss_style(
            *wxSWISS_FONT,
            _u8L("SWISS")), // A font identic to wxNORMAL_FONT except for the family used which is wxFONTFAMILY_SWISS.
        WxFontUtils::create_emboss_style(wxFont(10, wxFONTFAMILY_MODERN, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_BOLD),
                                         _u8L("MODERN")),
    };
...
std::optional<wxString> get_installed_face_name(const std::optional<std::string> &face_name_opt,
                                                ::Facenames &face_names, Worker &worker)
{
    ...
    init_face_names(face_names, worker);
    face_names.is_init = false;

    auto cmp = [](const FaceName &fn, const wxString &wx_name)
    {
        return fn.wx_name < wx_name;
    };
...
void GLGizmoEmboss::set_volume_by_selection()
{
    ...
    if (volume->id() == m_volume_id && m_volume != nullptr &&
    ...
    auto has_same_name = [&name = style.name](const StyleManager::Style &style_item)
    {
        return style_item.name == name;
    };
    ...
    if (m_volume == nullptr || get_model_volume(m_volume_id, objects) == nullptr ||
        (m_volume->get_object()->volumes.size() == 1) != (volume->get_object()->volumes.size() == 1))
    {
...
bool GLGizmoEmboss::process(bool make_snapshot, std::optional<Transform3d> volume_transformation)
{
    ...
    const Selection &selection = m_parent.get_selection();
    DataBasePtr base = create_emboss_data_base(m_text, m_style_manager, m_text_lines, selection, m_volume->type(),
                                               m_job_cancel);
    ...
    m_parent.clear_before_change_volume(*m_volume, _u8L("Custom supports, seams and multimaterial painting were "
...
void GLGizmoEmboss::close()
{
    if (m_volume != nullptr && m_volume->text_configuration.has_value())
    {
        ...
        if (is_text_empty(m_text))
        {
            ...
            {
                m_parent.remove_object(m_parent.get_selection().get_object_idx());
            }
            else
            {
                m_parent.event_poster()->postEvent(CanvasEventType::RemoveObject);
            }
        }
...
void GLGizmoEmboss::draw_window()
{
    ...
    {
        ImGui::SetTooltip(
            "%s", ((m_allow_open_near_volume) ? "Fix settings position" : "Allow floating window near text").c_str());
    }
...
{
    auto create_range_text_prep =
        [&mng = m_style_manager, &text = m_text, &exist_unknown = m_text_contain_unknown_glyph]()
    {
    ...
    bool exist_font = imgui_font != nullptr && imgui_font->IsLoaded() && imgui_font->Scale > 0.f &&
    ...
    else
    {
        ...
        if (is_multiline && (prop.align.first == FontProp::HorizontalAlign::center ||
                             prop.align.first == FontProp::HorizontalAlign::right))
...
void GLGizmoEmboss::draw_font_list()
{
    ...
    ImGuiWindowFlags popup_flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize |
    ...
    if (ImGui::BeginPopup(popup_id, popup_flags))
    {
        ...
        {
            init_face_names(*m_face_names, m_parent.get_job_worker());
        ...
        for (FaceName &face : m_face_names->faces)
        {
            ...
            if (ImGuiPureWrap::selectable_contrast(face.name_truncated.c_str(), is_selected, flags, selectable_size))
            {
                ...
                {
                    del_index = index;
                    MessageDialog(m_parent.get_wxglcanvas_parent(),
                                  GUI::format_wxstr(_L("Font \"%1%\" can't be selected."), wx_face_name));
                }
            }
...
void GLGizmoEmboss::draw_model_type()
{
    ...
    {
        m_parent.take_typed_snapshot(_u8L("Change Text Type"), static_cast<int>(UndoRedo::SnapshotType::GizmoAction));
...
void GLGizmoEmboss::draw_style_rename_popup()
{
    ...
    if (store)
    {
        ...
        for (const ModelObject *mo : m_parent.get_model()->objects)
        {
...
void GLGizmoEmboss::draw_style_rename_button()
{
    ...
    {
        if (can_rename)
            ImGui::SetTooltip("%s", _u8L("Rename current style.").c_str());
        else
            ImGui::SetTooltip("%s", _u8L("Can't rename temporary style.").c_str());
    }
...
void GLGizmoEmboss::draw_delete_style_button()
{
    ...
    if (draw_button(m_icons, IconType::erase, !can_delete))
    {
        ...
        wxWindow *dialog_parent = m_parent.get_wxglcanvas_parent();
        bool exist_change = false;
        while (true)
        {
            ...
            {
                MessageDialog msg(dialog_parent, _L("Can't remove the last existing style."), dialog_title,
                                  wxICON_ERROR | wxOK);
                msg.ShowModal();
                break;
            }
            ...
            wxString message = GUI::format_wxstr(_L("Are you sure you want to permanently remove the \"%1%\" style?"),
                                                 style_name);
            MessageDialog msg(dialog_parent, message, dialog_title, wxICON_WARNING | wxYES | wxNO);
...
std::optional<Transform3d> fix_transformation(const StyleManager::Style &from, const StyleManager::Style &to,
                                              GLCanvas3D &canvas)
{
...
void GLGizmoEmboss::draw_style_list()
{
    ...
    else
    {
        ...
        if (ImGui::IsItemHovered())
        {
            ...
            std::string tooltip = is_modified ? GUI::format(_L("Modified style \"%1%\""), current_style.name)
    ...
    if (selected_style_index.has_value())
    {
        ...
        else
        {
            ...
            wxString message = GUI::format_wxstr(_L("Style \"%1%\" can't be used and will be removed from a list."),
                                                 style.name);
...
bool GLGizmoEmboss::revertible(const std::string &name, T &value, const T *default_value,
...
template<>
bool imgui_input(const char *label, float *v, float step, float step_fast, const char *format,
                 ImGuiInputTextFlags flags)
{
    return ImGui::InputFloat(label, v, step, step_fast, format, flags);
}
template<>
bool imgui_input(const char *label, double *v, double step, double step_fast, const char *format,
                 ImGuiInputTextFlags flags)
{
...
bool GLGizmoEmboss::rev_input(const std::string &name, T &value, const T *default_value,
                              const std::string &undo_tooltip, T step, T step_fast, const char *format,
                              ImGuiInputTextFlags flags) const
{
    // draw offseted input
    auto draw_offseted_input = [&offset = m_gui_cfg->input_offset, &width = m_gui_cfg->input_width, &name, &value,
...
template<typename T>
bool GLGizmoEmboss::rev_input_mm(const std::string &name, T &value, const T *default_value_ptr,
                                 const std::string &undo_tooltip, T step, T step_fast, const char *format,
                                 bool use_inch, const std::optional<float> &scale) const
{
...
bool GLGizmoEmboss::rev_checkbox(const std::string &name, bool &value, const bool *default_value,
...
bool GLGizmoEmboss::rev_slider(const std::string &name, std::optional<float> &value,
                               const std::optional<float> *default_value, const std::string &undo_tooltip,
                               const MinMax<float> &min_max, const std::string &format, const wxString &tooltip) const
{
    auto draw_slider_optional_float = [&]() -> bool
    {
        ...
        return m_imgui->slider_optional_float(("##" + name).c_str(), value, min_max.min, min_max.max, format.c_str(),
                                              1.f, false, tooltip);
    };
    float undo_offset = ImGui::GetStyle().FramePadding.x;
...
bool GLGizmoEmboss::rev_slider(const std::string &name, float &value, const float *default_value,
                               const std::string &undo_tooltip, const MinMax<float> &min_max, const std::string &format,
                               const wxString &tooltip) const
{
    auto draw_slider_float = [&]() -> bool
    {
        ...
        ImGui::SameLine(slider_offset);
        ImGui::SetNextItemWidth(slider_width);
        return m_imgui->slider_float("##" + name, &value, min_max.min, min_max.max, format.c_str(), 1.f, false,
                                     tooltip);
    };
    float undo_offset = ImGui::GetStyle().FramePadding.x;
...
void GLGizmoEmboss::draw_advanced()
{
    ...
    std::string ff_property = "ascent=" + std::to_string(font_info.ascent) +
                              ", descent=" + std::to_string(font_info.descent) +
                              ", lineGap=" + std::to_string(font_info.linegap) +
                              ", unitPerEm=" + std::to_string(font_info.unit_per_em) + ", cache(" +
                              std::to_string(cache_size) + " glyphs)";
...
void GLGizmoEmboss::draw_use_surface()
{
    bool can_use_surface = (m_volume->emboss_shape->projection.use_surface)
                               ? true
                               : // already used surface must have option to uncheck
    ...
    const bool *def_use_surface = m_style_manager.exist_stored_style()
                                      ? &m_style_manager.get_stored_style()->projection.use_surface
                                      : nullptr;
...
void GLGizmoEmboss::draw_per_glyph()
{
    ...
    const bool *def_per_glyph = m_style_manager.exist_stored_style()
                                    ? &m_style_manager.get_stored_style()->prop.per_glyph
                                    : nullptr;
    // TRN - Tooltip after mouse hover above reverting button (undo icon)
...
void GLGizmoEmboss::draw_align()
{
    ...
    auto draw_fnc = [&align, input_offset = m_gui_cfg->advanced_input_offset, &icons = m_icons]()
    {
        ...
        static const std::array<HAlignItem, 3> h_aligns{
            {{FontProp::HorizontalAlign::left, IconType::align_horizontal_left,
              _CTX_utf8(L_CONTEXT("Left", "Alignment"), "Alignment")},
             {FontProp::HorizontalAlign::center, IconType::align_horizontal_center,
              _CTX_utf8(L_CONTEXT("Center", "Alignment"), "Alignment")},
             {FontProp::HorizontalAlign::right, IconType::align_horizontal_right,
        ...
        static const std::array<VAlignItem, 3> v_aligns{
            {{FontProp::VerticalAlign::top, IconType::align_vertical_top,
              _CTX_utf8(L_CONTEXT("Top", "Alignment"), "Alignment")},
             {FontProp::VerticalAlign::center, IconType::align_vertical_center,
              _CTX_utf8(L_CONTEXT("Middle", "Alignment"), "Alignment")},
             {FontProp::VerticalAlign::bottom, IconType::align_vertical_bottom,
              _CTX_utf8(L_CONTEXT("Bottom", "Alignment"), "Alignment")}}};
        ...
        for (const HAlignItem &h_align : h_aligns)
        {
            ImGui::SameLine(offset);
            offset = .0f;
            if (align.first == h_align.align)
                draw(get_icon(icons, h_align.icon, IconState::hovered));
            ...
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip("%s", h_align.tooltip.c_str());
        }

        for (const VAlignItem &v_align : v_aligns)
        {
            ImGui::SameLine();
            if (align.second == v_align.align)
                draw(get_icon(icons, v_align.icon, IconState::hovered));
            ...
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip("%s", v_align.tooltip.c_str());
        }
        return is_change;
    };

    const FontProp::Align *def_align = m_style_manager.exist_stored_style()
                                           ? &m_style_manager.get_stored_style()->prop.align
                                           : nullptr;
    float undo_offset = ImGui::GetStyle().FramePadding.x;
...
void GLGizmoEmboss::draw_char_gap()
{
    ...
    auto def_char_gap_font_point = m_style_manager.exist_stored_style()
                                       ? &m_style_manager.get_stored_style()->prop.char_gap
                                       : nullptr;
    ...
    float def_char_gap_value = def_char_gap_font_point
                                   ? static_cast<float>(def_char_gap_font_point->value_or(0) * scale)
                                   : 0.f;
...
void GLGizmoEmboss::draw_line_gap()
{
    ...
    auto def_line_gap_font_point = m_style_manager.exist_stored_style()
                                       ? &m_style_manager.get_stored_style()->prop.line_gap
                                       : nullptr;
    float def_line_gap_value = def_line_gap_font_point
                                   ? static_cast<float>(def_line_gap_font_point->value_or(0) * scale)
                                   : 0.f;
...
void GLGizmoEmboss::draw_boldness()
{
    ...
    auto def_boldness_font_point = m_style_manager.exist_stored_style()
                                       ? &m_style_manager.get_stored_style()->prop.boldness
                                       : nullptr;
    float def_boldness_value = def_boldness_font_point
                                   ? static_cast<float>(def_boldness_font_point->value_or(0) * scale)
                                   : 0.f;
...
void GLGizmoEmboss::draw_surface_distance()
{
    ...
    if (use_inch)
    {
        ...
        if (distance.has_value())
            distance_inch = (*distance * ObjectManipulation::mm_to_in);
        std::optional<float> def_distance_inch;
        if (def_distance != nullptr)
        {
            if (def_distance->has_value())
                def_distance_inch = ObjectManipulation::mm_to_in * (*(*def_distance));
        ...
        if (rev_slider(tr_from_surface, distance_inch, def_distance, undo_move_tooltip, min_max_distance, "%.3f in",
                       move_tooltip))
        {
    ...
    {
        if (rev_slider(tr_from_surface, distance, def_distance, undo_move_tooltip, min_max_distance, "%.2f mm",
                       move_tooltip))
            is_moved = true;
    }
...
void GLGizmoEmboss::draw_rotation()
{
    ...
    const auto def_angle_rad = m_style_manager.exist_stored_style() ? &m_style_manager.get_stored_style()->angle
                                                                    : nullptr;
    ...
    bool exist_change = false;
    if (rev_slider(tr_rotation, angle_deg, def_angle_deg, undo_tooltip, limits.angle,
                   std::string(reinterpret_cast<const char *>(u8"%.2f °")), tooltip))
    {
    ...
    if (!m_volume->is_the_only_one_part())
    {
        ...
        const IconManager::Icon &icon_hover = get_icon(m_icons, m_keep_up ? IconType::lock_bold : IconType::unlock_bold,
                                                       IconState::activable);
        const IconManager::Icon &icon_disable = get_icon(m_icons, m_keep_up ? IconType::lock : IconType::unlock,
                                                         IconState::disabled);
...
bool GLGizmoEmboss::choose_font_by_wxdialog()
{
    ...
    wxFontDialog font_dialog(m_parent.get_wxglcanvas_parent(), data);
        ...
        (!use_deserialized_font && !m_style_manager.load_style(emboss_style, wx_font)))
    {
        ...
        wxString message = GUI::format_wxstr("Font \"%1%\" can't be used. Please select another.", emboss_style.name);
...
bool GLGizmoEmboss::choose_true_type_file()
{
    ...
    wxFileDialog dialog(nullptr, "Choose one or more files (TTF, TTC):", fontDir, selectedFile,
                        file_wildcards(FT_FONTS), wxFD_OPEN | wxFD_FILE_MUST_EXIST);
...
void GLGizmoEmboss::init_icons()
{
    ...
    std::vector<std::string> filenames{"edit_button.svg",
                                       "delete.svg",
                                       "add_copies.svg",
                                       "save.svg",
                                       "undo.svg",
...
TextDataBase::TextDataBase(DataBase &&parent, const FontFileWithCache &font_file,
...
bool TextDataBase::create_text_lines(const Transform3d &tr, const ModelVolumePtrs &vols)
{
    ...
    text_lines = Slic3r::Emboss::create_text_lines(tr, vols, *m_font_file.font_file, m_text_configuration.style.prop,
                                                   count_lines);
    if (text_lines.empty())
...
ImVec2 calc_fine_position(const Selection &selection, const ImVec2 &windows_size, const Size &canvas_size)
{
    ...
    const Camera &camera = selection.get_camera();
    Slic3r::Polygon hull = CameraUtils::create_hull2d(camera, *volume);
...
void init_face_names(Facenames &face_names, Worker &worker)
{
    ...
    ScopeGuard sg(
        [&enumerate_start, &face_names = face_names]()
        {
...
GuiCfg create_gui_configuration()
{
    ...
    float max_text_width = std::max({ImGui::CalcTextSize(tr.font.c_str()).x, ImGui::CalcTextSize(tr.height.c_str()).x,
    ...
    float max_advanced_text_width = std::max(
        {ImGui::CalcTextSize(tr.use_surface.c_str()).x, ImGui::CalcTextSize(tr.per_glyph.c_str()).x,
    ...
    cfg.minimal_window_size_with_collections = ImVec2(cfg.minimal_window_size_with_advance.x,
