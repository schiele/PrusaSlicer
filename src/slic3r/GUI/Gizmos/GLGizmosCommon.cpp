...
ModelInstance *SelectionInfo::model_instance() const
{
    ...
    return inst_idx < int(m_model_object->instances.size()) ? m_model_object->instances[get_active_instance()]
                                                            : nullptr;
}
...
void ObjectClipper::render_cut(const std::vector<size_t> *ignore_idxs) const
{
    ...
    const Geometry::Transformation inst_trafo =
        sel_info->model_object()->instances[sel_info->get_active_instance()]->get_transformation();
    ...
    for (auto &clipper : m_clippers)
    {
        ...
        ignore_idxs_local.erase(ignore_idxs_local.begin(),
                                std::find_if(ignore_idxs_local.begin(), ignore_idxs_local.end(),
                                             [num_of_contours](size_t idx) { return idx >= size_t(num_of_contours); }));
...
void ObjectClipper::set_position_by_ratio(double pos, bool keep_normal)
{
    ...
    m_clp_ratio = pos;
    m_clp.reset(
        new ClippingPlane(normal, (dist - (-m_active_inst_bb_radius) - m_clp_ratio * 2 * m_active_inst_bb_radius)));
...
{
    // SLA supports clipper not available
}
