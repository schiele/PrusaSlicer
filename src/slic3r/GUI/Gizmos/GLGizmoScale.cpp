...
std::string GLGizmoScale3D::get_tooltip() const
{
    ...
    else if (m_hover_id == 6 || m_hover_id == 7 || m_hover_id == 8 || m_hover_id == 9 || m_grabbers[6].dragging ||
             m_grabbers[7].dragging || m_grabbers[8].dragging || m_grabbers[9].dragging)
    {
...
double GLGizmoScale3D::calc_ratio(const UpdateData &data) const
{
    ...
    if (len_starting_vec != 0.0)
    {
        ...
        const Vec3d inters = data.mouse_ray.a +
                             (m_starting.drag_position - data.mouse_ray.a).dot(mouse_dir) * mouse_dir;
