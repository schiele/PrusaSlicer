...
void GLGizmoSimplify::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    std::vector<std::string> reduce_captions = {static_cast<std::string>(_u8L("Extra high")),
    ...
    if (!m_configuration.use_count)
    {
        ...
        m_configuration.decimate_ratio = (1.0f - (m_configuration.wanted_count / (float) m_original_triangle_count)) *
                                         100.f;
    }
    ...
    ImGui::SameLine(m_gui_cfg->bottom_left_width);
    ImGui::SetNextItemWidth(m_gui_cfg->input_width);
    const char *format = (m_configuration.decimate_ratio > 10)
                             ? "%.0f %%"
...
void GLGizmoSimplify::create_gui_cfg()
{
    ...
    cfg.bottom_left_width = std::max(ImGuiPureWrap::calc_text_size(tr_detail_level).x,
...
void GLGizmoSimplify::request_rerender(bool force)
{
    ...
    if (force || now > m_last_rerender_timestamp + 250)
    { // 250 ms
        set_dirty();
...
void GLGizmoSimplify::on_render()
{
    ...
    if (m_volume_ids != get_selected_volume_ids(selection))
        return;
