...
bool GLGizmoFdmSupports::on_init()
{
    ...
    m_desc["enforcer_type"] = _u8L("Support type") + ":";
...
void GLGizmoFdmSupports::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    const float autoset_slider_left =
        ImGuiPureWrap::calc_text_size(m_desc.at("highlight_by_angle"), false, autoset_slider_label_max_width).x +
        m_imgui->scaled(1.f);
    ...
    ImGui::PushItemWidth(enforcer_radio_snug);
    if (ImGuiPureWrap::radio_button(m_desc["enforcer_snug"], m_enforcer_type == 0))
    {
        m_enforcer_type = 0;
        if (m_highlight_by_angle_threshold_deg > 0.f)
            m_parent.set_slope_color(0.7f, 0.7f, 1.0f); // Light blue - Snug
    }
    ...
    ImGui::SameLine();
    ImGui::PushItemWidth(enforcer_radio_organic);
    if (ImGuiPureWrap::radio_button(m_desc["enforcer_organic"], m_enforcer_type == 2))
    {
        m_enforcer_type = 2;
        if (m_highlight_by_angle_threshold_deg > 0.f)
            m_parent.set_slope_color(0.5f, 0.85f, 0.5f); // Light green - Organic
    }
    if (ImGui::IsItemHovered())
    ...
    // The support spots search is now skipped for objects with support enabled (SKIP-SUPPORT-SPOTS-001/002)
    // to improve slicing performance. Since the gizmo's auto-paint relies on this data, we disable
    // the button rather than have it fail silently or trigger expensive re-slicing.
    ...
    wxString tooltip = format_wxstr(
        _L("Preselects faces by overhang angle. It is possible to restrict paintable facets to only preselected faces when "
    ...
    if (ImGui::IsItemHovered())
        ImGuiPureWrap::tooltip(GUI::format(_u8L("Allows painting only on facets selected by: \"%1%\""),
...
void GLGizmoFdmSupports::select_facets_by_angle(float threshold_deg, bool block)
{
    ...
    for (const ModelVolume *mv : mo->volumes)
    {
        ...
        for (const stl_triangle_vertex_indices &face : its.indices)
        {
            ...
            {
                TriangleStateType state = block ? TriangleStateType::BLOCKER : get_left_button_state_type();
                m_triangle_selectors[mesh_id]->set_facet(idx, state);
    ...
    m_parent.take_gizmo_snapshot(block ? _u8L("Block supports by angle") : _u8L("Add supports by angle"));
    update_model_object();
    this->waiting_for_autogenerated_supports = false;
    m_parent.set_as_dirty();
}
        ...
        if (po->model_object()->id() == mo->id())
        {
            ...
            for (ModelVolume *model_volume : po->model_object()->volumes)
            {
                if (model_volume->is_model_part())
                {
                    ...
                    selectors.emplace(model_volume->id().id,
                                      TriangleSelectorWrapper{model_volume->mesh(), mesh_transformation});
...
bool GLGizmoFdmSupports::has_backend_supports()
{
    ...
    if (!done && !m_parent.is_bg_process_update_scheduled())
    {
        waiting_for_autogenerated_supports = false;
    }
...
{
    std::string err = m_parent.validate_current_print();
    if (!err.empty())
    {
        MessageDialog dlg(m_parent.get_wxglcanvas_parent(),
                          _L("Automatic painting requires valid print setup.") + " \n" + from_u8(err), _L("Warning"),
                          wxOK);
        dlg.ShowModal();
        return;
    }
    ...
    ModelObject *mo = m_c->selection_info()->model_object();
    bool printable = std::any_of(mo->instances.begin(), mo->instances.end(),
                                 [](const ModelInstance *p) { return p->is_printable(); });
    if (!printable)
    {
        MessageDialog dlg(m_parent.get_wxglcanvas_parent(), _L("Automatic painting requires printable object."),
                          _L("Warning"), wxOK);
        dlg.ShowModal();
        return;
    }
    ...
    if (first_instance_bed != s_multiple_beds.get_inst_map().end() &&
    ...
    bool not_painted =
        std::all_of(mo->volumes.begin(), mo->volumes.end(), [](const ModelVolume *vol)
    ...
    {
        m_parent.take_gizmo_snapshot(_u8L("Automatic painting support points"));
        int mesh_id = -1.0f;
        for (ModelVolume *mv : mo->volumes)
        {
            if (mv->is_model_part())
        ...
        m_parent.call_after(
            [this]()
            {
                m_parent.reslice_until_step(static_cast<int>(posSupportSpotsSearch),
                                            *m_c->selection_info()->model_object(), false);
...
wxString GLGizmoFdmSupports::handle_snapshot_action_name(bool control_down,
