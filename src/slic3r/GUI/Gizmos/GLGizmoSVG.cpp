...
constexpr double get_tesselation_tolerance(double scale)
{
    ...
    constexpr double tesselation_tolerance_scaled = (tesselation_tolerance_in_mm * tesselation_tolerance_in_mm) /
...
EmbossShape select_shape(std::string_view filepath = "",
                         double tesselation_tolerance_in_mm = get_tesselation_tolerance(1.));
...
DataBasePtr create_emboss_data_base(std::shared_ptr<std::atomic<bool>> &cancel, ModelVolumeType volume_type,
...
bool GLGizmoSVG::wants_enter_leave_snapshots() const
{
    return true;
}
std::string GLGizmoSVG::get_gizmo_entering_text() const
{
    return _u8L("Enter SVG gizmo");
}
std::string GLGizmoSVG::get_gizmo_leaving_text() const
{
    return _u8L("Leave SVG gizmo");
}
std::string GLGizmoSVG::get_action_snapshot_name() const
{
    return _u8L("SVG actions");
}
...
void GLGizmoSVG::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    )
    {
        ...
        m_icons = init_icons(m_icon_manager,
                             *m_gui_cfg); // need regeneration when change resolution(move between monitors)
    }

    const ImVec2 &window_size = m_volume->is_the_only_one_part() ? m_gui_cfg->window_size_for_object
                                                                 : m_gui_cfg->window_size;
...
void wu_draw_line(Linef line, const std::function<void(int x, int y, float brightess)> &plot)
{
    auto ipart = [](float x) -> int
    {
...
void draw_side_outline(const ExPolygons &shape, const std::array<unsigned char, N> &color,
                       std::vector<unsigned char> &data, size_t data_width, double scale)
{
    ...
    auto draw = [&data, data_width, count_lines, get_offset, &color](int x, int y, float brightess)
    {
        ...
        else if (alpha != 255)
        {
            alpha = static_cast<unsigned char>(
                std::min(255, int(alpha) + static_cast<int>(std::round(brightess * 255))));
        }
    };
...
template<unsigned int N> // N .. count of channels per pixel
void draw_filled(const ExPolygons &shape, const std::array<unsigned char, N> &color, std::vector<unsigned char> &data,
                 size_t data_width, double scale)
{
    ...
    auto draw =
        [&data, width = static_cast<int>(data_width), count_lines, get_offset, &color](int x, int y, float brightess)
    {
    ...
    for (int y = std::max(0, static_cast<int>(std::round(bb.min.y()))); y < max_y; ++y)
    {
        ...
        Intersections intersections = Slic3r::AABBTreeLines::get_intersections_with_line<false, Vec2d, Linef>(lines,
                                                                                                              tree,
                                                                                                              line);
...
bool init_texture(Texture &texture, const ExPolygonsWithIds &shapes_with_ids, unsigned max_size_px,
                  const std::vector<std::string> &shape_warnings)
{
    ...
    std::array<unsigned char, 4> color_warning{234, 160, 50, alpha}; // icons orange
    ...
    glsafe(::glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (GLsizei) texture.width, (GLsizei) texture.height, 0, GL_RGBA,
...
std::vector<std::string> create_shape_warnings(const EmbossShape &shape, float scale)
{
    ...
    for (NSVGshape *shape = image.shapes; shape != NULL; shape = shape->next, ++shape_index)
    {
        ...
        if (!fill_warning.empty())
        {
            ...
            add_warning(shape_index * 2,
                        GUI::format(_L("Fill of shape (%1%) contains unsupported: %2%."), shape->id, fill_warning));
        }

        float minimal_width_in_mm = 1e-3f;
        if (shape->strokeWidth <= minimal_width_in_mm * scale)
        {
            add_warning(shape_index * 2, GUI::format(_L("Stroke of shape (%1%) is too thin (minimal width is %2% mm)."),
                                                     shape->id, minimal_width_in_mm));
            continue;
        }
        std::string stroke_warning = create_stroke_warning(*shape);
        if (!stroke_warning.empty())
            add_warning(shape_index * 2 + 1,
                        GUI::format(_L("Stroke of shape (%1%) contains unsupported: %2%."), shape->id, stroke_warning));
    }
    return result;
}
...
void GLGizmoSVG::draw_filename()
{
    ...
    if (ImGui::BeginCombo("##file_options", nullptr, flags))
    {
        ...
        if (ImGui::Selectable((_L("Save as") + dots).ToUTF8().data()))
        {
            ...
            wxFileDialog dlg(parent, dlg_title, last_used_directory, dlg_file, wildcard,
...
void GLGizmoSVG::draw_size()
{
    ...
    if (m_keep_ratio)
    {
        ...
        if (m_imgui->slider_float("##width_size_slider", &width_f, minmax.min, minmax.max, ss.str().c_str(), 1.f,
                                  false))
        {
    ...
    const IconManager::Icon &icon_hover = get_icon(m_icons,
                                                   m_keep_ratio ? IconType::lock_hover : IconType::unlock_hover);
...
void GLGizmoSVG::draw_distance()
{
    ...
    if (use_inch)
    {
        ...
        if (m_distance.has_value())
            distance_inch = (*m_distance * ObjectManipulation::mm_to_in);
        ...
        if (m_imgui->slider_optional_float("##distance", m_distance, min_distance, max_distance, "%.3f in", 1.f, false,
                                           move_tooltip))
        {
    ...
    {
        if (m_imgui->slider_optional_float("##distance", m_distance, min_distance, max_distance, "%.2f mm", 1.f, false,
                                           move_tooltip))
            is_moved = true;
    }
...
void GLGizmoSVG::draw_rotation()
{
    ...
    if (m_imgui->slider_float("##angle", &angle_deg, limits.angle.min, limits.angle.max,
                              reinterpret_cast<const char *>(u8"%.2f °"), 1.f, false, _L("Rotate Clock-wise.")))
    {
    ...
    if (!m_volume->is_the_only_one_part())
    {
        ...
        const IconManager::Icon &icon_hover = get_icon(m_icons,
                                                       m_keep_up ? IconType::lock_hover : IconType::unlock_hover);
        if (button(icon, icon_hover, icon))
            m_keep_up = !m_keep_up;
        if (ImGui::IsItemHovered())
...
GuiCfg create_gui_configuration(ImGuiWrapper *imgui)
{
    ...
    float window_height = window_title + // window title
                          ...
                          input_height *
                              7 + // depth + size + use_surface + FromSurface + Rotation + Mirror + FaceTheCamera
...
EmbossShape select_shape(std::string_view filepath, double tesselation_tolerance)
{
    ...
    {
        show_error(nullptr,
                   GUI::format(_u8L("SVG file does NOT contain a single path to be embossed (%1%)."), svg.path));
        return {};
    }
