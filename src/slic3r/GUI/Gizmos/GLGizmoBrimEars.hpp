...
private:
    ...
    public:
        CacheEntry()
            : brim_point(BrimPoint()), selected(false), normal(Vec3f(0, 0, 1)), is_hover(false), is_error(false)
        {
        }
...
private:
    ...
    void on_render_when_inactive() override;

    void render_points(const Selection &selection);
    void render_points(const Selection &selection, bool use_object_color);

    float m_new_point_head_diameter; // Size of a new point.
    float m_new_point_overlap = 0.f;
    float m_max_angle = 125.f;
    float m_detection_radius = 1.f;
    double m_detection_radius_max = .0f;
    CacheEntry m_point_before_drag;       // undo/redo - so we know what state was edited
    float m_old_point_head_diameter = 0.; // the same
    float m_old_point_overlap = 0.f;
    mutable std::vector<CacheEntry>
        m_editing_cache; // a support point and whether it is currently selectedchanges or undo/redo
    ...
    bool m_always_show_brims = false; // Keep brim points visible when gizmo is closed
    ...
    void begin_overlap_change(float initial_value);
    void update_cache_overlap();
    void apply_overlap_change();
...
protected:
    ...
    mutable int m_last_canvas_width = 0;
    mutable int m_last_canvas_height = 0;
};
