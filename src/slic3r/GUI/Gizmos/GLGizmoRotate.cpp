...
void GLGizmoRotate::enable_grabber()
{
    m_grabbers[0].enabled = true;
}
void GLGizmoRotate::disable_grabber()
{
...
void GLGizmoRotate::init_data_from_selection(const Selection &selection)
{
    const auto [box, box_trafo] = m_force_local_coordinate
                                      ? selection.get_bounding_box_in_reference_system(ECoordinatesType::Local)
...
{
    // Rotation optimization feature disabled - depends on SLA code that was removed
    m_rotoptimizewin_state.accuracy = 0.75f;
    m_rotoptimizewin_state.method_id = 0;
}
...
Transform3d GLGizmoRotate::local_transform(const Selection &selection) const
{
    ...
    case X:
    {
        ret = Geometry::rotation_transform(0.5 * PI * Vec3d::UnitY()) *
              Geometry::rotation_transform(-0.5 * PI * Vec3d::UnitZ());
        break;
    }
    case Y:
    {
        ret = Geometry::rotation_transform(-0.5 * PI * Vec3d::UnitZ()) *
              Geometry::rotation_transform(-0.5 * PI * Vec3d::UnitY());
        break;
    }
    default:
    case Z:
...
GLGizmoRotate3D::GLGizmoRotate3D(GLCanvas3D &parent, const std::string &icon_filename, unsigned int sprite_id)
    ...
    , m_gizmos({GLGizmoRotate(parent, GLGizmoRotate::X), GLGizmoRotate(parent, GLGizmoRotate::Y),
...
{
    ImGuiPureWrap::begin(_u8L("Optimize orientation"),
    ...
    // Rotation optimization feature not available in preFlight
    ImGui::TextDisabled("%s", _u8L("Feature not available").c_str());
}
