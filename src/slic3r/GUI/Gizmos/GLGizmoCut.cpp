...
bool GLGizmoCut3D::on_mouse(const MouseInput &mouse)
{
    Vec2i mouse_coord((int) mouse.x, (int) mouse.y);
    Vec2d mouse_pos = mouse_coord.cast<double>();

    if (mouse.shift && mouse.type == MouseEventType::LeftDown)
        return gizmo_event(SLAGizmoEventType::LeftDown, mouse_pos, mouse.shift, mouse.alt, mouse.cmd);
    if (mouse.cmd && mouse.type == MouseEventType::LeftDown)
        return false;
    if (cut_line_processing())
    {
        if (mouse.shift)
        {
            if (mouse.type == MouseEventType::Motion || mouse.dragging)
                return gizmo_event(SLAGizmoEventType::Moving, mouse_pos, mouse.shift, mouse.alt, mouse.cmd);
            if (mouse.type == MouseEventType::LeftUp)
                return gizmo_event(SLAGizmoEventType::LeftUp, mouse_pos, mouse.shift, mouse.alt, mouse.cmd);
        }
        discard_cut_line_processing();
    }
    else if (mouse.type == MouseEventType::Motion && !mouse.dragging)
        return false;

    if (m_hover_id >= CutPlane && mouse.type == MouseEventType::LeftDown && !m_connectors_editing)
    {
        Vec3d pos;
        Vec3d pos_world;
    ...
    if (use_grabbers(mouse))
    {
        if (m_hover_id >= m_connectors_group_id)
        {
            if (mouse.type == MouseEventType::LeftDown && !mouse.cmd && !mouse.alt)
                unselect_all_connectors();
            if (mouse.type == MouseEventType::LeftUp && !mouse.shift)
                gizmo_event(SLAGizmoEventType::LeftUp, mouse_pos, mouse.shift, mouse.alt, mouse.cmd);
        }
        else if (m_hover_id == CutPlane)
        {
            ...
            else if (mouse.type == MouseEventType::LeftUp && !m_was_cut_plane_dragged && !m_was_contour_selected)
                flip_cut_plane();
        }

        if (m_hover_id >= CutPlane && mouse.dragging && !m_connectors_editing)
        {
            Vec3d pos;
            Vec3d pos_world;
            if (unproject_on_cut_plane(mouse_pos, pos, pos_world, false))
    ...
    if (mouse.type == MouseEventType::LeftDown)
    {
        ...
        if ((!mouse.shift || grabber_contains_mouse) &&
    ...
    else if (mouse.type == MouseEventType::LeftUp && !m_parent.is_mouse_dragging())
    {
        gizmo_event(SLAGizmoEventType::LeftUp, mouse_pos, mouse.shift, mouse.alt, mouse.cmd);
        return true;
    }
    ...
    {
        if (!m_connectors_editing && !mouse.shift && !mouse.ctrl && !mouse.alt && !mouse.meta &&
            CutMode(m_mode) == CutMode::cutPlanar)
        {
            if (!m_part_selection.valid())
                process_contours();
            m_part_selection.toggle_selection(mouse_pos, m_parent.get_camera());
    ...
    else if (pending_right_up && mouse.type == MouseEventType::RightUp)
    {
        pending_right_up = false;
        return true;
    }
...
{
    m_parent.take_gizmo_snapshot(_u8L("Move cut plane"));
...
{
    vec = Transformation(translation_transform(m_plane_center) * m_rotation_m * translation_transform(-m_plane_center))
              .get_matrix() *
          vec;
}
...
bool GLGizmoCut3D::render_cut_mode_combo()
{
    ...
    const bool is_changed = ImGuiPureWrap::combo(_u8L("Mode"), m_modes, selection_idx, 0, m_label_width,
                                                 m_control_width);

    if (is_changed)
    {
        m_parent.take_gizmo_snapshot(_u8L("Change cut mode"));
...
bool GLGizmoCut3D::render_slider_double_input(const std::string &label, float &value_in, float &tolerance_in,
                                              float min_val /* = -0.1f*/, float max_tolerance /* = -0.1f*/)
{
    static constexpr const float UndefMinVal = -0.1f;
    const float f_mm_to_in = static_cast<float>(ObjectManipulation::mm_to_in);

    auto render_slider =
        [this, f_mm_to_in](const std::string &label, float &val, float def_val, float max_val, const wxString &tooltip)
    {
        ...
        const std::string format = val < 0.f ? UndefLabel
    ...
    const bool is_tolerance_changed = render_slider("##tolerance_" + label, tolerance_in, 0.f, max_tolerance_v,
                                                    _L("Tolerance"));
...
void GLGizmoCut3D::render_move_center_input(int axis)
{
    ...
    if (m_imperial_units)
        value *= ObjectManipulation::mm_to_in;
    ImGui::InputDouble(("##move_" + m_axis_names[axis]).c_str(), &value, 0.0, 0.0, "%.2f",
...
bool GLGizmoCut3D::render_reset_button(const std::string &label_id, const std::string &tooltip) const
{
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, {1.0f * m_imgui->get_style_scaling(), style.ItemSpacing.y});
...
indexed_triangle_set GLGizmoCut3D::its_make_groove_plane()
{
    ...
    const float side_width = is_approx(m_groove.flaps_angle, 0.f) ? m_groove.depth
                                                                  : (m_groove.depth / sin(m_groove.flaps_angle));
    ...
    if (groove_half_width_upper > proj && groove_half_width_lower > proj)
    {
        ...
        auto get_vertices = [x, y](float z_upper, float z_lower, float nar_upper_x, float nar_lower_x,
                                   float ext_upper_x, float ext_lower_x)
        {
        ...
        std::vector<stl_vertex> vertices = get_vertices(z_upper, z_lower, nar_upper_x, nar_lower_x, ext_upper_x,
                                                        ext_lower_x);
    ...
    if (groove_half_width_upper < proj && groove_half_width_lower < proj)
    {
        ...
        auto get_vertices = [x, y](float z_upper, float z_lower, float cross_pt_upper_y, float cross_pt_lower_y,
                                   float ext_upper_x, float ext_lower_x)
        {
        ...
        std::vector<stl_vertex> vertices = get_vertices(z_upper, z_lower, cross_pt_upper_y, cross_pt_lower_y,
                                                        ext_upper_x, ext_lower_x);
        mesh.vertices.insert(mesh.vertices.end(), vertices.begin(), vertices.end());
...
void GLGizmoCut3D::render_cut_plane()
{
    ...
    const Transform3d view_model_matrix = camera.get_view_matrix() * translation_transform(m_plane_center) *
                                          m_rotation_m;
...
void GLGizmoCut3D::render_rotation_snapping(GrabberID axis, const ColorRGBA &color)
{
    ...
    const Camera &camera = m_parent.get_camera();
    Transform3d view_model_matrix = camera.get_view_matrix() * translation_transform(m_plane_center) *
                                    m_start_dragging_m;

    if (axis == X)
        view_model_matrix = view_model_matrix * rotation_transform(0.5 * PI * Vec3d::UnitY()) *
...
void GLGizmoCut3D::render_grabber_connection(const ColorRGBA &color, Transform3d view_matrix,
                                             double line_len_koef /* = 1.0*/)
{
    const Transform3d line_view_matrix = view_matrix *
                                         scale_transform(Vec3d(1.0, 1.0, line_len_koef * m_grabber_connection_len));
...
void GLGizmoCut3D::render_cut_plane_grabbers()
{
    ...
    const Transform3d view_matrix = m_parent.get_camera().get_view_matrix() * translation_transform(m_plane_center) *
                                    m_rotation_m;
    ...
    if (!no_xy_dragging && m_hover_id != CutPlaneZRotation && m_hover_id != CutPlaneXMove &&
        m_hover_id != CutPlaneYMove)
    {
        ...
        color = m_hover_id == Y   ? complementary(ColorRGBA::GREEN())
                ...
                : m_hover_id == Z ? GRABBER_COLOR
                                  : ColorRGBA::GRAY();
        render_model(m_sphere.model, color,
                     view_matrix * translation_transform(m_grabber_connection_len * Vec3d::UnitZ()) *
    ...
    if (CutMode(m_mode) == CutMode::cutTongueAndGroove)
    {
        ...
        if (no_xy_grabber_hovered || m_hover_id == CutPlaneZRotation)
        {
            ...
            if (m_hover_id == CutPlaneZRotation)
            {
                ...
                render_grabber_connection(GRABBER_COLOR, view_matrix * rotation_transform(0.5 * PI * Vec3d::UnitX()),
                                          1.75);

                Vec3d offset = Vec3d(1.25 * size, grabber_shift, 0.0);
                render_model(m_cone.model, color,
                             view_matrix * translation_transform(offset) *
                                 rotation_transform(0.5 * PI * Vec3d::UnitY()) * scale_transform(cone_scale));
...
void GLGizmoCut3D::render_cut_line()
{
    ...
    render_line(m_cut_line, GRABBER_COLOR, m_parent.get_camera().get_view_matrix(), 0.25f);
}
...
void GLGizmoCut3D::on_load(cereal::BinaryInputArchive &ar)
{
    ...
    ar(m_keep_upper, m_keep_lower, m_rotate_lower, m_rotate_upper, m_hide_cut_plane, mode, m_connectors_editing,
       m_ar_plane_center, m_rotation_m, groove_depth, groove_width, groove_flaps_angle, groove_angle,
       groove_depth_tolerance, groove_width_tolerance);
...
void GLGizmoCut3D::on_save(cereal::BinaryOutputArchive &ar) const
{
    ar(m_keep_upper, m_keep_lower, m_rotate_lower, m_rotate_upper, m_hide_cut_plane, m_mode, m_connectors_editing,
       m_ar_plane_center, m_start_dragging_m, m_groove.depth, m_groove.width, m_groove.flaps_angle, m_groove.angle,
       m_groove.depth_tolerance, m_groove.width_tolerance);
}
...
void GLGizmoCut3D::on_set_state()
{
    ...
    else
    {
        ...
        if (m_part_selection.valid())
            m_parent.call_after([this]() { m_part_selection = PartSelection(); });
    }
}
...
void GLGizmoCut3D::on_register_raycasters_for_picking()
{
        ...
        if (CommonGizmosDataObjects::SelectionInfo *si = m_c->selection_info())
        {
            ...
            for (int i = 0; i < int(connectors.size()); ++i)
                m_raycasters.emplace_back(
                    m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo, i + m_connectors_group_id,
    ...
    {
        m_raycasters.emplace_back(m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo, X,
                                                                     *m_cone.mesh_raycaster, Transform3d::Identity()));
        m_raycasters.emplace_back(m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo, X,
                                                                     *m_cone.mesh_raycaster, Transform3d::Identity()));

        m_raycasters.emplace_back(m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo, Y,
                                                                     *m_cone.mesh_raycaster, Transform3d::Identity()));
        m_raycasters.emplace_back(m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo, Y,
                                                                     *m_cone.mesh_raycaster, Transform3d::Identity()));

        m_raycasters.emplace_back(m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo, Z,
        ...
        if (CutMode(m_mode) == CutMode::cutTongueAndGroove)
        {
            m_raycasters.emplace_back(m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo,
                                                                         CutPlaneZRotation, *m_sphere.mesh_raycaster,
                                                                         Transform3d::Identity()));
            m_raycasters.emplace_back(m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo,
                                                                         CutPlaneZRotation, *m_cone.mesh_raycaster,
                                                                         Transform3d::Identity()));
            m_raycasters.emplace_back(m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo,
                                                                         CutPlaneZRotation, *m_cone.mesh_raycaster,
                                                                         Transform3d::Identity()));
            ...
            m_raycasters.emplace_back(m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo, CutPlaneXMove,
                                                                         *m_cube.mesh_raycaster,
                                                                         Transform3d::Identity()));
            m_raycasters.emplace_back(m_parent.add_raycaster_for_picking(SceneRaycaster::EType::Gizmo, CutPlaneXMove,
                                                                         *m_cone.mesh_raycaster,
                                                                         Transform3d::Identity()));
...
{
    std::vector<std::shared_ptr<SceneRaycasterItem>> *raycasters = m_parent.get_raycasters_for_picking(
...
void GLGizmoCut3D::update_raycasters_for_picking_transform()
{
    if (m_connectors_editing)
    {
        ...
        for (size_t i = 0; i < connectors.size(); ++i)
        {
            ...
            const Transform3d scale_trafo = scale_transform(
                Vec3f(connector.radius, connector.radius, height).cast<double>());
    ...
    else if (!cut_line_processing())
    {
        ...
        Vec3d offset = Vec3d(0.0, 1.25 * size, m_grabber_connection_len);
        m_raycasters[id++]->set_transform(trafo * translation_transform(offset) *
                                          rotation_transform(-0.5 * PI * Vec3d::UnitX()) * scale_transform(scale));
        offset = Vec3d(0.0, -1.25 * size, m_grabber_connection_len);
        m_raycasters[id++]->set_transform(trafo * translation_transform(offset) *
                                          rotation_transform(0.5 * PI * Vec3d::UnitX()) * scale_transform(scale));
        ...
        offset = Vec3d(1.25 * size, 0.0, m_grabber_connection_len);
        m_raycasters[id++]->set_transform(trafo * translation_transform(offset) *
                                          rotation_transform(0.5 * PI * Vec3d::UnitY()) * scale_transform(scale));
        offset = Vec3d(-1.25 * size, 0.0, m_grabber_connection_len);
        m_raycasters[id++]->set_transform(trafo * translation_transform(offset) *
                                          rotation_transform(-0.5 * PI * Vec3d::UnitY()) * scale_transform(scale));

        m_raycasters[id++]->set_transform(trafo * translation_transform(m_grabber_connection_len * Vec3d::UnitZ()) *
                                          scale_transform(size));
        ...
        if (CutMode(m_mode) == CutMode::cutTongueAndGroove)
        {
            ...
            m_raycasters[id++]->set_transform(trafo * translation_transform(grabber_y_shift * Vec3d::UnitY()) *
                                              scale_transform(size));
            ...
            if (m_groove.angle > 0.0f)
            {
                ...
                offset = (size + xy_connection_len) * Vec3d::UnitY();
                m_raycasters[id++]->set_transform(trafo * translation_transform(offset) *
                                                  rotation_transform(-0.5 * PI * Vec3d::UnitX()) *
                                                  scale_transform(cone_scale));
            }
            else
            {
...
bool GLGizmoCut3D::on_is_activable() const
{
    ...
    if (const ModelObject *mo = m_parent.get_model()->objects[object_idx]; mo->is_cut() && mo->volumes.size() == 1)
    {
...
{
    return m_parent.get_mode() != comSimple;
}
...
void GLGizmoCut3D::dragging_grabber_move(const GLGizmoBase::UpdateData &data)
{
    ...
    if (m_hover_id == Z)
        starting_drag_position = translation_transform(m_plane_center) * m_rotation_m *
                                 (m_grabber_connection_len * Vec3d::UnitZ());
    else
...
void GLGizmoCut3D::on_start_dragging()
{
    ...
    if (m_hover_id >= m_connectors_group_id && m_connector_mode == CutConnectorMode::Manual)
        m_parent.take_gizmo_snapshot(_u8L("Move connector"));
...
void GLGizmoCut3D::on_stop_dragging()
{
    if (m_hover_id == X || m_hover_id == Y || m_hover_id == CutPlaneZRotation)
    {
        m_angle_arc.reset();
        m_angle = 0.0;
        m_parent.take_gizmo_snapshot(_u8L("Rotate cut plane"));
        m_start_dragging_m = m_rotation_m;
    }
                                                     ...
                                                     const Transform3d &rotation_m /* = Transform3d::Identity()*/) const
{
    ...
    const auto cut_matrix = Transform3d::Identity() * rotation_m.inverse() *
...
void GLGizmoCut3D::update_bb()
{
    ...
    if (!m_max_pos.isApprox(box.max) || !m_min_pos.isApprox(box.min))
    {
        ...
        if (const int object_idx = m_parent.get_selection().get_object_idx();
            object_idx >= 0 && !m_parent.get_model()->objects[object_idx]->cut_connectors.empty())
...
void GLGizmoCut3D::PartSelection::add_object(const ModelObject *object, double sla_shift_z)
{
    m_model = Model();
...
GLGizmoCut3D::PartSelection::PartSelection(const ModelObject *mo, const Transform3d &cut_matrix, int instance_idx_in,
                                           const Vec3d &center, const Vec3d &normal,
                                           const CommonGizmosDataObjects::ObjectClipper &oc, double sla_shift_z)
    : m_instance_idx(instance_idx_in)
{
    Cut cut(mo, instance_idx_in, cut_matrix);
    add_object(cut.perform_with_plane().front(), sla_shift_z);
    ...
    if (std::vector<Vec3d> pts = oc.point_per_contour(); !pts.empty())
    {
        ...
        for (size_t pt_idx = 0; pt_idx < pts.size(); ++pt_idx)
        {
            ...
            for (size_t part_id = 0; part_id < m_parts.size(); ++part_id)
            {
                ...
                const Transform3d &tr = (translation_transform(
                                             model_object()->instances[m_instance_idx]->get_offset()) *
...
GLGizmoCut3D::PartSelection::PartSelection(const ModelObject *object, int instance_idx_in, double sla_shift_z)
    : m_instance_idx(instance_idx_in)
{
    add_object(object, sla_shift_z);
...
void GLGizmoCut3D::PartSelection::render(const Vec3d *normal, GLModel &sphere_model, const Camera &camera,
                                         GLShaderProgram *shader)
{
    ...
    if (shader)
    {
        ...
        for (size_t id = 0; id < m_parts.size(); ++id)
        {
            ...
            m_parts[id].glmodel.set_color(m_parts[id].is_modifier
                                              ? MODIFIER_COLOR
...
void GLGizmoCut3D::PartSelection::toggle_selection(const Vec2d &mouse_pos, const Camera &camera)
{
        ...
        for (const auto &[parts_above, parts_below] : m_contour_to_parts)
        {
            for (size_t upper : parts_above)
            {
                ...
                if (std::find_if(parts_below.begin(), parts_below.end(), [this, &upper_sel](const size_t &i)
                                 { return m_parts[i].selected == upper_sel; }) != parts_below.end())
                {
...
void GLGizmoCut3D::on_render()
{
    ...
    if (!m_connectors_editing)
        m_part_selection.render(nullptr, m_sphere.model, m_parent.get_camera(), m_parent.get_shader("gouraud_light"));
    else
        m_part_selection.render(&m_cut_normal, m_sphere.model, m_parent.get_camera(),
                                m_parent.get_shader("gouraud_light"));
...
void GLGizmoCut3D::adjust_window_position(float x, float y, float bottom_limit)
{
    ...
    float x_pos = x - (last_w > 0 ? last_w : 400.0f * m_imgui->get_style_scaling());
...
void GLGizmoCut3D::render_connectors_input_window(CutConnectors &connectors)
{
    ...
    {
        m_parent.take_gizmo_snapshot(std::string(act_name.ToUTF8().data()));
        reset_connectors();
    }
    ...
    {
        render_snap_specific_input(_u8L("Bulge"), _L("Bulge proportion related to radius"), m_snap_bulge_proportion,
                                   0.15f, 5.f, 100.f * m_snap_space_proportion);
        render_snap_specific_input(_u8L("Space"), _L("Space proportion related to radius"), m_snap_space_proportion,
                                   0.3f, 10.f, 50.f);
    }
...
void GLGizmoCut3D::flip_cut_plane()
{
    ...
    m_parent.take_gizmo_snapshot(_u8L("Flip cut plane"));
    m_start_dragging_m = m_rotation_m;
...
void GLGizmoCut3D::process_contours()
{
    ...
    if (CutMode(m_mode) == CutMode::cutTongueAndGroove)
    {
        if (has_valid_groove())
        {
            ...
            {
                double sla_shift = selection.get_first_volume() ? selection.get_first_volume()->get_sla_shift_z() : 0.;
                m_part_selection = PartSelection(new_objects.front(), instance_idx, sla_shift);
            }
        }
    }
    else
    {
        reset_cut_by_contours();
        double sla_shift = selection.get_first_volume() ? selection.get_first_volume()->get_sla_shift_z() : 0.;
        m_part_selection = PartSelection(model_objects[object_idx], get_cut_matrix(selection), instance_idx,
                                         m_plane_center, m_cut_normal, *m_c->object_clipper(), sla_shift);
    }
...
void GLGizmoCut3D::render_groove_float_input(const std::string &label, float &in_val, const float &init_val,
                                             float &in_tolerance)
{
    ...
    if (render_slider_double_input(label, val, tolerance, -0.1f, std::min(0.3f * in_val, 1.5f)))
    {
        ...
        {
            m_parent.take_gizmo_snapshot(Slic3r::format(_u8L("Groove change") + ": %1%", label));
    ...
    {
        m_parent.take_gizmo_snapshot(act_name + ": " + label);
        in_val = init_val;
        in_tolerance = 0.1f;
        is_changed = true;
    }
...
bool GLGizmoCut3D::render_angle_input(const std::string &label, float &in_val, const float &init_val, float min_val,
                                      float max_val)
{
    bool is_changed{false};
    ...
    m_imgui->slider_float(("##angle_" + label).c_str(), &val, min_val, max_val, format.c_str(), 1.f, true,
                          from_u8(label));
    ...
    if (!is_approx(old_val, val))
    {
        if (m_imgui->get_last_slider_status().can_take_snapshot)
        {
            ...
            m_parent.take_gizmo_snapshot(_u8L("Edited") + ": " + label);
            m_imgui->get_last_slider_status().invalidate_snapshot();
            if (m_mode == size_t(CutMode::cutTongueAndGroove))
                m_groove_editing = true;
        }
...
void GLGizmoCut3D::render_groove_angle_input(const std::string &label, float &in_val, const float &init_val,
...
void GLGizmoCut3D::render_snap_specific_input(const std::string &label, const wxString &tooltip, float &in_val,
                                              const float &init_val, const float min_val, const float max_val)
{
    ImGuiPureWrap::text(label);
...
void GLGizmoCut3D::render_cut_plane_input_window(CutConnectors &connectors)
{
    ...
    if (mode == CutMode::cutPlanar || mode == CutMode::cutTongueAndGroove)
    {
        ...
        const bool is_cut_plane_init = m_rotation_m.isApprox(Transform3d::Identity()) &&
                                       m_bb_center.isApprox(m_plane_center);
        ...
        {
            m_parent.take_gizmo_snapshot(act_name);
            reset_cut_plane();
        }
        m_imgui->disabled_end();
        ...
        else if (mode == CutMode::cutTongueAndGroove)
        {
            ...
            render_groove_float_input(m_labels_map["Depth"], m_groove.depth, m_groove.depth_init,
                                      m_groove.depth_tolerance);
            render_groove_float_input(m_labels_map["Width"], m_groove.width, m_groove.width_init,
                                      m_groove.width_tolerance);
            render_groove_angle_input(m_labels_map["Flap Angle"], m_groove.flaps_angle, m_groove.flaps_angle_init, 30.f,
                                      120.f);
        ...
        auto render_part_name =
            [this, marker_size, has_connectors](const std::string &name, bool &keep_part, const ImU32 &color)
        {
            bool keep = true;
            add_horizontal_shift(m_imgui->scaled(1.2f));
            ImGuiPureWrap::checkbox((m_keep_as_parts ? _u8L("Part") : _u8L("Object")) + " " + name,
                                    has_connectors ? keep : keep_part);
            render_color_marker(marker_size, color);
        };

        auto render_part_actions = [this, h_shift](const std::string &suffix, const bool &keep_part,
                                                   bool &place_on_cut_part, bool &rotate_part)
        {
            ...
            if (ImGuiPureWrap::radio_button(m_part_orientation_names.at("none") + suffix,
                                            !place_on_cut_part && !rotate_part))
            {
                rotate_part = false;
...
{
    m_imperial_units = m_parent.app_config()->get_bool("use_inches");
...
void GLGizmoCut3D::render_input_window_warning() const
{
    ...
    if (!m_invalid_connectors_idxs.empty())
    {
        ...
        if (m_info_stats.outside_cut_contour > size_t(0))
            out += "\n - " +
                   GUI::format(_L_PLURAL("%1$d connector is out of cut contour",
                                         "%1$d connectors are out of cut contour", m_info_stats.outside_cut_contour),
                               m_info_stats.outside_cut_contour);
        if (m_info_stats.outside_bb > size_t(0))
            out += "\n - " + GUI::format(_L_PLURAL("%1$d connector is out of object",
                                                   "%1$d connectors are out of object", m_info_stats.outside_bb),
...
void GLGizmoCut3D::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    // adjust_window_position(x, y, bottom_limit); // Disabled - handled by SIMPLE-POSITIONING-CUT
    ...
    if (size.y > 0.0f && m_popup_height != size.y)
    {
        ...
        // Request extra frame if height changed on first render
...
bool GLGizmoCut3D::is_outside_of_cut_contour(size_t idx, const CutConnectors &connectors, const Vec3d cur_pos)
{
    ...
    const int sectorCount = shape == CutConnectorShape::Triangle ? 3
                            ...
                            : shape == CutConnectorShape::Circle
                                ? 60
                                : // supposably, 60 points are enough for conflict detection
...
bool GLGizmoCut3D::is_conflict_for_connector(size_t idx, const CutConnectors &connectors, const Vec3d cur_pos)
{
    ...
    const Transform3d matrix =
        translation_transform(cur_pos) * m_rotation_m *
...
void GLGizmoCut3D::toggle_model_objects_visibility()
{
    ...
    else if (!m_part_selection.valid() && !has_active_volume)
    {
        const Selection &selection = m_parent.get_selection();
        const ModelObjectPtrs &model_objects = selection.get_model()->objects;
        m_parent.toggle_model_objects_visibility(true, model_objects[selection.get_object_idx()],
...
void GLGizmoCut3D::render_connectors()
{
    ...
    for (size_t i = 0; i < connectors.size(); ++i)
    {
        ...
        const bool conflict_connector = std::binary_search(m_invalid_connectors_idxs.begin(),
                                                           m_invalid_connectors_idxs.end(), i);
        ...
        if (!m_connectors_editing)
            ...
            render_color = connector.attribs.type == CutConnectorType::Dowel ? SELECTED_DOWEL_COLOR
                                                                             : SELECTED_PLAG_COLOR;
...
static void check_objects_after_cut(const ModelObjectPtrs &objects, wxWindow *parent_window)
{
    ...
    if (!err_objects_names.empty())
    {
        ...
        WarningDialog(parent_window,
                      format_wxstr("Objects(%1%) have duplicated connectors. "
                                   "Some connectors may be missing in slicing result.\n"
                                   "Please report to preFlight team in which scenario this issue happened.\n"
                                   "Thank you.",
                                   names))
            .ShowModal();
    }
    ...
    {
        auto dlg = WarningDialog(parent_window,
        ...
        if (dlg.ShowModal() == wxID_YES)
        {
            ...
            auto fix_and_update_progress =
                [model_names, &objects](const int obj_idx, int model_idx, wxProgressDialog &progress_dlg,
                                        std::vector<std::string> &succes_models,
            ...
            wxProgressDialog progress_dlg(_L("Fixing by Windows repair algorithm"), "", 100,
                                          find_toplevel_parent(parent_window),
            ...
            InfoDialog(parent_window, _L("Repair operation finished"), msg).ShowModal();
        }
    }
}
...
void GLGizmoCut3D::perform_cut(const Selection &selection)
{
    ...
    ModelObject *mo = m_parent.get_model()->objects[object_idx];
    if (!mo)
        return;
    ...
    // perform cut
    {
        m_parent.take_typed_snapshot(_u8L("Cut by Plane"), static_cast<int>(UndoRedo::SnapshotType::Action));
        ...
        ModelObjectCutAttributes attributes =
            ...
            only_if(!has_connectors && !cut_with_groove && !cut_mo->cut_id.valid(),
        ...
        const ModelObjectPtrs &new_objects = cut_by_contour ? cut.perform_by_contour(m_part_selection.get_cut_parts(),
                                                                                     dowels_count)
                                             : cut_with_groove ? cut.perform_with_groove(m_groove, m_rotation_m)
                                                               : cut.perform_with_plane();

        check_objects_after_cut(new_objects, m_parent.get_wxglcanvas_parent());

        // save cut_id to post update synchronization
        const CutId cut_id = cut_mo->cut_id;

        // update cut results on plater and in the model
        m_parent.apply_cut_object_to_model(object_idx, new_objects);

        synchronize_model_after_cut(*m_parent.get_model(), cut_id);
    }
}
    ...
    for (const CutConnectorType &type : {CutConnectorType::Dowel, CutConnectorType::Plug, CutConnectorType::Snap})
        for (const CutConnectorStyle &style : {CutConnectorStyle::Frustum, CutConnectorStyle::Prism})
        {
            if (type == CutConnectorType::Dowel && style == CutConnectorStyle::Frustum)
                continue;
            for (const CutConnectorShape &shape : {CutConnectorShape::Circle, CutConnectorShape::Hexagon,
                                                   CutConnectorShape::Square, CutConnectorShape::Triangle})
            {
                ...
                m_shapes[attribs].mesh_raycaster = std::make_unique<MeshRaycaster>(
...
{
    CutConnectorAttributes attribs = {m_connector_type, CutConnectorStyle(m_connector_style),
...
bool GLGizmoCut3D::process_cut_line(SLAGizmoEventType action, const Vec2d &mouse_position)
{
    ...
    if (cut_line_processing())
    {
        ...
        if (action == SLAGizmoEventType::LeftDown || action == SLAGizmoEventType::LeftUp)
        {
            ...
            {
                m_parent.take_gizmo_snapshot(_u8L("Cut by line"));
...
bool GLGizmoCut3D::add_connector(CutConnectors &connectors, const Vec2d &mouse_position)
{
    ...
    {
        m_parent.take_gizmo_snapshot(_u8L("Add connector"));
        unselect_all_connectors();

        connectors.emplace_back(pos, m_rotation_m, m_connector_size * 0.5f, m_connector_depth_ratio,
                                m_connector_size_tolerance * 0.5f, m_connector_depth_ratio_tolerance, m_connector_angle,
...
bool GLGizmoCut3D::delete_selected_connectors(CutConnectors &connectors)
{
    ...
    m_parent.take_gizmo_snapshot(_u8L("Delete connector"));
    ...
    m_selected.erase(std::remove_if(m_selected.begin(), m_selected.end(),
                                    [](const auto &selected) { return selected; }),
                     m_selected.end());
    m_selected_count = 0;
...
bool GLGizmoCut3D::gizmo_event(SLAGizmoEventType action, const Vec2d &mouse_position, bool shift_down, bool alt_down,
                               bool control_down)
{
    ...
    if ((m_hover_id < 0 || m_hover_id == CutPlane) && shift_down && !m_connectors_editing &&
        (action == SLAGizmoEventType::LeftDown || action == SLAGizmoEventType::LeftUp ||
...
void GLGizmoCut3D::apply_cut_connectors(ModelObject *mo, const std::string &connector_name)
{
    ...
    for (const CutConnector &connector : mo->cut_connectors)
    {
        ...
        new_volume->set_transformation(
            translation_transform(connector.pos) * connector.rotation_m *
            rotation_transform(-connector.z_angle * Vec3d::UnitZ()) *
