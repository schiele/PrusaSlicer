...
bool dragging(const Vec2d &mouse_pos, const Camera &camera,
...
bool face_selected_volume_to_camera(const Camera &camera, GLCanvas3D &canvas,
                                    const std::optional<double> &wanted_up_limit)
{
    ...
    Transform3d new_volume_tr = get_volume_transformation(world_tr, wanted_direction, world_position, fix,
                                                          instance_tr_inv, current_angle, wanted_up_limit);
...
{
    return is_embossed_object(selection) ? TransformationType::Instance_Relative_Joint
                                         : TransformationType::Local_Relative_Joint;
}

void dragging_rotate_gizmo(double gizmo_angle, std::optional<float> &current_angle, std::optional<float> &start_angle,
                           Selection &selection)
{
...
bool dragging(const Vec2d &mouse_pos, const Camera &camera, SurfaceDrag &surface_drag, GLCanvas3D &canvas,
              const RaycastManager &raycast_manager, const std::optional<double> &up_limit)
{
    Vec2d offseted_mouse = mouse_pos + surface_drag.mouse_offset_without_sla_shift;
    std::optional<RaycastManager::Hit> hit = ray_from_camera(raycast_manager, offseted_mouse, camera,
                                                             &surface_drag.condition);
    ...
    if (volume != nullptr && volume->emboss_shape.has_value() && volume->emboss_shape->fix_3mf_tr.has_value())
        fix = volume->emboss_shape->fix_3mf_tr;
    Transform3d volume_new = get_volume_transformation(surface_drag.world, hit->normal, hit->position, fix,
                                                       surface_drag.instance_inv, surface_drag.start_angle, up_limit);
