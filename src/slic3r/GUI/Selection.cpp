...
Selection::VolumeCache::VolumeCache(const Geometry::Transformation &volume_transform,
                                    const Geometry::Transformation &instance_transform)
...
void Selection::add(unsigned int volume_idx, bool as_single_selection, bool check_for_already_contained)
{
    ...
    if ((PrinterTechnology) printer_technology() == ptSLA && volume->is_modifier &&
...
void Selection::set_deserialized(EMode mode, const std::vector<std::pair<size_t, size_t>> &volumes_and_instances)
{
    ...
    m_list.clear();
    for (unsigned int i = 0; i < (unsigned int) m_volumes->size(); ++i)
        if (std::binary_search(volumes_and_instances.begin(), volumes_and_instances.end(),
                               (*m_volumes)[i]->geometry_id))
            do_add_volume(i);
    update_type();
...
void Selection::clear()
{
    ...
    if (!m_obj_manipul_reset_cache)
        return;

    // resets the cache in the sidebar
    if (m_obj_manipul_reset_cache)
        m_obj_manipul_reset_cache();
...
void Selection::instances_changed(const std::vector<size_t> &instance_ids_selected)
{
    ...
    const PrinterTechnology pt = (PrinterTechnology) printer_technology();
    ...
    for (unsigned int volume_idx = 0; volume_idx < (unsigned int) m_volumes->size(); ++volume_idx)
    {
        ...
        if (pt == ptSLA && volume->is_modifier &&
            m_model->objects[volume->object_idx()]->volumes[volume->volume_idx()]->is_modifier())
            continue;
        auto it = std::lower_bound(instance_ids_selected.begin(), instance_ids_selected.end(),
...
const BoundingBoxf3 &Selection::get_unscaled_instance_bounding_box() const
{
    ...
    {
        std::optional<BoundingBoxf3> *bbox = const_cast<std::optional<BoundingBoxf3> *>(
        ...
        *bbox = BoundingBoxf3();
        if (m_valid)
        {
            for (unsigned int i : m_list)
            {
                ...
                if (volume.is_modifier)
                    continue;
                Transform3d trafo = volume.get_instance_transformation().get_matrix() *
                                    volume.get_volume_transformation().get_matrix();
                trafo.translation().z() += volume.get_sla_shift_z();
                (*bbox)->merge(volume.transformed_convex_hull_bounding_box(trafo));
            }
...
const std::pair<BoundingBoxf3, Transform3d> &Selection::get_bounding_box_in_current_reference_system() const
{
    ...
    ECoordinatesType coordinates_type = get_coordinates_type();
...
BoundingBoxf Selection::get_screen_space_bounding_box()
{
    ...
    if (!is_empty())
    {
        ...
        for (const Vec3d &v : vertices)
        {
            ...
            const Vec2d ss = Vec2d(half_w * ndc.x() + double(viewport[0]) + half_w,
                                   h - (half_h * ndc.y() + double(viewport[1]) + half_h));
            ss_box.merge(ss);
        }
    }
...
const std::pair<Vec3d, double> Selection::get_bounding_sphere() const
{
    ...
    {
        std::optional<std::pair<Vec3d, double>> *sphere = const_cast<std::optional<std::pair<Vec3d, double>> *>(
            &m_bounding_sphere);
...
void Selection::translate(const Vec3d &displacement, TransformationType transformation_type)
{
        ...
        if (m_mode == Instance && !is_wipe_tower())
        {
            ...
            if (transformation_type.instance())
            {
                ...
                transform_instance_relative(v, volume_data, transformation_type,
                                            Geometry::translation_transform(displacement), m_cache.dragging_center);
        }
        else
        {
            if (transformation_type.local() && transformation_type.absolute())
            {
                ...
                const Geometry::Transformation &inst_trafo = volume_data.get_instance_transform();
                v.set_volume_offset(vol_trafo.get_offset() + inst_trafo.get_scaling_factor_matrix().inverse() *
                                                                 vol_trafo.get_rotation_matrix() * displacement);
            }
            else
            {
                Vec3d relative_disp = displacement;
                if (transformation_type.world() && transformation_type.instance())
                    relative_disp = volume_data.get_instance_transform().get_scaling_factor_matrix().inverse() *
                                    relative_disp;

                transform_volume_relative(v, volume_data, transformation_type,
                                          Geometry::translation_transform(relative_disp), m_cache.dragging_center);
            }
        }
    }
    ...
    set_bounding_boxes_dirty();
    m_canvas->requires_check_outside_state();
}
    ...
    for (unsigned int i : m_list)
    {
        ...
        if (m_mode == Instance && !is_wipe_tower())
        {
            ...
            if (transformation_type.instance())
            {
                ...
                const Vec3d inst_pivot = inst_trafo.get_matrix_no_offset().inverse() *
                                         (m_cache.rotation_pivot - inst_trafo.get_offset());
                ...
                if (!requires_general_synchronization)
                {
                    ...
                    const Transform3d new_world_rotation_matrix =
                        (inst_trafo * Geometry::Transformation(rotation_matrix) * vol_trafo).get_rotation_matrix();
                    if (std::abs((old_world_rotation_matrix * Vec3d::UnitX()).z() -
                                 (new_world_rotation_matrix * Vec3d::UnitX()).z()) > EPSILON)
                        requires_general_synchronization = true;
                    else if (std::abs((old_world_rotation_matrix * Vec3d::UnitY()).z() -
                                      (new_world_rotation_matrix * Vec3d::UnitY()).z()) > EPSILON)
                        requires_general_synchronization = true;
                    else if (std::abs((old_world_rotation_matrix * Vec3d::UnitZ()).z() -
                                      (new_world_rotation_matrix * Vec3d::UnitZ()).z()) > EPSILON)
                        requires_general_synchronization = true;
                }
            }
            ...
            else
            {
                ...
                else
                {
                    if (transformation_type.local())
                    {
                        ...
                        rotation_matrix = vol_matrix_no_offset.inverse() * inst_scale_matrix.inverse() *
                                          vol_rotation_matrix * rotation_matrix * vol_rotation_matrix.inverse() *
                                          inst_scale_matrix * vol_matrix_no_offset;
                    }
                }
                transform_volume_relative(v, volume_data, transformation_type, rotation_matrix, m_cache.rotation_pivot);
            }
...
void Selection::flattening_rotate(const Vec3d &normal)
{
    ...
    for (unsigned int i : m_list)
    {
        ...
        const Transform3d rotation_matrix = Transform3d(
            Eigen::Quaterniond().setFromTwoVectors(tnormal, -Vec3d::UnitZ()));
        v.set_instance_transformation(old_inst_trafo.get_offset_matrix() * rotation_matrix *
                                      old_inst_trafo.get_matrix_no_offset());
    }
...
void Selection::scale_to_fit_print_volume(const BuildVolume &volume)
{
    auto fit = [this](double s, Vec3d offset, bool undoredo_snapshot)
    {
        ...
        if (undoredo_snapshot)
            m_event_poster->postEvent(CanvasEventType::TakeSnapshot, _u8L("Scale To Fit"));
        ...
        m_canvas->do_scale(""); // avoid storing another snapshot
        ...
        m_canvas->do_move(""); // avoid storing another snapshot
    ...
    auto fit_circle = [this, fit](const BuildVolume &volume, bool undoredo_snapshot, double *max_height = nullptr)
    {
        ...
        const Vec3d offset = s * (Vec3d(unscale<double>(circle.center.x()), unscale<double>(circle.center.y()),
                                        0.5 * max_z) -
                                  sel_center);
        const Vec3d print_center = {unscale<double>(print_circle.center.x()), unscale<double>(print_circle.center.y()),
                                    0.5 * volume.max_print_height()};
    ...
    if ((PrinterTechnology) printer_technology() == ptSLA)
    {
    ...
    if ((PrinterTechnology) printer_technology() == ptFFF)
    {
        // check whether the top layer exceeds the maximum height of the print volume
        // and, in case, reduce the scale accordingly
        const auto [slicing_parameters, profile] = m_canvas->get_layers_height_data(get_object_idx());
...
void Selection::scale_and_translate(const Vec3d &scale, const Vec3d &world_translation,
    ...
    for (unsigned int i : m_list)
    {
        ...
        if (m_mode == Instance)
        {
            if (transformation_type.instance())
            {
                ...
                v.set_instance_transformation(Geometry::translation_transform(world_inst_pivot) * offset_trafo *
                                              Transform3d(inst_rotation) * scale_trafo *
                                              Geometry::translation_transform(-local_inst_pivot));
            }
            else
                transform_instance_relative(v, volume_data, transformation_type,
                                            Geometry::translation_transform(world_translation) *
                                                Geometry::scale_transform(relative_scale),
        ...
        else
        {
            ...
            else
            {
                ...
                transform_volume_relative(v, volume_data, transformation_type,
                                          Geometry::translation_transform(translation) *
                                              Geometry::scale_transform(scale),
                                          m_cache.dragging_center);
            }
        }
    ...
    for (unsigned int i : m_list)
    {
        ...
        if (world_trafo.has_skew())
        {
            if (!inst_trafo.has_skew() && !vol_trafo.has_skew())
            {
                ...
                v.set_volume_transformation(vol_trafo.get_offset_matrix() *
                                            inst_trafo.get_matrix_no_offset().inverse() * world_trafo.get_matrix());
            }
            else
            {
                // <W> = <I><V>
...
void Selection::translate(unsigned int object_idx, unsigned int instance_idx, const Vec3d &displacement)
{
    ...
    for (unsigned int i : m_list)
    {
        ...
        if (v.object_idx() == (int) object_idx && v.instance_idx() == (int) instance_idx)
            v.set_instance_transformation(Geometry::translation_transform(displacement) *
                                          v.get_instance_transformation().get_matrix());
    }
...
{
    if ((is_single_full_instance() && is_world_coordinates()) ||
        (is_single_volume_or_modifier() && !is_local_coordinates()))
    {
        ...
        {
            MessageDialog dlg(
                m_canvas->get_wxglcanvas_parent(),
                ...
                SLIC3R_APP_NAME, wxYES_NO | wxNO_DEFAULT | wxICON_QUESTION);
            if (dlg.ShowModal() != wxID_YES)
                return -1;

            m_event_poster->postEvent(CanvasEventType::TakeSnapshot, _u8L("Bake transform"));

            // Bake the rotation into the meshes of the object.
            m_model->objects[volume.composite_id.object_id]->bake_xy_rotation_into_meshes(
                volume.composite_id.instance_id);
            // Update the 3D scene, selections etc.
            m_event_poster->postEvent(CanvasEventType::ForceUpdate);
            return 0;
        }
    }
...
void Selection::erase()
{
    ...
    else if (is_multiple_full_instance())
    {
        ...
        for (ObjectIdxsToInstanceIdxsMap::iterator obj_it = m_cache.content.begin(); obj_it != m_cache.content.end();
             ++obj_it)
        {
            for (InstanceIdxsList::reverse_iterator inst_it = obj_it->second.rbegin(); inst_it != obj_it->second.rend();
                 ++inst_it)
            {
    ...
    else if (is_mixed())
    {
        ...
        for (auto i : m_list)
        {
            ...
            if (model_object->instances.size() == 1)
            {
                ...
                else
                {
                    ...
                    int idx = (volumes_in_obj.find(glv_obj_idx) == volumes_in_obj.end())
                                  ? 0
                                  : volumes_in_obj.at(glv_obj_idx);
        ...
        for (const ItemForDelete &i : items_set)
        {
            ...
            {
                const int vol_in_obj_cnt = volumes_in_obj.find(i.obj_idx) == volumes_in_obj.end()
                                               ? 0
                                               : volumes_in_obj.at(i.obj_idx);
...
void Selection::copy_to_clipboard()
{
    ...
    for (const ObjectIdxsToInstanceIdxsMap::value_type &object : m_cache.content)
    {
        ...
        dst_object->brim_points_status = src_object->brim_points_status;
...
std::vector<unsigned int> Selection::get_volume_idxs_from_instance(unsigned int object_idx,
                                                                   unsigned int instance_idx) const
{
    std::vector<unsigned int> idxs;
    for (unsigned int i = 0; i < (unsigned int) m_volumes->size(); ++i)
    {
...
std::vector<unsigned int> Selection::get_volume_idxs_from_volume(unsigned int object_idx, unsigned int instance_idx,
...
void Selection::update_type()
{
    ...
    else
    {
        ...
        else
        {
                ...
                else if (selected_instances_count == 1)
                {
                    ...
                    else
                    {
                        ...
                        requires_disable = true;
                         ...
                         selected_instances_count * model_volumes_count + sla_volumes_count ==
                             (unsigned int) m_list.size())
                {
            ...
            else
            {
                ...
                for (ObjectIdxsToInstanceIdxsMap::iterator it = m_cache.content.begin(); it != m_cache.content.end();
                     ++it)
                {
    ...
    for (GLVolume *v : *m_volumes)
    {
        ...
        if (!v->disabled && v->is_modifier && v->object_idx() >= 0)
        {
            int oi = v->object_idx();
            int vi = v->volume_idx();
            if (oi < (int) m_model->objects.size() && vi >= 0 && vi < (int) m_model->objects[oi]->volumes.size() &&
...
void Selection::render_synchronized_volumes()
{
    ...
    const ECoordinatesType coordinates_type = get_coordinates_type();
    BoundingBoxf3 box;
    Transform3d trafo;
...
void Selection::render_sidebar_position_hints(const std::string &sidebar_field, GLShaderProgram &shader,
...
void Selection::render_sidebar_rotation_hints(const std::string &sidebar_field, GLShaderProgram &shader,
                                              const Transform3d &matrix)
{
    auto render_sidebar_rotation_hint =
        [this](GLShaderProgram &shader, const Transform3d &view_matrix, const Transform3d &model_matrix)
    {
        shader.set_uniform("view_model_matrix", view_matrix * model_matrix);
        ...
        view_normal_matrix = view_matrix.matrix().block(0, 0, 3, 3) *
                             matrix.matrix().block(0, 0, 3, 3).inverse().transpose();
        shader.set_uniform("view_normal_matrix", view_normal_matrix);
        m_curved_arrow.render();
    };
    ...
    if (boost::ends_with(sidebar_field, "x"))
    {
        ...
        render_sidebar_rotation_hint(shader, view_matrix,
                                     matrix * Geometry::rotation_transform(0.5 * PI * Vec3d::UnitY()));
    }
    else if (boost::ends_with(sidebar_field, "y"))
    {
        m_curved_arrow.set_color(get_color(Y));
        render_sidebar_rotation_hint(shader, view_matrix,
                                     matrix * Geometry::rotation_transform(-0.5 * PI * Vec3d::UnitX()));
    }
    else if (boost::ends_with(sidebar_field, "z"))
    {
...
void Selection::render_sidebar_scale_hints(const std::string &sidebar_field, GLShaderProgram &shader,
                                           const Transform3d &matrix)
{
    const bool uniform_scale = get_uniform_scaling();

    auto render_sidebar_scale_hint = [this, uniform_scale](Axis axis, GLShaderProgram &shader,
                                                           ...
                                                           const Transform3d &model_matrix)
    {
        ...
        matrix = model_matrix * Geometry::translation_transform(-5.0 * Vec3d::UnitY()) *
                 Geometry::rotation_transform(PI * Vec3d::UnitZ());
        shader.set_uniform("view_model_matrix", view_matrix * matrix);
        view_normal_matrix = view_matrix.matrix().block(0, 0, 3, 3) *
                             matrix.matrix().block(0, 0, 3, 3).inverse().transpose();
        shader.set_uniform("view_normal_matrix", view_normal_matrix);
    ...
    if (boost::ends_with(sidebar_field, "x") || uniform_scale)
        render_sidebar_scale_hint(X, shader, view_matrix,
                                  matrix * Geometry::rotation_transform(-0.5 * PI * Vec3d::UnitZ()));

    if (boost::ends_with(sidebar_field, "y") || uniform_scale)
        render_sidebar_scale_hint(Y, shader, view_matrix, matrix);

    if (boost::ends_with(sidebar_field, "z") || uniform_scale)
        render_sidebar_scale_hint(Z, shader, view_matrix,
                                  matrix * Geometry::rotation_transform(0.5 * PI * Vec3d::UnitX()));
}
...
void Selection::render_sidebar_layers_hints(const std::string &sidebar_field, GLShaderProgram &shader)
{
    ...
    const bool camera_on_top = get_camera().is_looking_downward();
        ...
        if (ImGui::CollapsingHeader(name.c_str(), ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            else
            {
                ...
                if (svd.rotation)
                {
                    ...
                    imgui.text_colored(ImGuiWrapper::COL_ORANGE_LIGHT,
                                       svd.rotation_90_degrees ? "Rotation 90 degs" : "Rotation");
                    spacing = 10.0f;
                }
                if (svd.scale)
                {
                    ImGui::SameLine(0.0f, spacing);
                    imgui.text_colored(ImGuiWrapper::COL_ORANGE_LIGHT,
                                       svd.anisotropic_scale ? "Anisotropic scale" : "Isotropic scale");
                    spacing = 10.0f;
                }
                if (svd.mirror)
                {
...
static bool is_rotation_xy_synchronized(const Transform3d::ConstLinearPart &trafo_from,
                                        const Transform3d::ConstLinearPart &trafo_to)
{
    ...
    return
        ...
        is_approx(rot(0, 0), rot(1, 1), eps) && is_approx(rot(0, 1), -rot(1, 0), eps) &&
        is_approx(rot(2, 2), 1., eps) &&
        // Rest should be zeros.
        is_approx(rot(0, 2), 0., eps) && is_approx(rot(1, 2), 0., eps) && is_approx(rot(2, 0), 0., eps) &&
...
static void verify_instances_rotation_synchronized(const Model &model, const GLVolumePtrs &volumes)
{
    for (int idx_object = 0; idx_object < int(model.objects.size()); ++idx_object)
    {
        ...
        if (idx_volume_first == -1)
            continue;
        const Transform3d::ConstLinearPart &rotation0 =
            volumes[idx_volume_first]->get_instance_transformation().get_matrix().linear();
        for (int i = idx_volume_first + 1; i < (int) volumes.size(); ++i)
            if (volumes[i]->object_idx() == idx_object && volumes[i]->volume_idx() >= 0)
            {
                const Transform3d::ConstLinearPart &rotation =
                    volumes[i]->get_instance_transformation().get_matrix().linear();
                assert(is_rotation_xy_synchronized(rotation, rotation0));
            }
    }
}
    ...
    for (unsigned int i : m_list)
    {
        ...
        for (unsigned int j = 0; j < (unsigned int) m_volumes->size(); ++j)
        {
            ...
            if (sync_rotation_type == SyncRotationType::RESET)
            {
                ...
                new_inst_trafo_j.linear() = (old_inst_trafo_j.linear() * old_inst_trafo_i.linear().inverse()) *
                                            curr_inst_trafo_i.linear();
...
void Selection::ensure_on_bed()
{
    ...
    for (size_t i = 0; i < m_volumes->size(); ++i)
    {
        ...
        if (!volume->is_wipe_tower() && !volume->is_modifier &&
            std::find(m_cache.sinking_volumes.begin(), m_cache.sinking_volumes.end(), i) ==
                m_cache.sinking_volumes.end())
        {
...
bool Selection::is_from_fully_selected_instance(unsigned int volume_idx) const
{
    struct SameInstance
    {
        ...
        SameInstance(int obj_idx, int inst_idx, GLVolumePtrs &volumes)
            : obj_idx(obj_idx), inst_idx(inst_idx), volumes(volumes)
        {
        }
        bool operator()(unsigned int i)
        {
            return (volumes[i]->volume_idx() >= 0) && (volumes[i]->object_idx() == obj_idx) &&
                   (volumes[i]->instance_idx() == inst_idx);
        }
    };
    ...
    PrinterTechnology pt = (PrinterTechnology) printer_technology();
    const ModelVolumePtrs &volumes = m_model->objects[object_idx]->volumes;
    const unsigned int vol_cnt = (unsigned int) std::count_if(volumes.begin(), volumes.end(),
                                                              [pt](const ModelVolume *volume)
                                                              { return pt == ptFFF || !volume->is_modifier(); });
    ...
    if (src_object != nullptr)
    {
        ...
        for (ModelVolume *src_volume : src_object->volumes)
        {
            ...
            if (from_same_object)
            {
                ...
                //                double offset = m_canvas->get_size_proportional_to_max_bed_size(0.05);
        ...
        if (!from_same_object)
        {
            for (ModelVolume *v : volumes)
            {
                v->set_offset((v->get_offset() - total_bb.center()) +
                              dst_matrix.inverse() *
                                  (Vec3d(dst_instance_bb.max(0), dst_instance_bb.min(1), dst_instance_bb.min(2)) +
...
void Selection::transform_instance_relative(GLVolume &volume, const VolumeCache &volume_data,
                                            TransformationType transformation_type, const Transform3d &transform,
                                            const Vec3d &world_pivot)
{
    ...
    if (transformation_type.world())
    {
        const Vec3d inst_pivot = transformation_type.independent() && !is_from_single_instance()
                                     ? inst_trafo.get_offset()
                                     : world_pivot;
        const Transform3d trafo = Geometry::translation_transform(inst_pivot) * transform *
                                  Geometry::translation_transform(-inst_pivot);
...
void Selection::transform_volume_relative(GLVolume &volume, const VolumeCache &volume_data,
                                          TransformationType transformation_type, const Transform3d &transform,
                                          const Vec3d &world_pivot)
{
    assert(transformation_type.relative());
    ...
    if (transformation_type.world())
    {
        const Vec3d inst_pivot = transformation_type.independent()
                                     ? vol_trafo.get_offset()
                                     : (Vec3d) (inst_trafo.get_matrix().inverse() * world_pivot);
        const Transform3d inst_matrix_no_offset = inst_trafo.get_matrix_no_offset();
        const Transform3d trafo = Geometry::translation_transform(inst_pivot) * inst_matrix_no_offset.inverse() *
                                  transform * inst_matrix_no_offset * Geometry::translation_transform(-inst_pivot);
