...
void GLTexture::Compressor::send_compressed_data_to_gpu()
{
    ...
    for (int i = 0; i < num_compressed; ++i)
    {
        ...
        if (!level.sent_to_gpu && !level.compressed_data.empty())
        {
            ...
            glsafe(
                ::glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, (i > 0) ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR));
...
void GLTexture::Compressor::compress()
{
    ...
    for (Level &level : m_levels)
    {
        ...
        level.compressed_data = std::vector<unsigned char>(std::max((unsigned int) 64,
                                                                    (unsigned int) level.src_data.size() / 2),
                                                           0);
...
bool GLTexture::load_from_file(const std::string &filename, bool use_mipmaps, ECompressionType compression_type,
...
bool GLTexture::load_from_svg_file(const std::string &filename, bool use_mipmaps, bool compress, bool apply_anisotropy,
                                   unsigned int max_size_px)
{
    reset();
...
bool GLTexture::load_from_svg_files_as_sprites_array(const std::vector<std::string> &filenames,
                                                     ...
                                                     unsigned int sprite_size_px, bool compress)
{
    reset();
    ...
    for (const std::string &filename : filenames)
    {
        ...
        for (const std::pair<int, bool> &state : states)
        {
            ...
            if (state.second)
            {
                ...
                for (unsigned int r = 1; r <= sprite_size_px; ++r)
                {
                    ...
                    for (unsigned int c = 1; c <= sprite_size_px; ++c)
                    {
                        ...
                        output_data.data()[offset + 3] = (unsigned char) (128 * (1.0f - alpha) +
                                                                          output_data.data()[offset + 3] * alpha);
                    }
                }
            }
            ...
            {
                ::memcpy((void *) &data.data()[(state_offset_px + j * m_width) * 4],
                         (const void *) &output_data.data()[j * sprite_stride], sprite_stride);
            }
        }
    ...
    if (compress && OpenGLManager::are_compressed_textures_supported())
        glsafe(::glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, (GLsizei) m_width, (GLsizei) m_height,
                              0, GL_RGBA, GL_UNSIGNED_BYTE, (const void *) data.data()));
    else
        glsafe(::glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (GLsizei) m_width, (GLsizei) m_height, 0, GL_RGBA,
...
void GLTexture::render_sub_texture(unsigned int tex_id, float left, float right, float bottom, float top,
                                   const GLTexture::Quad_UVs &uvs)
{
...
bool GLTexture::load_from_png(const std::string &filename, bool use_mipmaps, ECompressionType compression_type,
                              bool apply_anisotropy)
{
    ...
    {
        if (to_squared_power_of_two(boost::filesystem::path(filename).filename().string(),
                                    OpenGLManager::get_gl_info().get_max_tex_size(), m_width, m_height))
    ...
    if (compression_enabled)
    {
        if (compression_type == SingleThreaded)
            glsafe(::glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, (GLsizei) m_width,
                                  (GLsizei) m_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, (const void *) data.data()));
        else
        {
            // initializes the texture on GPU
            glsafe(::glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, (GLsizei) m_width,
                                  (GLsizei) m_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0));
    ...
    if (use_mipmaps)
    {
        ...
        while (lod_w > 1 || lod_h > 1)
        {
            ...
            {
                if (compression_type == SingleThreaded)
                    glsafe(::glTexImage2D(GL_TEXTURE_2D, level, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, (GLsizei) m_width,
                                          (GLsizei) m_height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                                          (const void *) data.data()));
                else
                {
                    // initializes the texture on GPU
                    glsafe(::glTexImage2D(GL_TEXTURE_2D, level, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, (GLsizei) lod_w,
                                          (GLsizei) lod_h, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0));
                    // and send the uncompressed data to the compressor
...
bool GLTexture::load_from_svg(const std::string &filename, bool use_mipmaps, bool compress, bool apply_anisotropy,
                              unsigned int max_size_px)
{
