...
void Chart::draw()
{
    ...
    {
        dc.DrawText(_(L("NO RAMMING AT ALL")), wxPoint(m_rect.GetLeft() + m_rect.GetWidth() / 2 - legend_side,
    ...
    if (!m_line_to_draw.empty())
    {
        ...
        {
            if (splines)
                dc.DrawLine(m_rect.GetLeft() + i, (m_line_to_draw)[i], m_rect.GetLeft() + i + 1,
                            (m_line_to_draw)[i + 1]);
            else
            {
                dc.DrawLine(m_rect.GetLeft() + i, (m_line_to_draw)[i], m_rect.GetLeft() + i + 1, (m_line_to_draw)[i]);
                dc.DrawLine(m_rect.GetLeft() + i + 1, (m_line_to_draw)[i], m_rect.GetLeft() + i + 1,
                            (m_line_to_draw)[i + 1]);
            }
        }
    }
    ...
    for (float math_x = int(visible_area.m_x * 10) / 10; math_x < (visible_area.m_x + visible_area.m_width);
         math_x += 0.1f)
    {
    ...
    dc.DrawText(label, wxPoint(0.5 * (m_rect.GetRight() + m_rect.GetLeft()) - text_width / 2.f,
                               m_rect.GetBottom() + 0.5 * legend_side));
...
void Chart::recalculate_line()
{
    ...
    if (points.size() > 1)
    {
        ...
        if (boundary_first_derivative)
        {
            ...
            mu[N] = 1;
            rhs[0] = (6.f / h[1]) *
                     (float(points[0].y - points[1].y) / (points[0].x - points[1].x) - endpoints_derivative);
            rhs[N] = (6.f / h[N]) *
                     (endpoints_derivative - float(points[N - 1].y - points[N].y) / (points[N - 1].x - points[N].x));
        }
        else
        {
            lambda[0] = 0;
        ...
        for (int x = m_rect.GetLeft(); x <= m_rect.GetRight(); ++x)
        {
            ...
            m_total_volume += (m_rect.GetBottom() - m_line_to_draw.back()) *
                              (visible_area.m_width / m_rect.GetWidth()) * (visible_area.m_height / m_rect.GetHeight());
        }
    }
