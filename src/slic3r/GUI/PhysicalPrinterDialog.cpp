...
PresetForPrinter::PresetForPrinter(PhysicalPrinterDialog *parent, const std::string &preset_name) : m_parent(parent)
{
    ...
    m_presets_list->set_selection_changed_function(
        [this](int selection)
        {
            std::string selected_string = Preset::remove_suffix_modified(
                m_presets_list->GetString(selection).ToUTF8().data());
...
PhysicalPrinterDialog::PhysicalPrinterDialog(wxWindow *parent, wxString printer_name)
    : DPIDialog(parent, wxID_ANY, _L("Physical Printer"), wxDefaultPosition, wxSize(45 * wxGetApp().em_unit(), -1),
...
void PhysicalPrinterDialog::update_printers()
{
    ...
    catch (const HostNetworkError &err)
    {
        printers.clear();
        show_error(this,
                   _L("Connection to printers connected via the print host failed.") + "\n\n" + from_u8(err.what()));
    }
...
void PhysicalPrinterDialog::build_printhost_settings(ConfigOptionsGroup *m_optgroup)
{
    ...
    auto create_sizer_with_btn =
        [](wxWindow *parent, ScalableButton **btn, const std::string &icon_name, const wxString &label)
    {
    ...
    auto printhost_browse = [=](wxWindow *parent)
    {
        ...
        m_printhost_browse_btn->Bind(wxEVT_BUTTON,
                                     [=](wxCommandEvent &e)
                                     {
    ...
    auto print_host_test = [=](wxWindow *parent)
    {
        ...
        m_printhost_test_btn->Bind(wxEVT_BUTTON,
    ...
    auto api_key_copy = [=](wxWindow *parent)
    {
        ...
        m_api_key_copy_btn->Bind(wxEVT_BUTTON,
                                 [=](wxCommandEvent &e)
                                 {
                                     if (Field *apikey_field = m_optgroup->get_field("printhost_apikey"); apikey_field)
                                     {
                                         if (wxTextCtrl *temp = dynamic_cast<wxTextCtrl *>(apikey_field->getWindow());
                                             temp)
                                         {
                                             wxTheClipboard->Open();
    ...
    if (Http::ca_file_supported())
    {
        ...
        auto printhost_cafile_browse = [=](wxWindow *parent)
        {
            auto sizer = create_sizer_with_btn(parent, &m_printhost_cafile_browse_btn, "browse",
                                               _L("Browse") + " " + dots);
            m_printhost_cafile_browse_btn->Bind(
                wxEVT_BUTTON,
                [this, m_optgroup](wxCommandEvent e)
                {
    ...
    if (is_secret_store_ok())
    {
        ...
        line.widget = [ca_file_hint](wxWindow *parent)
        {
            wxString info = GUI::format_wxstr(
                L"%1%:\n\t%2%\n", _L("Storing passwords"),
                GUI::format_wxstr(
                    _L("On this system, %1% uses the system password store to safely store and read passwords and API keys."),
                    SLIC3R_APP_NAME));
    ...
    if (printhost_field)
    {
        ...
        if (temp)
        {
            temp->Bind(wxEVT_TEXT,
                       (
                           [printhost_field, temp](wxEvent &e)
                           {
#ifndef __WXGTK__
                               e.Skip();
...
void PhysicalPrinterDialog::update_host_type(bool printer_change)
{
...
#if 0  // Functions replaced with model_supports_service as all new printers will support both services.
    // allowed models are: all MINI, all MK3 and newer, MK2.5 and MK2.5S  
    auto model_supports_link = [](const std::string& model) {
        return model.size() >= 2 &&
    ...
    for (PresetForPrinter *prstft : m_presets)
    {
        ...
        if (!printer_with_vendor.vendor || printer_with_vendor.vendor->name.find("oozeBot") == std::string::npos)
        {
            connect.supported = false;
            break;
        }
        // model id should be enough for this case
        if (!model_supports_service(model_id))
        {
            connect.supported = false;
            break;
        }
...
void PhysicalPrinterDialog::OnOK(wxEvent &event)
{
    ...
    if (existing && into_u8(printer_name) != printers.get_selected_printer_name())
    {
        wxString msg_text = from_u8(
            (boost::format(_u8L("Printer with name \"%1%\" already exists.")) % existing->name /*printer_name*/).str());
        msg_text += "\n" + _L("Replace?");
    ...
    if (!repeat_presets.empty())
    {
        ...
        wxString msg_text = format_wxstr(_L_PLURAL("Following printer preset is duplicated:%1%"
                                                   ...
                                                   "The above presets for printer \"%2%\" will be used just once.",
                                                   repeat_cnt),
                                         repeatable_presets, printer_name);
    ...
    {
        if (save_secret(m_printer.name, "printhost_password", m_printer.config.opt_string("printhost_user"),
                        m_printer.config.opt_string("printhost_password")))
        {
