...
wxFont get_default_font_for_dpi(const wxWindow *window, int dpi)
{
    ...
    static auto SystemParametersInfoForDpi_fn =
        winapi_get_function<SystemParametersInfoForDpi_t>(L"User32.dll", "SystemParametersInfoForDpi");
...
void update_dark_ui(wxWindow *window)
{
    ...
    // Check if the window is a top-level window (dialog or frame) and apply dark title bar
    if (wxTopLevelWindow *tlw = dynamic_cast<wxTopLevelWindow *>(window))
    {
...
CheckboxFileDialog::CheckboxFileDialog(wxWindow *parent, const wxString &checkbox_label, bool checkbox_value,
                                       ...
                                       const wxString &default_file, const wxString &wildcard, long style,
...
boost::optional<WindowMetrics> WindowMetrics::deserialize(const std::string &str)
{
    ...
    if ((metrics[4] & ~1) != 0)
    { // Checks if the maximized flag is 1 or 0
        metrics[4] = 0;
    }
...
{
    return (boost::format("%1%; %2%; %3%; %4%; %5%") % rect.x % rect.y % rect.width % rect.height %
