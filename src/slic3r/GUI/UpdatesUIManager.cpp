...
RepositoryUpdateUIManager::RepositoryUpdateUIManager(wxWindow *parent, Slic3r::PresetUpdaterWrapper *puw, int em)
    : m_parent(parent), m_puw(puw), m_main_sizer(new wxBoxSizer(wxVERTICAL))
{
    ...
    {
        wxHtmlWindow *offline_info = new wxHtmlWindow(m_parent, wxID_ANY, wxDefaultPosition, wxSize(60 * em, 5 * em),
                                                      wxHW_SCROLLBAR_NEVER);
        offline_info->SetBorders(0);

        offline_info->Bind(wxEVT_HTML_LINK_CLICKED,
...
void RepositoryUpdateUIManager::fill_entries(bool init_selection /* = false*/)
{
    ...
    for (const auto *archive : archs)
    {
        ...
        {
            // online repo
            m_online_entries.push_back(
                {is_selected, uuid, data.name, data.description, data.visibility, data.not_in_manifest});
        }
        else
        {
            // offline repo
            m_offline_entries.push_back({is_selected, uuid, data.name, data.description,
                                         data.source_path.filename().string(), fs::exists(data.source_path),
                                         data.source_path});
        }
    }
}
...
void RepositoryUpdateUIManager::fill_grids()
{
    ...
    {
        auto add = [this](wxWindow *win)
        {
            m_online_sizer->Add(win, 0, wxALIGN_CENTER_VERTICAL);
        };
        ...
        for (const auto &entry : m_online_entries)
        {
            ...
            if (entry.not_in_manifest)
            {
                ...
                if (wxGetApp().is_account_logged_in())
                {
                    ...
                    bmp->SetToolTip(_L(
                        "Some vendors were installed from this source, but you do not have the rights to receive updates from it.\n"
                ...
                else
                {
                    // TRN tooltip in Configuration Wizard - Configuration Sources
                    bmp->SetToolTip(_L(
                        "Some vendors were installed from this source, but you do not have rights to receive updates from it.\n"
    ...
    if (!m_offline_entries.empty())
    {
        auto add = [this](wxWindow *win)
        {
            m_offline_sizer->Add(win, 0, wxALIGN_CENTER_VERTICAL);
        };

        // header

        for (const wxString &l :
             std::initializer_list<wxString>{"", _L("Name"), _L("Description"), "", _L("Source file"), "", ""})
        {
            auto text = new wxStaticText(m_parent, wxID_ANY, l);
            text->SetFont(wxGetApp().bold_font());
            add(text);
        ...
        for (const auto &entry : m_offline_entries)
        {
            ...
            {
                wxStaticBitmap *bmp = new wxStaticBitmap(m_parent, wxID_ANY,
                                                         *get_bmp_bundle(entry.is_ok ? "tick_mark" : "exclamation"));
...
void RepositoryUpdateUIManager::load_offline_repos()
{
    ...
    wxFileDialog dialog(m_parent, _L("Choose one or more ZIP files") + ":",
                        from_u8(wxGetApp().app_config->get_last_dir()), "", file_wildcards(FT_ZIP),
...
ManagePresetRepositoriesDialog::ManagePresetRepositoriesDialog(Slic3r::PresetUpdaterWrapper *puw)
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY,
                format_wxstr("%1% - %2%", SLIC3R_APP_NAME, _L("Manage Updates")), wxDefaultPosition, wxDefaultSize,
