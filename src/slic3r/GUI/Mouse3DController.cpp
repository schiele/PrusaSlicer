    ...
    else
    {
        ...
        else
        {
                    ...
                    for (size_t i = 0; i < _3DCONNEXION_DEVICES.size(); ++i)
                    {
                        if (_3DCONNEXION_DEVICES[i] == current->product_id)
                        {
                            ...
                            if (it == detected_devices.end())
                                it = detected_devices
                                         .insert(DetectedDevices::value_type(detected_device, DeviceDataList()))
                                         .first;
            ...
            if (vendor_id != 0 && product_id != 0)
            {
                ...
                BOOST_LOG_TRIVIAL(trace) << "Detected device: " << std::hex << vendor_id << std::dec << "::" << std::hex
                                         << product_id << std::dec << " " << ret;
            }
            else
...
bool Mouse3DController::State::process_mouse_wheel()
{
    ...
    if (m_mouse_wheel_counter == 0)
        ...
        return false;
    if (std::find_if(m_input_queue.begin(), m_input_queue.end(),
                     [](const QueueItem &item) { return item.is_rotation(); }) != m_input_queue.end())
    {
...
bool Mouse3DController::State::apply(const Mouse3DController::Params &params, Camera &camera)
{
    ...
    for (const QueueItem &input_queue_item : input_queue)
    {
        ...
        {
            Vec3d translation = params.swap_yz ? Vec3d(input_queue_item.vector.x(), -input_queue_item.vector.z(),
                                                       input_queue_item.vector.y())
                                               : input_queue_item.vector;
            double zoom_factor = camera.min_zoom() / camera.get_zoom();
            camera.set_target(camera.get_target() +
                              zoom_factor * params.translation.scale *
...
void Mouse3DController::load_config(const AppConfig &appconfig)
{
    ...
    for (const std::string &device_name : appconfig.get_mouse_device_names())
    {
        ...
        Params params;
        params.translation.scale = Params::DefaultTranslationScale * std::clamp(translation_speed,
                                                                                Params::MinTranslationScale,
                                                                                Params::MaxTranslationScale);
...
void Mouse3DController::save_config(AppConfig &appconfig) const
{
    ...
    for (const auto &key_value_pair : m_params_by_device)
    {
        ...
        appconfig.set_mouse_device(device_name, params.translation.scale / Params::DefaultTranslationScale,
                                   params.translation.deadzone, params.rotation.scale / Params::DefaultRotationScale,
                                   params.rotation.deadzone, params.zoom.scale / Params::DefaultZoomScale,
                                   params.swap_yz);
    }
}
...
void Mouse3DController::render_settings_dialog(GLCanvas3D &canvas) const
{
    ...
    ImGuiPureWrap::set_next_window_pos(0.5f * (float) cnv_size.get_width(), 0.5f * (float) cnv_size.get_height(),
                                       ImGuiCond_Always, 0.5f, 0.5f);

    static ImVec2 last_win_size(0.0f, 0.0f);
    bool shown = true;
    if (ImGuiPureWrap::begin(_u8L("3Dconnexion settings"), &shown,
                                 ...
                                 ImGuiWindowFlags_NoCollapse))
    {
        if (shown)
        {
            ...
            if (imgui.slider_float(_L("Translation"), &translation_scale, float(Params::MinTranslationScale),
            ...
            if (imgui.slider_float(_L("Translation") + "/" + _L("Zoom"), &translation_deadzone, 0.0f,
            ...
            if (imgui.slider_float(_L("Rotation") + "##2", &rotation_deadzone, 0.0f, Params::MaxRotationDeadzone,
                                   "%.2f"))
            {
            ...
            int input_queue_size_current[2] = {int(m_state.input_queue_size_current()),
                                               int(m_state.input_queue_max_size_achieved)};
    ...
    if (res != 0)
    {
        ...
        if (res == -1)
            ...
            BOOST_LOG_TRIVIAL(error)
                << "Unable to initialize hidapi library: failed to load libudev.so.1 or libudev.so.0";
        else if (res == -2)
...
bool Mouse3DController::connect_device()
{
    ...
    while (cur != nullptr)
    {
        ...
        std::cout << "\" code: " << cur->vendor_id << "/" << cur->product_id << " (" << std::hex << cur->vendor_id
                  << "/" << cur->product_id << std::dec << ")";
        std::cout << " serial number: '";
        std::wcout << ((cur->serial_number != nullptr) ? cur->serial_number : L"Unknown");
        std::cout << "' usage page: " << cur->usage_page << " usage: " << cur->usage
                  << " interface number: " << cur->interface_number << std::endl;
        ...
        if (vendor_id != 0)
        {
            for (size_t i = 0; i < _3DCONNEXION_DEVICES.size(); ++i)
            {
                if (_3DCONNEXION_DEVICES[i] == current->product_id)
                {
                    ...
                    DetectedDevices::iterator it = detected_devices.find(detected_device);
                    if (it == detected_devices.end())
                        it = detected_devices.insert(DetectedDevices::value_type(detected_device, DeviceDataList()))
                                 .first;
...
#if ENABLE_3DCONNEXION_DEVICES_DEBUG_OUTPUT
                    std::wcout << "\""
                               << ((current->manufacturer_string != nullptr) ? current->manufacturer_string
                                                                             : L"Unknown");
                    std::cout << "/";
                    std::wcout << ((current->product_string != nullptr) ? current->product_string : L"Unknown");
                    std::cout << "\" code: " << current->vendor_id << "/" << current->product_id << " (" << std::hex
                              << current->vendor_id << "/" << current->product_id << std::dec << ")";
                    std::cout << " serial number: '";
    ...
    for (const DetectedDevices::value_type &device : detected_devices)
    {
        ...
        else
        {
            ...
            {
#if ENABLE_3DCONNEXION_DEVICES_DEBUG_OUTPUT
                std::cout << "Test device: " << std::hex << device.first.first << std::dec << "/" << std::hex
                          << device.first.second << std::dec << " \"" << data.path << "\"";
#endif // ENABLE_3DCONNEXION_DEVICES_DEBUG_OUTPUT
...
#if ENABLE_3DCONNEXION_DEVICES_DEBUG_OUTPUT
            std::cout << std::endl
                      << "Opening device: " << std::hex << vendor_id << std::dec << "/" << std::hex << product_id
                      << std::dec << " using hid_open()" << std::endl;
#endif // ENABLE_3DCONNEXION_DEVICES_DEBUG_OUTPUT
    ...
    else
    {
        ...
        std::cout << std::endl
                  << "Opening device: " << std::hex << vendor_id << std::dec << "/" << std::hex << product_id
                  << std::dec << "\"" << path << "\" using hid_open_path()" << std::endl;
#endif // ENABLE_3DCONNEXION_DEVICES_DEBUG_OUTPUT
    ...
    else
    {
        ...
        std::cout << "Manufacturer id.....: " << vendor_id << " (" << std::hex << vendor_id << std::dec << ")"
                  << std::endl;
        std::cout << "Product id..........: " << product_id << " (" << std::hex << product_id << std::dec << ")"
                  << std::endl;
...
bool Mouse3DController::handle_input(const DataPacketRaw &packet, const int packet_length, const Params &params,
                                     State &state_in_out)
{
    if (!wxGetApp().IsActive())
        return false;
...
bool Mouse3DController::handle_packet(const DataPacketRaw &packet, const int packet_length, const Params &params,
                                      State &state_in_out)
{
    switch (packet[0])
    {
...
bool Mouse3DController::handle_packet_translation(const DataPacketRaw &packet, const Params &params,
                                                  State &state_in_out)
{
...
bool Mouse3DController::handle_packet_rotation(const DataPacketRaw &packet, unsigned int first_byte,
                                               const Params &params, State &state_in_out)
{
