...
static Polygon get_wtpoly(const GLCanvas3D::WipeTowerInfo &wti)
{
    ...
    Polygon poly = Polygon({{bb.min}, {bb.max.x(), bb.min.y()}, {bb.max}, {bb.min.x(), bb.max.y()}});
...
public:
    explicit ArrangeableWT(const ObjectID &oid, const GLCanvas3D::WipeTowerInfo &wti, std::function<bool(int)> sel_pred,
...
struct WTH : public arr2::WipeTowerHandler
{
    ...
    WTH(
        const ObjectID &objid, const GLCanvas3D::WipeTowerInfo &w,
    ...
    void set_selection_predicate(std::function<bool(int)> pred) override { sel_pred = std::move(pred); }

    ObjectID get_id() const override { return this->oid; }
};
