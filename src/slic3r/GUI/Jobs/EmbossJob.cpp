...
SurfaceVolumeData::ModelSources create_sources(const ModelVolumePtrs &volumes,
...
void CreateVolumeJob::finalize(bool canceled, std::exception_ptr &eptr)
{
    ...
    create_volume(std::move(m_result), m_input.object_id, m_input.volume_type, m_input.trmat, *m_input.base,
                  m_input.gizmo);
}
...
void CreateSurfaceVolumeJob::finalize(bool canceled, std::exception_ptr &eptr)
{
    if (!::finalize(canceled, eptr, *m_input.base))
        return;
    create_volume(std::move(m_result), m_input.object_id, m_input.volume_type, m_input.transform, *m_input.base,
                  m_input.gizmo);
}
...
bool start_create_volume_job(Worker &worker, const ModelObject &object, const std::optional<Transform3d> &volume_tr,
                             DataBasePtr data, ModelVolumeType volume_type, GLGizmosManager::EType gizmo);
...
const GLVolume *find_closest(const Selection &selection, const Vec2d &screen_center, const Camera &camera,
                             const ModelObjectPtrs &objects, Vec2d *closest_center);
...
bool start_update_volume(DataUpdate &&data, const ModelVolume &volume, const Selection &selection,
...
TriangleMesh create_mesh_per_glyph(DataBase &input, Fnc was_canceled)
{
    ...
    static_assert(std::is_same<Point::coord_type, int64_t>());
    ...
    for (size_t text_line_index = 0; text_line_index < input.text_lines.size(); ++text_line_index)
    {
        ...
        for (size_t i = 0; i < line_bbs.size(); ++i)
        {
            ...
            float surface_offset = input.is_outside ? -SAFE_SURFACE_OFFSET
...
indexed_triangle_set cut_surface_to_its(const ExPolygons &shapes, const Transform3d &tr,
                                        const SurfaceVolumeData::ModelSources &sources, DataBase &input,
                                        std::function<bool()> was_canceled)
{
    ...
    OrthoProject3d projection = create_emboss_projection(input.is_outside, input.shape.projection.depth, emboss_tr,
                                                         cut);
                                   ...
                                   std::function<bool()> was_canceled)
{
    ...
    for (size_t text_line_index = 0; text_line_index < input1.text_lines.size(); ++text_line_index)
    {
        ...
        for (size_t i = 0; i < line_bbs.size(); ++i)
        {
            ...
            Point offset(-glyph_bb.center().x(), coord_t(0));
...
bool start_create_volume_on_surface_job(CreateVolumeParams &input, const Vec2d &screen_coor, bool try_no_coor)
{
    auto on_bad_state = [&input, try_no_coor](const ModelObject *object = nullptr)
    {
        ...
        else
        {
            ...
            auto gizmo_type = static_cast<GLGizmosManager::EType>(input.gizmo);
            return start_create_volume_job(input.worker, *object, {}, std::move(input.data), input.volume_type,
                                           gizmo_type);
        }
    };
    ...
    return start_create_volume_job(input.worker, *object, transform, std::move(input.data), input.volume_type,
                                   gizmo_type);
}
