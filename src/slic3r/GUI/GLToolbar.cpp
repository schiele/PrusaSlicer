...
const GLToolbarItem::VisibilityCallback GLToolbarItem::Default_Visibility_Callback = []() -> bool
{
    return true;
};
const GLToolbarItem::EnablingCallback GLToolbarItem::Default_Enabling_Callback = []() -> bool
{
    return true;
};
...
GLToolbarItem::Data::Option::Option()
    : toggable(false), action_callback(Default_Action_Callback), render_callback(nullptr)
{
}
...
void GLToolbarItem::render(const GLCanvas3D &parent, unsigned int tex_id, float left, float right, float bottom,
                           float top, unsigned int tex_width, unsigned int tex_height, unsigned int icon_size) const
{
    auto uvs = [this](unsigned int tex_width, unsigned int tex_height, unsigned int icon_size) -> GLTexture::Quad_UVs
    {
        ...
        const float icon_size_px = (float) (tex_width - 1) /
                                   ((float) Num_States + (float) Num_Rendered_Highlight_States);
        const char render_state = m_highlight_state == NotHighlighted
                                      ? (m_state == Pressed ? Normal : // use regular icon, without black background
                                             m_state == HoverPressed ? Hover
                                                                     : m_state)
                                      : // use regular icon, without black background
...
{
    if (m_layout.dirty)
        const_cast<GLToolbar *>(this)->calc_layout();
...
std::pair<float, float> GLToolbar::get_item_center_position(const std::string &name, const GLCanvas3D &parent) const
{
    // Find the item index
    int target_idx = -1;
    for (int i = 0; i < (int) m_items.size(); ++i)
    ...
    if (target_idx == -1)
        return {0.0f, 0.0f}; // Item not found
    ...
    // Calculate position by iterating through items
    for (int i = 0; i < (int) m_items.size(); ++i)
    {
        const GLToolbarItem *item = m_items[i];
        ...
        if (i == target_idx)
        {
            // Found the target item - calculate its center in normalized coords
            float norm_center_x = left + icons_size_x * 0.5f;
            float norm_center_y = top - icons_size_y * 0.5f;
            // Convert from normalized coordinates [-1, 1] to screen coordinates [0, cnv_w/h]
            float center_x = (norm_center_x + 1.0f) * 0.5f * cnv_w;
            float center_y = (1.0f - norm_center_y) * 0.5f * cnv_h;
    ...
    return {0.0f, 0.0f}; // Should not reach here
}
...
bool GLToolbar::is_any_item_visible() const
{
...
void GLToolbar::do_action(GLToolbarItem::EActionType type, int item_id, GLCanvas3D &parent, bool check_hover)
{
    if (m_pressed_toggable_id == -1 || m_pressed_toggable_id == item_id)
    {
        if (0 <= item_id && item_id < (int) m_items.size())
        {
            ...
            if (item != nullptr && !item->is_separator() && !item->is_disabled() &&
                (!check_hover || item->is_hovered()))
            {
...
void GLToolbar::render_background(float left, float top, float right, float bottom, float border_w,
...
void GLToolbar::render_horizontal(const GLCanvas3D &parent)
{
    ...
    for (const GLToolbarItem *item : m_items)
    {
        ...
        {
            if (item->is_pressed())
                render_background(left - (id == m_items.size() ? border_w : margin_w), top + margin_h,
                                  left + icons_size_x + (id == 1 ? border_w : margin_w), top - icons_size_y - margin_h,
                                  border_w, border_h);

            item->render(parent, tex_id, left, left + icons_size_x, top - icons_size_y, top, (unsigned int) tex_width,
                         (unsigned int) tex_height, (unsigned int) (m_layout.icons_size));
            left += icon_stride;
        }
    }
}
