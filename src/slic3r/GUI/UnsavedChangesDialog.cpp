...
static const std::map<Preset::Type, std::string> type_icon_names = {
    {Preset::TYPE_PRINT, "cog"},          {Preset::TYPE_SLA_PRINT, "cog"},   {Preset::TYPE_FILAMENT, "spool"},
    {Preset::TYPE_SLA_MATERIAL, "resin"}, {Preset::TYPE_PRINTER, "printer"},
};
...
ModelNode::ModelNode(ModelNode *parent, const wxString &text, const std::string &icon_name)
    : m_parent_win(parent->m_parent_win), m_parent(parent), m_icon_name(icon_name), m_text(text)
{
    UpdateIcons();
}
...
ModelNode::ModelNode(ModelNode *parent, const wxString &text)
    : m_parent_win(parent->m_parent_win), m_parent(parent), m_icon_name("dot_small"), m_text(text)
{
    UpdateIcons();
}
...
// option node
ModelNode::ModelNode(ModelNode *parent, const wxString &text, const wxString &old_value, const wxString &mod_value,
...
wxDataViewItem DiffModel::AddPreset(Preset::Type type, wxString preset_name, PrinterTechnology pt,
                                    wxString new_preset_name /* = wxString()*/)
{
    // "color" strings
...
ModelNode *DiffModel::AddOption(ModelNode *group_node, wxString option_name, wxString old_value, wxString mod_value,
                                wxString new_value)
{
...
ModelNode *DiffModel::AddOptionWithGroup(ModelNode *category_node, wxString group_name, wxString option_name,
                                         wxString old_value, wxString mod_value, wxString new_value)
{
...
ModelNode *DiffModel::AddOptionWithGroupAndCategory(ModelNode *preset_node, wxString category_name, wxString group_name,
                                                    wxString option_name, wxString old_value, wxString mod_value,
                                                    wxString new_value, const std::string category_icon_name)
{
...
wxDataViewItem DiffModel::AddOption(Preset::Type type, wxString category_name, wxString group_name,
                                    wxString option_name, wxString old_value, wxString mod_value, wxString new_value,
                    ...
                    for (std::unique_ptr<ModelNode> &group : category->GetChildren())
                        if (group->text() == group_name)
                            return wxDataViewItem(
                                (void *) AddOption(group.get(), option_name, old_value, mod_value, new_value));

                    return wxDataViewItem((void *) AddOptionWithGroup(category.get(), group_name, option_name,
                                                                      old_value, mod_value, new_value));
                }

            return wxDataViewItem((void *) AddOptionWithGroupAndCategory(preset.get(), category_name, group_name,
                                                                         option_name, old_value, mod_value, new_value,
                                                                         category_icon_name));
        }
...
void DiffViewCtrl::AppendBmpTextColumn(const wxString &label, unsigned model_column, int width,
                                       bool set_expander /* = false*/)
{
...
#endif
    wxDataViewColumn *column = new wxDataViewColumn(label, rd, model_column, width * m_em_unit, wxALIGN_TOP,
                                                    wxDATAVIEW_COL_RESIZABLE | wxDATAVIEW_CELL_INERT);
#else
    wxDataViewColumn *column = new wxDataViewColumn(label, new BitmapTextRenderer(true, wxDATAVIEW_CELL_INERT),
                                                    model_column, width * m_em_unit, wxALIGN_TOP,
...
void DiffViewCtrl::Append(const std::string &opt_key, Preset::Type type, wxString category_name, wxString group_name,
                          wxString option_name, wxString old_value, wxString mod_value, wxString new_value,
                          const std::string category_icon_name)
{
    ...
    m_items_map.emplace(model->AddOption(type, category_name, group_name, option_name, old_val, mod_val, new_val,
                                         category_icon_name),
                        item_data);
}
...
void DiffViewCtrl::context_menu(wxDataViewEvent &event)
{
    ...
    FullCompareDialog(it->second.opt_name, it->second.old_val, it->second.mod_val, it->second.new_val, old_value_header,
                      mod_value_header, new_value_header)
        .ShowModal();
...
UnsavedChangesDialog::UnsavedChangesDialog(const wxString &caption, const wxString &header,
    ...
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, caption + ": " + _L("Unsaved Changes"),
...
UnsavedChangesDialog::UnsavedChangesDialog(Preset::Type type, PresetCollection *dependent_presets,
                                           const std::string &new_selected_preset)
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, _L("Switching Presets: Unsaved Changes"),
                wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    ...
    else
    {
        ...
        if (m_exit_action == Action::Save ||
            (m_exit_action == Action::Transfer && dependent_presets &&
             (type == dependent_presets->type()
                  ...
                  : printers.get_edited_preset().printer_technology() !=
                        printers.find_preset(new_selected_preset)->printer_technology())))
            save(dependent_presets);
    }
    wxGetApp().UpdateDlgDarkUI(this);
}

void UnsavedChangesDialog::build(Preset::Type type, PresetCollection *dependent_presets,
                                 const std::string &new_selected_preset, const wxString &header)
{
                                                                    ...
                                                                    const wxString &label, bool process_enable = true)
    {
        *btn = new ScalableButton(this, btn_id = NewControlId(), icon_name, label, wxDefaultSize, wxDefaultPosition,
        ...
        (*btn)->Bind(wxEVT_BUTTON,
                     [this, close_act, dependent_presets](wxEvent &)
                     {
                         update_config(close_act);
                         bool save_names_and_types = close_act == Action::Save ||
                                                     (close_act == Action::Transfer && ActionButtons::KEEP & m_buttons);
    ...
    {
        const PresetCollection *switched_presets = type == Preset::TYPE_INVALID
                                                       ? nullptr
                                                       : wxGetApp().get_tab(type)->get_presets();
        if (dependent_presets && switched_presets &&
            ...
            add_btn(&m_transfer_btn, m_move_btn_id, "paste_menu", Action::Transfer,
                    switched_presets->get_edited_preset().name == new_selected_preset ? _L("Keep") : _L("Transfer"));
    }
    ...
    { // "Don't save" / "Discard" button
        std::string btn_icon = (ActionButtons::DONT_SAVE & m_buttons)                     ? ""
                               : (dependent_presets || (ActionButtons::KEEP & m_buttons)) ? "switch_presets"
                                                                                          : "exit";
    ...
    if (!m_app_config_key.empty())
    {
        ...
        m_remember_choice->Bind(
            wxEVT_CHECKBOX,
            [this](wxCommandEvent &evt)
            {
                if (!evt.IsChecked())
                    return;
                wxString preferences_item =
                    m_app_config_key == "default_action_on_new_project"
                        ? _L("Ask for unsaved changes in presets when creating new project")
                    : m_app_config_key == "default_action_on_select_preset"
                        ? _L("Ask for unsaved changes in presets when selecting new preset")
                        : _L("Ask to save unsaved changes in presets when closing the application or when loading a new project");
                wxString action =
                    m_app_config_key == "default_action_on_new_project"
                        ? _L("You will not be asked about the unsaved changes in presets the next time you create new project")
                    : m_app_config_key == "default_action_on_select_preset"
                        ? _L("You will not be asked about the unsaved changes in presets the next time you switch a preset")
                ...
                wxString msg =
                    _L("preFlight will remember your action.") + "\n\n" + action + "\n\n" +
                    format_wxstr(_L("Visit \"Preferences\" and check \"%1%\"\nto be asked about unsaved changes again."),
                                 preferences_item);

                MessageDialog dialog(nullptr, msg, _L("preFlight: Don't ask me again"),
...
void UnsavedChangesDialog::show_info_line(Action action, std::string preset_name)
{
    ...
    else
    {
        ...
        if (action == Action::Undef)
            ...
            text = ActionButtons::DONT_SAVE & m_buttons ? _L("All settings changes will not be saved")
        ...
        else
        {
            if (preset_name.empty())
                ...
                text = format_wxstr(action == Action::Save
...
bool UnsavedChangesDialog::save(PresetCollection *dependent_presets, bool show_save_preset_dialog /* = true*/)
{
    ...
    else
    {
        ...
        PrinterTechnology printer_technology =
            wxGetApp().preset_bundle->printers.get_edited_preset().printer_technology();
            ...
            if (opt_key == "gcode_substitutions")
            {
                if (!strings->empty())
                    for (size_t id = 0; id < strings->size(); id += 4)
                        out += from_u8(strings->get_at(id)) + ";\t" + from_u8(strings->get_at(id + 1)) + ";\t" +
...
void UnsavedChangesDialog::update(Preset::Type type, PresetCollection *dependent_presets,
                                  const std::string &new_selected_preset, const wxString &header)
{
    ...
    if (m_save_btn)
        m_save_btn->Bind(wxEVT_ENTER_WINDOW,
                         [this, presets](wxMouseEvent &e)
                         {
    ...
    if (m_transfer_btn)
    {
        ...
        m_transfer_btn->Bind(wxEVT_ENTER_WINDOW,
                             [this, new_selected_preset, is_empty_name](wxMouseEvent &e)
                             {
    ...
    if (m_discard_btn)
        m_discard_btn->Bind(wxEVT_ENTER_WINDOW,
                            [this](wxMouseEvent &e)
                            {
    ...
    else
    {
        ...
        {
            action_msg = format_wxstr(_L("Preset \"%1%\" has the following unsaved changes:"),
                                      presets->get_edited_preset().name);
        }
        else
        {
            action_msg = format_wxstr(
                                       ...
                                       const std::string &new_selected_preset)
{
    ...
    for (PresetCollection *presets : presets_list)
    {
        ...
        const DynamicPrintConfig &new_config = m_tree->has_new_value_column()
                                                   ? presets->find_preset(new_selected_preset, false, false)->config
                                                   : mod_config;
        type = presets->type();
        ...
        if (type == Preset::TYPE_PRINTER && old_pt == ptFFF &&
            old_config.opt<ConfigOptionStrings>("extruder_colour")->values.size() !=
                mod_config.opt<ConfigOptionStrings>("extruder_colour")->values.size())
        {
            wxString local_label = _L("Extruders count");
            wxString old_val = from_u8(
                (boost::format("%1%") % old_config.opt<ConfigOptionStrings>("extruder_colour")->values.size()).str());
            wxString mod_val = from_u8(
                (boost::format("%1%") % mod_config.opt<ConfigOptionStrings>("extruder_colour")->values.size()).str());
            wxString new_val = !m_tree->has_new_value_column()
                                   ? ""
                                   : from_u8((boost::format("%1%") %
                                              new_config.opt<ConfigOptionStrings>("extruder_colour")->values.size())
                                                 .str());

            m_tree->Append("extruders_count", type, _L("General"), _L("Capabilities"), local_label, old_val, mod_val,
                           new_val, category_icon_map.at("General"));
        }
        ...
        for (const std::string &opt_key : dirty_options)
        {
            ...
            m_tree->Append(opt_key, type, option.category_local, option.group_local, option.label_local,
                           ...
                           category_icon_map.at(option.category));
        }
...
FullCompareDialog::FullCompareDialog(const wxString &option_name, const wxString &old_value, const wxString &mod_value,
                                     const wxString &new_value, const wxString &old_value_header,
                                     const wxString &mod_value_header, const wxString &new_value_header)
    : wxDialog(nullptr, wxID_ANY, option_name, wxDefaultPosition, wxDefaultSize,
               wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    ...
    std::set_difference(old_set.begin(), old_set.end(), mod_set.begin(), mod_set.end(),
                        std::inserter(old_mod_diff_set, old_mod_diff_set.begin()));
    std::set_difference(mod_set.begin(), mod_set.end(), old_set.begin(), old_set.end(),
...
void DiffPresetDialog::create_presets_sizer()
{
    ...
    for (auto new_type : {Preset::TYPE_PRINT, Preset::TYPE_SLA_PRINT, Preset::TYPE_FILAMENT, Preset::TYPE_SLA_MATERIAL,
                          Preset::TYPE_PRINTER})
    {
        ...
        auto add_preset_combobox =
            [collection, sizer, new_type, this](PresetComboBox **cb_, PresetBundle *preset_bundle)
        {
            *cb_ = new PresetComboBox(this, new_type, wxSize(em_unit() * 35, -1), preset_bundle);
            ...
            cb->set_selection_changed_function(
                [this, new_type, preset_bundle, cb](int selection)
                {
        ...
        equal_bmp->Bind(wxEVT_BUTTON,
                        [presets_left, presets_right, this](wxEvent &)
                        {
...
void DiffPresetDialog::create_show_all_presets_chb()
{
    ...
    m_show_all_presets->Bind(wxEVT_CHECKBOX,
                             [this](wxCommandEvent &)
                             {
...
void DiffPresetDialog::create_buttons()
{
    ...
    // Transfer
    m_transfer_btn = new ScalableButton(this, wxID_ANY, "paste_menu", _L("Transfer"), wxDefaultSize, wxDefaultPosition,
                                        wxBORDER_DEFAULT, 24);
    ...
    m_transfer_btn->Bind(wxEVT_UPDATE_UI,
                         [this, enable_transfer](wxUpdateUIEvent &evt)
                         {
    ...
    m_transfer_btn->Bind(
        wxEVT_ENTER_WINDOW,
        [show_in_bottom_info](wxMouseEvent &e)
        {
            show_in_bottom_info(
                _L("Transfer the selected options from left preset to the right.\n"
                   "Note: New modified presets will be selected in settings tabs after close this dialog."),
                e);
        });

    // Save
    m_save_btn = new ScalableButton(this, wxID_ANY, "save", _L("Save"), wxDefaultSize, wxDefaultPosition,
                                    wxBORDER_DEFAULT, 24);
    ...
    // Cancel
    m_cancel_btn = new ScalableButton(this, wxID_CANCEL, "cross", _L("Cancel"), wxDefaultSize, wxDefaultPosition,
                                      wxBORDER_DEFAULT, 24);
...
void DiffPresetDialog::create_edit_sizer()
{
    ...
    m_use_for_transfer->SetToolTip(_L(
        "If checked, this dialog can be used for transferring selected values from the preset on the left to the preset on the right."));
    m_use_for_transfer->Bind(wxEVT_CHECKBOX,
                             [this](wxCommandEvent &)
                             {
...
DiffPresetDialog::DiffPresetDialog(MainFrame *mainframe)
    : DPIDialog(mainframe, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
                wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER, "diff_presets_dialog",
    ...
    for (auto preset_combos : m_preset_combos)
    {
        ...
        bool show = type != Preset::TYPE_INVALID      ? type == cb_type
                    ...
                    : m_pr_technology == ptFFF
                        ? cb_type == Preset::TYPE_PRINT || cb_type == Preset::TYPE_FILAMENT
        ...
        if (show)
        {
            if (cb_type == Preset::TYPE_FILAMENT)
            {
                ...
                if (type == Preset::TYPE_FILAMENT)
                    extruder_id =
                        dynamic_cast<TabFilament *>(wxGetApp().get_tab(Preset::TYPE_FILAMENT))->get_active_extruder();
...
void DiffPresetDialog::update_tree()
{
    ...
    for (auto preset_combos : m_preset_combos)
    {
        ...
        const Preset *left_preset = presets->find_preset(preset_name, false,
                                                         preset_name_full.length() != preset_name.length());
        preset_name_full = get_selection(preset_combos.presets_right);
        preset_name = Preset::remove_suffix_modified(preset_name_full);
        const Preset *right_preset = presets->find_preset(preset_name, false,
                                                          preset_name_full.length() != preset_name.length());
        ...
        preset_combos.equal_bmp->SetToolTip(
            _L("Presets are different.\n"
               "Click this button to select the same preset for the right and left preset."));

        m_tree->model->AddPreset(type,
                                 "\"" + from_u8(left_preset->name) + "\" vs \"" + from_u8(right_preset->name) + "\"",
                                 left_pt);
        ...
        {
            wxString local_label = _L("Extruders count");
            wxString left_val = from_u8(
                (boost::format("%1%") % left_config.opt<ConfigOptionStrings>("extruder_colour")->values.size()).str());
            wxString right_val = from_u8(
                (boost::format("%1%") % right_congig.opt<ConfigOptionStrings>("extruder_colour")->values.size()).str());

            m_tree->Append("extruders_count", type, _L("General"), _L("Capabilities"), local_label, left_val, right_val,
                           "", category_icon_map.at("General"));
        }
        ...
        for (const std::string &opt_key : dirty_options)
        {
            ...
            if (option.opt_key() != opt_key)
            {
                ...
                m_tree->Append(opt_key, type, _L("Undef category"), _L("Undef group"), opt_key, left_val, right_val, "",
                               "question");
#endif
                // When founded option isn't the correct one.
                // It can be for dirty_options: "default_print_profile", "printer_model", "printer_settings_id",
            ...
            m_tree->Append(opt_key, type, option.category_local, option.group_local, option.label_local, left_val,
                           right_val, "", category_icon_map.at(option.category));
        }
    }
...
void DiffPresetDialog::update_compatibility(const std::string &preset_name, Preset::Type type,
                                            PresetBundle *preset_bundle)
{
    ...
    if (print_tab || printer_tab)
        preset_bundle->update_compatible(update_compatible_type(technology_changed, print_tab, true),
        ...
        if ((print_tab && ((pr_tech == ptFFF && presets_type == Preset::TYPE_FILAMENT) ||
            ...
            (printer_tab &&
             ((pr_tech == ptFFF && (presets_type == Preset::TYPE_PRINT || presets_type == Preset::TYPE_FILAMENT)) ||
              (pr_tech == ptSLA &&
               (presets_type == Preset::TYPE_SLA_PRINT || presets_type == Preset::TYPE_SLA_MATERIAL)))))
            cb->update();
    }

    if (technology_changed && m_preset_bundle_left.printers.get_selected_preset().printer_technology() ==
...
bool DiffPresetDialog::is_save_confirmed()
{
    ...
    for (const Preset::Type &type : types_list())
    {
        if (!m_tree->options(type, true).empty())
        {
            ...
            presets_to_save.emplace_back(PresetToSave{type, get_left_preset_name(type), get_right_preset_name(type),
...
void DiffPresetDialog::button_event(Action act)
{
    if (act == Action::Save)
    {
        if (is_save_confirmed())
        {
            ...
            for (const auto &preset : presets_to_save)
                if (wxGetApp().preset_bundle->transfer_and_save(preset.type, preset.from_name, preset.to_name,
                                                                preset.new_name, get_options_to_save(preset.type)))
                    saved_cnt++;
    ...
    else
    {
        ...
        if (act == Action::Transfer)
            process_options(
                [this](Preset::Type type)
                {
                    if (Tab *tab = wxGetApp().get_tab(type))
                        tab->transfer_options(get_left_preset_name(type), get_right_preset_name(type),
