...
static inline void throw_on_missing_variable(const std::string &opt_key, const char *dependent_opt_key)
{
    throw FlowErrorMissingVariable(
        (boost::format(_u8L("Cannot calculate extrusion width for %1%: Variable \"%2%\" not accessible.")) % opt_key %
         dependent_opt_key)
            .str());
}

// Used to provide hints to the user on default extrusion width values, and to provide reasonable values to the PlaceholderParser.
double Flow::extrusion_width(const std::string &opt_key, const ConfigOptionFloatOrPercent *opt,
                             const ConfigOptionResolver &config, const unsigned int first_printing_extruder)
{
    assert(opt != nullptr);
...
// This is the logic used for skit / brim, but not for the rest of the 1st layer.
    ...
    float nozzle_diameter = float(opt_nozzle_diameters->get_at(first_printing_extruder));

    if (opt->percent)
    {
        // Check whether percentages are relative to nozzle diameter or layer height.
        auto opt_pct_of_nozzle = config.option<ConfigOptionBool>("extrusion_width_percent_of_nozzle");
        if (opt_pct_of_nozzle && opt_pct_of_nozzle->value)
...
// Used to provide hints to the user on default extrusion width values, and to provide reasonable values to the PlaceholderParser.
double Flow::extrusion_width(const std::string &opt_key, const ConfigOptionResolver &config,
                             const unsigned int first_printing_extruder)
{
    return extrusion_width(opt_key, config.option<ConfigOptionFloatOrPercent>(opt_key), config,
                           first_printing_extruder);
}

// This constructor builds a Flow object from an extrusion width config setting
// and other context properties.
Flow Flow::new_from_config_width(FlowRole role, const ConfigOptionFloatOrPercent &width, float nozzle_diameter,
                                 float height, bool percent_of_nozzle)
{
    if (height <= 0)
    ...
    {
        // Percentages are relative to nozzle diameter or layer height depending on user setting.
        w = float(width.get_abs_value(width.percent && percent_of_nozzle ? nozzle_diameter : height));
    }
...
Flow Flow::with_cross_section(float area_new) const
{
    ...
    if (area_new > area + EPSILON)
    {
        ...
        if (new_full_spacing > m_spacing)
        {
            ...
            return Flow(rounded_rectangle_extrusion_width_from_spacing(m_spacing, height), height, m_spacing,
                        m_nozzle_diameter, false);
        }
        else
        {
...
Flow support_material_flow(const PrintObject *object, float layer_height)
{
    ...
    const float nozzle_diameter = extruder >= 0 ? static_cast<float>(print_config.nozzle_diameter.get_at(extruder))
                                                : min_nozzle_diameter(*object);

    return Flow::new_from_config_width(
        frSupportMaterial,
        // The width parameter accepted by new_from_config_width is of type ConfigOptionFloatOrPercent, the Flow class takes care of the percent to value substitution.
        (object->config().support_material_extrusion_width.value > 0)
            ? object->config().support_material_extrusion_width
            : object->config().extrusion_width,
        nozzle_diameter, (layer_height > 0.f) ? layer_height : float(object->config().layer_height.value),
        object->config().extrusion_width_percent_of_nozzle.value);
}
...
Flow support_material_1st_layer_flow(const PrintObject *object, float layer_height)
{
    ...
    const float nozzle_diameter = extruder >= 0 ? static_cast<float>(print_config.nozzle_diameter.get_at(extruder))
                                                : min_nozzle_diameter(*object);
    const auto &width = (print_config.first_layer_extrusion_width.value > 0)
    ...
    return Flow::new_from_config_width(
        frSupportMaterial,
        // The width parameter accepted by new_from_config_width is of type ConfigOptionFloatOrPercent, the Flow class takes care of the percent to value substitution.
        (width.value > 0) ? width : object->config().extrusion_width, nozzle_diameter,
        (layer_height > 0.f)
            ? layer_height
            : float(print_config.first_layer_height.get_abs_value(object->config().layer_height.value)),
        object->config().extrusion_width_percent_of_nozzle.value);
}
...
Flow support_material_interface_flow(const PrintObject *object, float layer_height)
{
    ...
    if (object->config().support_material_interface_extrusion_width.value > 0)
        width = object->config().support_material_interface_extrusion_width;
    else if (object->config().support_material_extrusion_width.value > 0)
        width = object->config().support_material_extrusion_width;
    else
        width = object->config().extrusion_width;

    return Flow::new_from_config_width(
        frSupportMaterialInterface,
        // The width parameter accepted by new_from_config_width is of type ConfigOptionFloatOrPercent, the Flow class takes care of the percent to value substitution.
        width, nozzle_diameter, (layer_height > 0.f) ? layer_height : float(object->config().layer_height.value),
        object->config().extrusion_width_percent_of_nozzle.value);
}
