...
TriangleMesh::TriangleMesh(std::vector<Vec3f> &&vertices, const std::vector<Vec3i> &&faces)
...
TriangleMesh::TriangleMesh(indexed_triangle_set &&its, const RepairedMeshErrors &errors /* = RepairedMeshErrors()*/)
    : its(std::move(its))
{
...
bool TriangleMesh::ReadSTLFile(const char *input_file, bool repair)
{
    ...
    m_stats.open_edges = stl.stats.backwards_edges + facets_w_1_bad_edge + facets_w_2_bad_edge * 2 +
                         facets_w_3_bad_edge * 3;

    m_stats.repaired_errors = {stl.stats.edges_fixed, stl.stats.degenerate_facets, stl.stats.facets_removed,
                               stl.stats.facets_reversed, stl.stats.backwards_edges};
...
struct EdgeToFace
{
    ...
    {
        return vertex_low == other.vertex_low && vertex_high == other.vertex_high;
    }
    bool operator<(const EdgeToFace &other) const
    {
...
static std::vector<EdgeToFace> create_edge_map(const typename IndexedTriangleSetType<mesh_info>::type &its,
                                               FaceFilter face_filter, ThrowOnCancelCallback throw_on_cancel)
{
...
static inline std::vector<Vec3i> its_face_edge_ids_impl(const typename IndexedTriangleSetType<mesh_info>::type &its,
                                                        FaceFilter face_filter, ThrowOnCancelCallback throw_on_cancel)
{
    std::vector<Vec3i> out(its.indices.size(), Vec3i(coord_t(-1), coord_t(-1), coord_t(-1)));
...
template std::vector<Vec3i> its_face_edge_ids<AdditionalMeshInfo::None>(
    const IndexedTriangleSetType<AdditionalMeshInfo::None>::type &);
template std::vector<Vec3i> its_face_edge_ids<AdditionalMeshInfo::Color>(
    const IndexedTriangleSetType<AdditionalMeshInfo::Color>::type &);
template std::vector<Vec3i> its_face_edge_ids<AdditionalMeshInfo::None>(
    const IndexedTriangleSetType<AdditionalMeshInfo::None>::type &, const std::vector<char> &);
template std::vector<Vec3i> its_face_edge_ids<AdditionalMeshInfo::Color>(
    const IndexedTriangleSetType<AdditionalMeshInfo::Color>::type &, const std::vector<char> &);
...
std::vector<Vec3i> its_face_edge_ids(const indexed_triangle_set &its, std::vector<Vec3i> &face_neighbors,
                                     bool assign_unbound_edges, int *num_edges)
{
...
int its_merge_vertices(indexed_triangle_set &its, bool shrink_to_fit)
{
    ...
    std::sort(sorted.begin(), sorted.end(),
              [&its](int il, int ir)
              {
                  ...
                  return l.x() < r.x() ||
                         (l.x() == r.x() &&
                          (l.y() < r.y() || (l.y() == r.y() && (l.z() < r.z() || (l.z() == r.z() && il < ir)))));
              });
...
{
    return std::count_if(its.indices.begin(), its.indices.end(),
...
bool its_store_triangles_to_obj(const indexed_triangle_set &its, const char *obj_filename,
...
void its_collect_mesh_projection_points_above(const indexed_triangle_set &its, const TransformVertex &transform_fn,
                                              const float z, Points &all_pts)
{
    all_pts.reserve(all_pts.size() + its.indices.size() * 3);
    for (const stl_triangle_vertex_indices &tri : its.indices)
    {
        const Vec3f pts[3] = {transform_fn(its.vertices[tri(0)]), transform_fn(its.vertices[tri(1)]),
                              transform_fn(its.vertices[tri(2)])};
        int iprev = 2;
        for (int iedge = 0; iedge < 3; ++iedge)
        {
            ...
            if ((p1.z() < z && p2.z() > z) || (p2.z() < z && p1.z() > z))
            {
                ...
                all_pts.emplace_back(scaled<coord_t>(p1.x() + (p2.x() - p1.x()) * t),
                                     scaled<coord_t>(p1.y() + (p2.y() - p1.y()) * t));
            }
            if (p2.z() >= z)
...
void its_collect_mesh_projection_points_above(const indexed_triangle_set &its, const Matrix3f &m, const float z,
                                              Points &all_pts)
{
    return its_collect_mesh_projection_points_above(its, [m](const Vec3f &p) { return m * p; }, z, all_pts);
}

void its_collect_mesh_projection_points_above(const indexed_triangle_set &its, const Transform3f &t, const float z,
                                              Points &all_pts)
{
...
Polygon its_convex_hull_2d_above(const indexed_triangle_set &its, const TransformVertex &transform_fn, const float z)
{
    auto collect_mesh_projection_points_above = [&](const tbb::blocked_range<size_t> &range)
    {
        ...
        for (size_t i = range.begin(); i < range.end(); ++i)
        {
            ...
            const Vec3f tri_pts[3] = {transform_fn(its.vertices[tri(0)]), transform_fn(its.vertices[tri(1)]),
                                      transform_fn(its.vertices[tri(2)])};
            int iprev = 2;
            for (int iedge = 0; iedge < 3; ++iedge)
            {
                ...
                if ((p1.z() < z && p2.z() > z) || (p2.z() < z && p1.z() > z))
                {
                    ...
                    pts.emplace_back(scaled<coord_t>(p1.x() + (p2.x() - p1.x()) * t),
                                     scaled<coord_t>(p1.y() + (p2.y() - p1.y()) * t));
                }
                if (p2.z() >= z)
...
indexed_triangle_set its_make_sphere(double radius, double fa)
{
    ...
    for (int iter = 0; iter < iterations; ++iter)
    {
        ...
        for (int i = 0; i < orig_indices_size; ++i)
        { // iterate over all old triangles
            ...
            std::array<std::pair<int, int>, 3> edge_children = {std::make_pair(i, last_triangle_idx + 2),
                                                                std::make_pair(last_triangle_idx + 2,
                                                                               last_triangle_idx + 3),
            ...
            indices.emplace_back(
                stl_triangle_vertex_indices(middle_vertices_idxs[0], middle_vertices_idxs[1], middle_vertices_idxs[2]));
            new_neighbors[indices.size() - 1] = Vec3i(coord_t(last_index + 2), coord_t(last_index + 3), coord_t(i));

            indices.emplace_back(
                stl_triangle_vertex_indices(middle_vertices_idxs[0], indices[i][1], middle_vertices_idxs[1]));
            new_neighbors[indices.size() - 1] = Vec3i(coord_t(new_neighbors_per_edge[0].second),
                                                      coord_t(new_neighbors_per_edge[1].first),
                                                      coord_t(last_index + 1));

            indices.emplace_back(
                stl_triangle_vertex_indices(middle_vertices_idxs[2], middle_vertices_idxs[1], indices[i][2]));
            new_neighbors[indices.size() - 1] = Vec3i(coord_t(last_index + 1),
                                                      coord_t(new_neighbors_per_edge[1].second),
                                                      coord_t(new_neighbors_per_edge[2].first));

            indices[i][1] = middle_vertices_idxs[0];
            indices[i][2] = middle_vertices_idxs[2];
            new_neighbors[i] = Vec3i(coord_t(new_neighbors_per_edge[0].first), coord_t(last_index + 1),
                                     coord_t(new_neighbors_per_edge[2].second));
        }
...
indexed_triangle_set its_make_snap(double r, double h, float space_proportion, float bulge_proportion)
{
    ...
    auto add_side_vertices = [b_vec, t_vec, b_height, m_height, t_height](std::vector<stl_vertex> &vertices,
                                                                          float b_angle, float t_angle,
                                                                          const Vec2f &m_vec)
    {
    ...
    auto add_side_facets =
        [](std::vector<stl_triangle_vertex_indices> &facets, int vertices_cnt, int frst_id, int scnd_id)
    {
    ...
    auto add_sub_mesh =
        [add_side_vertices, add_side_facets, get_m_len, b_height, t_height, b_angle, t_angle, b_angle_step,
    ...
    if (!pts.empty())
    {
        ...
        for (const orgQhull::QhullFacet &facet : qhull.facetList())
        {
            ...
            if (cnt == 3)
            {
                ...
                Vec3f n = (dst_vertices[indices(1)] - dst_vertices[indices(0)])
                              .cross(dst_vertices[indices(2)] - dst_vertices[indices(1)]);
...
float its_average_edge_length(const indexed_triangle_set &its)
{
    ...
    for (size_t i = 0; i < its.indices.size(); ++i)
    {
        ...
        edge_length += (v[1] - v[0]).cast<double>().norm() + (v[2] - v[0]).cast<double>().norm() +
...
bool its_write_stl_ascii(const char *file, const char *label, const std::vector<stl_triangle_vertex_indices> &indices,
...
bool its_write_stl_binary(const char *file, const char *label, const std::vector<stl_triangle_vertex_indices> &indices,
                          const std::vector<stl_vertex> &vertices)
{
