...
void read_tree(const boost::property_tree::ptree::value_type &section, boost::filesystem::path &model_path,
               std::string &material, std::string &material_color, std::vector<std::string> &transformation_matrix)
{
...
bool fill_model(Model *model, const boost::filesystem::path &model_path, const std::string &material,
                const std::vector<std::string> &transformation_matrix)
{
...
void add_instance(Model *model, const boost::filesystem::path &model_path,
                  const std::vector<std::string> &transformation_matrix)
{
    if (transformation_matrix.size() >= 16)
    {
        auto string_to_double = [model_path](const std::string &from) -> double
        {
            ...
            if (answer.ec != std::errc())
                throw Slic3r::RuntimeError("Failed reading PrintRequest file. Couldn't parse transformation matrix. " +
                                           model_path.string());
            return ret_val;
        };
        ...
        try
        {
            offset_vector = Slic3r::Vec3d(string_to_double(transformation_matrix[3]),
                                          string_to_double(transformation_matrix[7]),
                                          string_to_double(transformation_matrix[11]));
