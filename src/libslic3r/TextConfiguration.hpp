...
struct FontProp
{
    ...
    // Duplicated font metrics for .3mf store/load

    // Height of text line (letters)
    // duplicated from font descriptor PointSize
    ...
    {
        return char_gap == other.char_gap && line_gap == other.line_gap && per_glyph == other.per_glyph &&
               align == other.align && is_approx(size_in_mm, other.size_in_mm) && is_approx(boldness, other.boldness) &&
...
struct EmbossStyle
{
    ...
    enum class Type
    {
        ...
        // platform-dependent font descriptor strings
        win_font_descr, // on Windows
        lin_font_descr, // on Linux
        mac_font_descr, // on macOS
