...
public:
    ...
    public:
        ...
        virtual std::vector<int> get_facets_to_select(int facet_idx, const std::vector<Vertex> &vertices,
                                                      const std::vector<Triangle> &triangles, int orig_size_vertices,
                                                      int orig_size_indices) const
        {
            return {facet_idx};
        };
    ...
    protected:
        explicit Cursor(const Vec3f &source_, float radius_world, const Transform3d &trafo_,
    ...
    public:
        ...
        static std::unique_ptr<Cursor> cursor_factory(const Vec3f &center, const Vec3f &camera_pos,
                                                      const float cursor_radius, const CursorType cursor_type,
                                                      const Transform3d &trafo_matrix,
                                                      const ClippingPlane &clipping_plane)
        {
            assert(cursor_type == TriangleSelector::CursorType::CIRCLE ||
                   cursor_type == TriangleSelector::CursorType::SPHERE);
            if (cursor_type == TriangleSelector::CursorType::SPHERE)
                return std::make_unique<TriangleSelector::Sphere>(center, camera_pos, cursor_radius, trafo_matrix,
                                                                  clipping_plane);
            else
                return std::make_unique<TriangleSelector::Circle>(center, camera_pos, cursor_radius, trafo_matrix,
                                                                  clipping_plane);
        }

    protected:
        explicit SinglePointCursor(const Vec3f &center_, const Vec3f &source_, float radius_world,
    ...
    public:
        ...
        static std::unique_ptr<Cursor> cursor_factory(const Vec3f &first_center, const Vec3f &second_center,
                                                      const Vec3f &camera_pos, const float cursor_radius,
                                                      const CursorType cursor_type, const Transform3d &trafo_matrix,
                                                      const ClippingPlane &clipping_plane)
        {
            assert(cursor_type == TriangleSelector::CursorType::CIRCLE ||
                   cursor_type == TriangleSelector::CursorType::SPHERE);
            if (cursor_type == TriangleSelector::CursorType::SPHERE)
                return std::make_unique<TriangleSelector::Capsule3D>(first_center, second_center, camera_pos,
                                                                     cursor_radius, trafo_matrix, clipping_plane);
            else
                return std::make_unique<TriangleSelector::Capsule2D>(first_center, second_center, camera_pos,
                                                                     cursor_radius, trafo_matrix, clipping_plane);
        }

    protected:
        explicit DoublePointCursor(const Vec3f &first_center_, const Vec3f &second_center_, const Vec3f &source_,
                                   float radius_world, const Transform3d &trafo_, const ClippingPlane &clipping_plane_);
    ...
    public:
        Sphere() = delete;
        explicit Sphere(const Vec3f &center_, const Vec3f &source_, float radius_world, const Transform3d &trafo_,
                        const ClippingPlane &clipping_plane_)
    ...
    public:
        Circle() = delete;
        explicit Circle(const Vec3f &center_, const Vec3f &source_, float radius_world, const Transform3d &trafo_,
                        const ClippingPlane &clipping_plane_)
            : SinglePointCursor(center_, source_, radius_world, trafo_, clipping_plane_) {};
    ...
    public:
        Capsule3D() = delete;
        explicit Capsule3D(const Vec3f &first_center_, const Vec3f &second_center_, const Vec3f &source_,
                           float radius_world, const Transform3d &trafo_, const ClippingPlane &clipping_plane_)
            : TriangleSelector::DoublePointCursor(first_center_, second_center_, source_, radius_world, trafo_,
                                                  clipping_plane_)
        {
        }
    ...
    public:
        Capsule2D() = delete;
        explicit Capsule2D(const Vec3f &first_center_, const Vec3f &second_center_, const Vec3f &source_,
                           float radius_world, const Transform3d &trafo_, const ClippingPlane &clipping_plane_)
            : TriangleSelector::DoublePointCursor(first_center_, second_center_, source_, radius_world, trafo_,
                                                  clipping_plane_)
        {
    ...
    public:
        ...
        explicit HeightRange(const Vec3f &mesh_hit, const BoundingBoxf3 &mesh_bbox, float z_range,
        ...
        std::vector<int> get_facets_to_select(int facet_idx, const std::vector<Vertex> &vertices,
                                              const std::vector<Triangle> &triangles, int orig_size_vertices,
                                              int orig_size_indices) const override;
    ...
    private:
        ...
        {
            ar(triangle_idx, bitstream_start_idx);
        }
    };
    ...
    struct TriangleSplittingData
    {
        ...
        {
            return lhs.triangles_to_split == rhs.triangles_to_split && lhs.bitstream == rhs.bitstream &&
    ...
    private:
        ...
        void serialize(Archive &ar)
        {
            ar(triangles_to_split, bitstream, used_states);
        }
    };

    std::pair<std::vector<Vec3i>, std::vector<Vec3i>> precompute_all_neighbors() const;
    void precompute_all_neighbors_recursive(int facet_idx, const Vec3i &neighbors, const Vec3i &neighbors_propagated,
                                            std::vector<Vec3i> &neighbors_out,
                                            std::vector<Vec3i> &neighbors_normal_out) const;
    ...
    void select_patch(
        ...
        std::unique_ptr<Cursor> &&
            cursor, // Cursor containing information about the point where to start, camera position (mesh coords), matrix to get from mesh to world, and its shape and type.
        ...
        float highlight_by_angle_deg =
            0.f); // The maximal angle of overhang. If it is set to a non-zero value, it is possible to paint only the triangles of overhang defined by this angle in degrees.
    ...
    void seed_fill_select_triangles(
        ...
        float highlight_by_angle_deg =
            0.f, // The maximal angle of overhang. If it is set to a non-zero value, it is possible to paint only the triangles of overhang defined by this angle in degrees.
        ForceReselection force_reselection = ForceReselection::
            NO); // force reselection of the triangle mesh even in cases that mouse is pointing on the selected triangle
    ...
    void bucket_fill_select_triangles(
        ...
        BucketFillPropagate
            propagate, // if bucket fill is propagated to neighbor faces or if it fills the only facet of the modified mesh that the hit point belongs to.
        ForceReselection force_reselection = ForceReselection::
            NO); // force reselection of the triangle mesh even in cases that mouse is pointing on the selected triangle
    ...
    typename IndexedTriangleSetType<facet_info>::type get_facets(
    ...
    template<AdditionalMeshInfo facet_info = AdditionalMeshInfo::None>
    typename IndexedTriangleSetType<facet_info>::type get_facets_strict(
        const std::function<bool(const Triangle &)> &facet_filter) const;
...
private:
    ...
    bool select_triangle_recursive(int facet_idx, const Vec3i &neighbors, TriangleStateType type,
    ...
    Vec3i child_neighbors_propagated(const Triangle &tr, const Vec3i &neighbors_propagated, int child_idx,
    ...
    void append_touching_subtriangles(int itriangle, int vertexi, int vertexj,
    ...
    std::vector<int> get_all_touching_triangles(int facet_idx, const Vec3i &neighbors,
    ...
    void seed_fill_fill_gaps(
        const std::vector<int> &
            gap_fill_candidate_facets, // Facet of the original mesh (unsplit), which needs to be checked if the surrounding gap can be filled (selected).
        float seed_fill_gap_area); // The maximal area that will be automatically selected when the surrounding triangles have already been selected.

    void bucket_fill_fill_gaps(
        const std::vector<int> &
            gap_fill_candidate_facets, // Facet of the mesh (unsplit), which needs to be checked if the surrounding gap can be filled (selected).
        float bucket_fill_gap_area, // The maximal area that will be automatically selected when the surrounding triangles have already been selected.
        TriangleStateType
            start_facet_state, // The state of the starting facet that determines which neighbors to consider.
        const std::vector<Vec3i> &neighbors, const std::vector<Vec3i> &neighbors_propagate);
