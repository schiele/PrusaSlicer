...
public:
    ...
    static typename enable_if<
        typename gtl_and<
            typename gtl_if<typename is_point_concept<typename geometry_concept<Point<InCT1>>::type>::type>::type,
            typename gtl_if<typename is_segment_concept<typename geometry_concept<Segment<InCT2>>::type>::type>::type>::
            type,
        void>::type
    discretize(const Point<InCT1> &point, const Segment<InCT2> &segment, const CT max_dist,
...
private:
    ...
    static typename enable_if<
        typename gtl_and<
            typename gtl_if<typename is_point_concept<typename geometry_concept<Point<int>>::type>::type>::type,
...
inline point_type retrieve_point(const std::vector<segment_type> &segments, const cell_type &cell)
{
    ...
    return (cell.source_category() == boost::polygon::SOURCE_CATEGORY_SEGMENT_START_POINT)
               ? low(segments[cell.source_index()])
               : high(segments[cell.source_index()]);
}

inline void clip_infinite_edge(const std::vector<segment_type> &segments, const edge_type &edge,
                               coordinate_type bbox_max_size, std::vector<point_type> *clipped_edge)
{
    ...
    else
    {
        ...
        segment_type segment = cell1.contains_segment() ? segments[cell1.source_index()]
                                                        : segments[cell2.source_index()];
...
inline void sample_curved_edge(const std::vector<segment_type> &segments, const edge_type &edge,
                               std::vector<point_type> &sampled_edge, coordinate_type max_dist)
{
...
void dump_voronoi_to_svg(const Lines &lines, /* const */ boost::polygon::voronoi_diagram<double> &vd,
                         const ThickPolylines *polylines, const char *path)
{
    ...
    for (Lines::const_iterator it = lines.begin(); it != lines.end(); ++it)
        segments.push_back(
            Voronoi::Internal::segment_type(Voronoi::Internal::point_type(double(it->a(0)), double(it->a(1))),
                                            Voronoi::Internal::point_type(double(it->b(0)), double(it->b(1)))));

    // Color exterior edges.
    for (boost::polygon::voronoi_diagram<double>::const_edge_iterator it = vd.edges().begin(); it != vd.edges().end();
         ++it)
        if (!it->is_finite())
    ...
    for (Lines::const_iterator it = lines.begin(); it != lines.end(); ++it)
        svg.draw(Line(Point(coord_t(it->a(0)), coord_t(it->a(1))), Point(coord_t(it->b(0)), coord_t(it->b(1)))),
                 inputSegmentColor, inputSegmentLineWidth);
...
template<typename VD, typename SEGMENTS>
inline std::pair<typename VD::coord_type, typename VD::coord_type> measure_edge_thickness(
    const VD &vd, const typename VD::edge_type &edge, const SEGMENTS &segments)
{
...
void MedialAxis::build(ThickPolylines *polylines)
{
    ...
    */
    ...
    for (VD::const_edge_iterator edge = m_vd.edges().begin(); edge != m_vd.edges().end(); edge += 2)
        // Some degenerate input polygons can produce Voronoi diagrams where is_finite() returns true
        // but one of the vertices is still null. Check before accessing vertex properties.
        if (edge->is_primary() && edge->is_finite() && edge->vertex0() != nullptr && edge->vertex1() != nullptr &&
    ...
    for (VD::const_edge_iterator seed_edge = m_vd.edges().begin(); seed_edge != m_vd.edges().end(); seed_edge += 2)
        if (EdgeData &seed_edge_data = this->edge_data(*seed_edge).first; seed_edge_data.active)
        {
            ...
            polyline.points.insert(polyline.points.begin(), reverse_polyline.points.rbegin(),
                                   reverse_polyline.points.rend());
            polyline.width.insert(polyline.width.begin(), reverse_polyline.width.rbegin(),
                                  reverse_polyline.width.rend());
...
{
    // Some degenerate input can produce edges with null vertices even when is_finite() is true.
    if (edge->vertex0() == nullptr || edge->vertex1() == nullptr)
        return false;

    auto retrieve_segment = [this](const VD::cell_type *cell) -> const Line &
    {
        return m_lines[cell->source_index()];
    };
    ...
    constexpr double CLIPPER2_MAX_COORD = 4611686018427387903.0; // 2^62 - 1
    if (std::abs(edge->vertex0()->x()) > CLIPPER2_MAX_COORD || std::abs(edge->vertex0()->y()) > CLIPPER2_MAX_COORD ||
        std::abs(edge->vertex1()->x()) > CLIPPER2_MAX_COORD || std::abs(edge->vertex1()->y()) > CLIPPER2_MAX_COORD)
        return false;
