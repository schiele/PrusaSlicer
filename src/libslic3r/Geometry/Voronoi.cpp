                                  ...
                                  const bool try_to_repair_if_needed)
{
    ...
    {
        if (m_issue_type = detect_known_issues(*this, segment_begin, segment_end);
            m_issue_type != IssueType::NO_ISSUE_DETECTED)
        {
            ...
            else
            {
                BOOST_LOG_TRIVIAL(error)
                    << "Detected unknown Voronoi diagram issue, input polygons will be rotated back and forth.";
            }
...
VoronoiDiagram::detect_known_issues(const VoronoiDiagram &voronoi_diagram, SegmentIterator segment_begin,
                                                 ...
                                                 const SegmentIterator segment_begin, const SegmentIterator segment_end)
{
    ...
    for (VD::cell_type cell : voronoi_diagram.cells())
    {
        ...
        if (const SegmentCellRange cell_range = VoronoiUtils::compute_segment_cell_range(cell, segment_begin,
                                                                                         segment_end);
            cell_range.is_valid())
        {
            ...
            const Segment &source_segment = Geometry::VoronoiUtils::get_source_segment(cell, segment_begin,
                                                                                       segment_end);
            const Vec2d source_segment_from = boost::polygon::segment_traits<Segment>::get(source_segment,
                                                                                           boost::polygon::LOW)
                                                  .template cast<double>();
            const Vec2d source_segment_to = boost::polygon::segment_traits<Segment>::get(source_segment,
                                                                                         boost::polygon::HIGH)
                                                .template cast<double>();
...
typename boost::polygon::enable_if<
    typename boost::polygon::gtl_if<typename boost::polygon::is_segment_concept<typename boost::polygon::geometry_concept<
        typename std::iterator_traits<SegmentIterator>::value_type>::type>::type>::type,
    VoronoiDiagram::IssueType>::type
VoronoiDiagram::try_to_repair_degenerated_voronoi_diagram(const SegmentIterator segment_begin,
...
inline VD::vertex_type::color_type encode_input_segment_endpoint(
    const VD::cell_type::source_index_type cell_source_index, const boost::polygon::direction_1d dir)
{
...
decode_input_segment_endpoint(const VD::vertex_type::color_type color, const SegmentIterator segment_begin,
                              const SegmentIterator segment_end)
{
    ...
    const PointType source_point = boost::polygon::segment_traits<SegmentType>::get(
        *segment_it, ((color & 1) ? boost::polygon::HIGH : boost::polygon::LOW));
    return source_point;
}
                                                                      ...
                                                                      const double fix_angle)
{
    ...
    for (cell_type cell : m_cells)
    {
        ...
        {
            if (const SegmentCellRange cell_range = VoronoiUtils::compute_segment_cell_range(cell,
                                                                                             segments_rotated.begin(),
                ...
                cell_range.is_valid())
            {
                if (cell_range.edge_end->vertex1()->color() == 0)
                {
                    ...
                    VD::vertex_type::color_type color = encode_input_segment_endpoint(cell.source_index(),
                                                                                      boost::polygon::LOW);
