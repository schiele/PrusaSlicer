...
{
public:
    WipeTowerWriter(float layer_height, float line_width, GCodeFlavor flavor,
        ...
        , m_filpar(filament_parameters)
    {
        ...
        str << ";" << GCodeProcessor::reserved_tag(GCodeProcessor::ETags::Role)
            << gcode_extrusion_role_to_string(GCodeExtrusionRole::WipeTower) << "\n";
        m_gcode += str.str();
    ...
    WipeTowerWriter &set_initial_position(const Vec2f &pos, float width = 0.f, float depth = 0.f,
    ...
    {
        if (m_gcode_flavor == gcfRepRapSprinter || m_gcode_flavor == gcfRepRapFirmware || m_gcode_flavor == gcfRapid)
    ...
    WipeTowerWriter &extrude_explicit(float x, float y, float e, float f = 0.f, bool record_length = false,
    ...
    WipeTowerWriter &extrude_explicit(const Vec2f &dest, float e, float f = 0.f, bool record_length = false,
                                      bool limit_volumetric_flow = true)
    {
    ...
    WipeTowerWriter &load_move_x_advanced(float farthest_x, float loading_dist, float loading_speed,
    ...
    {
        // This is only supported by some firmware
    ...
    WipeTowerWriter &set_fan(unsigned speed)
    {
        ...
        else
        {
            ...
            case gcfMach3:
            case gcfMachinekit:
                m_gcode += "M106 P" + std::to_string(unsigned(255.0 * speed / 100.0)) + "\n";
                break;
            default:
    ...
    WipeTowerWriter &add_wipe_point(float x, float y) { return add_wipe_point(Vec2f(x, y)); }
...
private:
    ...
    Vec2f rotate(Vec2f pt) const
    {
        ...
        double c = cos(angle);
        double s = sin(angle);
        return Vec2f(float(pt.x() * c - pt.y() * s) + m_wipe_tower_width / 2.f,
                     float(pt.x() * s + pt.y() * c) + m_wipe_tower_depth / 2.f);
    }
...
WipeTower::WipeTower(const Vec2f &pos, double rotation_deg, const PrintConfig &config,
    ...
    , wipe_volumes(wiping_matrix)
{
    ...
    m_is_mk4mmu3 =
        boost::icontains(config.printer_notes.value, "RAMMING_EXTRA")
        ...
        || (boost::icontains(config.printer_notes.value, "PRINTER_MODEL_MK4") &&
            boost::icontains(config.printer_notes.value, "MMU"));
    ...
    m_bed_bottom_left = m_bed_shape == RectangularBed ? Vec2f(bed_points.front().x(), bed_points.front().y())
                                                      : Vec2f::Zero();
}
...
void WipeTower::set_extruder(size_t idx, const PrintConfig &config)
{
    ...
    m_filpar[idx].is_soluble = config.wipe_tower_extruder == 0 ? config.filament_soluble.get_at(idx)
                                                               : (idx != size_t(config.wipe_tower_extruder - 1));
    ...
    m_filpar[idx].filament_area = float(
        (M_PI / 4.f) * pow(config.filament_diameter.get_at(idx),
                           2)); // all extruders are assumed to have the same filament diameter at this point
    ...
    m_perimeter_width = nozzle_diameter *
                        Width_To_Nozzle_Ratio; // all extruders are now assumed to have the same diameter
    ...
    m_used_filament_length.resize(
        std::max(m_used_filament_length.size(),
                 idx + 1)); // makes sure that the vector is big enough so we don't have to check later
}
    ...
    bool /*last_wipe_inside_wipe_tower*/)
{
    ...
    // Some printers have a working space of [0, -2.2] to [250, 210].
    ...
    box_coordinates cleaning_box(Vec2f(0.02f * m_bed_width, 0.01f + m_perimeter_width / 2.f), prime_section_width,
                                 100.f);
    if (m_bed_shape == CircularBed)
    {
        ...
        cleaning_box.translate(-total_width_half,
                               -std::sqrt(std::max(0.f, std::pow(m_bed_width / 2, 2.f) -
                                                            std::pow(1.05f * total_width_half, 2.f))));
    }
    else
    ...
    for (size_t idx_tool = 0; idx_tool < tools.size(); ++idx_tool)
    {
        ...
        toolchange_Change(
            writer, tool,
            m_filpar[tool].material);          // Select the tool, set a speed override for soluble and flex materials.
        ...
        else
        {
            ...
            toolchange_Unload(writer, box, m_filpar[m_current_tool].material,
    ...
    m_old_temperature =
        -1; // If the priming is turned off in config, the temperature changing commands will not actually appear
...
WipeTower::ToolChangeResult WipeTower::tool_change(size_t tool)
{
    ...
    box_coordinates cleaning_box(Vec2f(m_perimeter_width / 2.f, m_perimeter_width / 2.f),
    ...
    if (tool != (unsigned) (-1))
    {
        ...
        writer
            .append(std::string("; material : " +
                                (m_current_tool < m_filpar.size() ? m_filpar[m_current_tool].material : "(NONE)") +
    ...
    { // This is not the last change.
        toolchange_Unload(writer, cleaning_box, m_filpar[m_current_tool].material,
                          (is_first_layer() ? m_filpar[m_current_tool].first_layer_temperature
                                            : m_filpar[m_current_tool].temperature),
                          (is_first_layer() ? m_filpar[tool].first_layer_temperature : m_filpar[tool].temperature));
        toolchange_Change(
            writer, tool,
            m_filpar[tool].material); // Change the tool, set a speed override for soluble and flex materials.
        toolchange_Load(writer, cleaning_box);
        writer.travel(writer.x(), writer.y() - m_perimeter_width); // cooling and loading were done a bit down the road
        toolchange_Wipe(writer, cleaning_box,
                        wipe_volume); // Wipe the newly loaded filament until the end of the assigned wipe area.
        ++m_num_tool_changes;
    }
    else
...
// Ram the hot material out of the melt zone, retract the filament into the cooling tubes and let it cool.
void WipeTower::toolchange_Unload(WipeTowerWriter &writer, const box_coordinates &cleaning_box,
                                  const std::string &current_material, const int old_temperature,
                                  const int new_temperature)
{
    ...
    const float line_width =
        m_perimeter_width * m_filpar[m_current_tool].ramming_line_width_multiplicator; // desired ramming line thickness
    ...
    if (do_ramming && (m_layer_info > m_plan.begin() && m_layer_info < m_plan.end() &&
                       (m_layer_info - 1 != m_plan.begin() || !m_adhesion)))
    {
        ...
        { // let's find this toolchange
            if (tch.old_tool == m_current_tool)
            {
                sum_of_depths += tch.ramming_depth;
                float ramming_end_y = sum_of_depths;
                ramming_end_y -= (y_step / m_extra_spacing_ramming - m_perimeter_width) /
                                 2.f; // center of final ramming line

                if ((m_current_shape == SHAPE_REVERSED &&
                     ramming_end_y < sparse_beginning_y - 0.5f * m_perimeter_width) ||
    ...
    while (do_ramming && i < m_filpar[m_current_tool].ramming_speed.size())
    {
        ...
        const float x = volume_to_length(m_filpar[m_current_tool].ramming_speed[i] * time_step, line_width,
                                         m_layer_height);
        const float e = m_filpar[m_current_tool].ramming_speed[i] * time_step /
                        filament_area(); // transform volume per sec to E move;
        const float dist = std::min(
            x - e_done, remaining); // distance to travel for either the next time_step, or to the next turnaround
        const float actual_time = dist / x * time_step;
        writer.ram(writer.x(), writer.x() + (m_left_to_right ? 1.f : -1.f) * dist, 0.f, 0.f, e * (dist / x),
                   dist / (actual_time / 60.f));
        remaining -= dist;

        if (remaining < WT_EPSILON)
    ...
    writer.change_analyzer_line_width(
        m_perimeter_width); // so the next lines are not affected by ramming_line_width_multiplier
    ...
    if (m_semm)
    {
        if (new_temperature != 0 && (new_temperature != m_old_temperature || is_first_layer() || cold_ramming))
    ...
    if (cooling_will_happen)
    {
        ...
        for (int i = 0; i < number_of_cooling_moves; ++i)
        {
            ...
            if (i > 0 && m_filpar[m_current_tool].filament_stamping_distance != 0)
            {
                ...
                float stamping_turning_point = std::clamp(old_x + 20.f * (turning_point - old_x > 0.f ? 1.f : -1.f), xl,
                                                          xr);
                ...
                if (stamping_dist_e > 5)
                {
                    ...
                    writer.load_move_x_advanced(cent, 5, m_filpar[m_current_tool].filament_stamping_loading_speed,
                                                m_travel_speed);
                    writer.travel(cent, writer.y());
                }
                else
                    writer.load_move_x_advanced_there_and_back(stamping_turning_point, stamping_dist_e,
                                                               m_filpar[m_current_tool].filament_stamping_loading_speed,
                                                               m_travel_speed);
    ...
    Vec2f pos = Vec2f(end_of_ramming.x(), end_of_ramming.y() +
...
void WipeTower::toolchange_Change(WipeTowerWriter &writer, const size_t new_tool, const std::string &new_material)
{
    ...
    writer
        .feedrate(m_travel_speed * 60.f) // Feedrate fix for wipe tower
...
void WipeTower::toolchange_Wipe(WipeTowerWriter &writer, const box_coordinates &cleaning_box, float wipe_volume)
{
    ...
    float dy =
        (is_first_layer() ? m_extra_flow : m_extra_spacing_wipe) *
    ...
    if (is_first_layer())
    {
        const float cleaning_box_height = cleaning_box.lu.y() - (0.5f * line_width) - float(EPSILON) -
                                          (writer.y() + dy);
    ...
    for (int i = 0; true; ++i)
    {
        ...
        writer.extrude(writer.x() + (i % 4 == 0 ? -1.f : (i % 4 == 1 ? 1.f : 0.f)) * 1.5f * line_width,
                       writer.y() + dy);
        m_left_to_right = !m_left_to_right;
    }
...
WipeTower::ToolChangeResult WipeTower::finish_layer()
{
    ...
    auto supported_rectangle = [this, &writer, spacing](const box_coordinates &wt_box, double feedrate,
                                                        bool infill_cone) -> Polygon
    {
        const auto [R, support_scale] = get_wipe_tower_cone_base(m_wipe_tower_width, m_wipe_tower_height,
                                                                 m_wipe_tower_depth, m_wipe_tower_cone_angle);
        ...
        if (double alpha_start = std::asin((0.5 * w) / r); !std::isnan(alpha_start) && r > 0.5 * w + 0.01)
        {
            for (double alpha = alpha_start; alpha < M_PI - alpha_start + 0.001;
                ...
                pts.emplace_back(Vec2f(center.x() + r * std::cos(alpha) / support_scale,
        ...
        for (int i = int(pts.size()) - 3; i > 0; --i)
            pts.emplace_back(Vec2f(pts[i].first.x(), 2 * center.y() - pts[i].first.y()), i == int(pts.size()) - 3
                                                                                             ? ArcStart
                                                                                         : i == 1 ? ArcEnd
                                                                                                  : Arc);
        ...
        while (i != start_i)
        {
            ...
            if (pts[i].second == ArcEnd)
            {
                ...
                if (!polylines.empty())
                {
                    ...
                    bool mirror = ((pts[i].first.y() - center.y()) *
                                   (unscale(polylines.front().points.front()).y() - center.y())) < 0.;
                    for (const Polyline &line : polylines)
                    {
                        writer.travel(center -
                                      (mirror ? 1.f : -1.f) * (unscale(line.points.front()).cast<float>() - center));
                        for (size_t i = 0; i < line.points.size(); ++i)
                            writer.extrude(center -
                                           (mirror ? 1.f : -1.f) * (unscale(line.points[i]).cast<float>() - center));
                    }
...
std::vector<std::vector<float>> WipeTower::extract_wipe_volumes(const PrintConfig &config)
{
    ...
    if (config.single_extruder_multi_material && !config.wiping_volumes_use_custom_matrix)
    {
        for (size_t i = 0; i < number_of_extruders; ++i)
        {
            ...
            {
                if (i != j)
                    wipe_volumes[i][j] = (i == j ? 0.f
                                                 : config.multimaterial_purging.value *
                                                       config.filament_purge_multiplier.get_at(j) / 100.f);
            }
        }
    }
    ...
    for (unsigned int i = 0; i < number_of_extruders; ++i)
        for (unsigned int j = 0; j < number_of_extruders; ++j)
            wipe_volumes[i][j] = std::max<float>(wipe_volumes[i][j],
                                                 config.filament_minimal_purge_on_wipe_tower.get_at(j));

    return wipe_volumes;
}

static float get_wipe_depth(float volume, float layer_height, float perimeter_width, float extra_flow,
                            float extra_spacing, float width)
{
...
void WipeTower::plan_toolchange(float z_par, float layer_height_par, unsigned int old_tool, unsigned int new_tool,
                                float wipe_volume)
{
    assert(m_plan.empty() || m_plan.back().z <= z_par + WT_EPSILON); // refuses to add a layer below the last one

    if (m_plan.empty() ||
        m_plan.back().z + WT_EPSILON < z_par) // if we moved to a new layer, we'll add it to m_plan first
    ...
    float length_to_extrude = volume_to_length(0.25f * std::accumulate(m_filpar[old_tool].ramming_speed.begin(),
                                               ...
                                               layer_height_par);
    float ramming_depth = (int(length_to_extrude / width) + 1) *
                          (m_perimeter_width * m_filpar[old_tool].ramming_line_width_multiplicator *
    ...
    float wiping_depth = get_wipe_depth(wipe_volume - first_wipe_volume, layer_height_par, m_perimeter_width,
                                        m_extra_flow, m_extra_spacing_wipe, width);

    m_plan.back().tool_changes.push_back(WipeTowerInfo::ToolChange(old_tool, new_tool, ramming_depth + wiping_depth,
                                                                   ramming_depth, first_wipe_line, wipe_volume));
}
...
void WipeTower::save_on_last_wipe()
{
    ...
    {
        set_layer(m_layer_info->z, m_layer_info->height, 0, m_layer_info->z == m_plan.front().z,
                  m_layer_info->z == m_plan.back().z);
        ...
        for (int i = 0; i < int(m_layer_info->tool_changes.size()); ++i)
        {
            ...
            if (i == idx)
            {
                ...
                float volume_to_save = length_to_volume(finish_layer().total_extrusion_length_in_plane(),
                                                        m_perimeter_width, m_layer_info->height);
                ...
                float volume_we_need_depth_for = std::max(0.f, volume_left_to_wipe -
                                                                   length_to_volume(toolchange.first_wipe_line,
                                                                                    m_perimeter_width * m_extra_flow,
                                                                                    m_layer_info->height));
                float depth_to_wipe = get_wipe_depth(volume_we_need_depth_for, m_layer_info->height, m_perimeter_width,
                                                     m_extra_flow, m_extra_spacing_wipe, width);
