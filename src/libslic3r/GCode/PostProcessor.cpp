...
static DWORD execute_process_winapi(const std::wstring &command_line)
{
    ...
    if (!::CreateProcessW(nullptr /* lpApplicationName */, (LPWSTR) command_line.c_str(),
                          nullptr /* lpProcessAttributes */, nullptr /* lpThreadAttributes */,
                          false /* bInheritHandles */,
                          CREATE_UNICODE_ENVIRONMENT /* | CREATE_NEW_CONSOLE */ /* dwCreationFlags */,
                          (LPVOID) envstr.c_str(), nullptr /* lpCurrentDirectory */, &startup_info, &process_info))
...
static int run_script(const std::string &script, const std::string &gcode, std::string & /*std_err*/)
{
    ...
    {
        // Run a Python script via cmd.exe so the Python launcher is found on PATH.
        command_line = L"cmd.exe /C py ";
    }
...
static int run_script(const std::string &script, const std::string &gcode, std::string &std_err)
{
    ...
    bp2::process child(ctx, shell, {"-c", command}, bp2::process_stdio{{}, {}, err_pipe});
    ...
    while (true)
    {
        size_t n = err_pipe.read_some(asio::buffer(buffer), ec);
        if (ec || n == 0)
            break;
        std_err.append(buffer.data(), n);
    }
...
bool run_post_process_scripts(std::string &src_path, bool make_copy, const std::string &host, std::string &output_name,
                              const DynamicPrintConfig &config)
{
    ...
    if (make_copy)
    {
        ...
        catch (const std::exception &err)
        {
            BOOST_LOG_TRIVIAL(error) << Slic3r::format(
                "Failed deleting an old temporary file %1% before running a post-processing script: %2%", path,
                err.what());
        }
        // Second make a copy.
        ...
        if (copy_file(src_path, path, error_message, false) != SUCCESS)
            throw Slic3r::RuntimeError(Slic3r::format(
                "Failed making a temporary copy of G-code file %1% before running a post-processing script: %2%",
    ...
    auto delete_copy = [&path, &src_path, make_copy]()
    {
        if (make_copy)
            ...
            {
                BOOST_LOG_TRIVIAL(error) << Slic3r::format(
                    "Failed deleting a temporary copy %1% of a G-code file %2% : %3%", path, src_path, err.what());
            }
    };
    ...
    config.setenv_();
    // Let the post-processing script know the target host ("File", "PrusaLink", "Repetier", "OctoPrint", "FlashAir", "Duet", "AstroBox" ...)
    ...
    auto remove_output_name_file = [&path_output_name, &src_path]()
    {
        ...
        {
            BOOST_LOG_TRIVIAL(error) << Slic3r::format(
                "Failed deleting a file %1% carrying the final name / path of a G-code file %2%: %3%", path_output_name,
                src_path, err.what());
        }
    };
    ...
    try
    {
        ...
        if (boost::filesystem::exists(path_output_name))
        {
            try
            {
                ...
                BOOST_LOG_TRIVIAL(trace) << "Post-processing script changed the file name from " << output_name
                                         << " to " << new_output_name;
