...
///|/
#ifndef slic3r_preFlight_GCodeProcessor_hpp_
#define slic3r_preFlight_GCodeProcessor_hpp_
...
struct GCodeProcessorResult
{
    ...
    struct RoleMetrics
    {
        unsigned int max_commands_per_sec{0}; // Ceiling of max cmd/sec
        unsigned int max_layer{0};            // Layer where max occurred
    };
    ...
    GCodeProcessorResult(GCodeProcessorResult &&other) noexcept;
    GCodeProcessorResult &operator=(GCodeProcessorResult &&other) noexcept;
...
{
    float retract_length{1.0f};        // S - mm
    float retract_restart_extra{0.0f}; // R - mm (additional length on unretract)
    float retract_feedrate{45.0f};     // F - mm/s (converted from mm/min)
    float unretract_feedrate{45.0f};   // T - mm/s (converted from mm/min)
    float retract_zlift{0.0f};         // Z - mm
    bool z_lifted{false};              // true if Z is currently lifted
    float z_before_lift{0.0f};         // Z position before the lift was applied

    void reset()
    {
        retract_length = 1.0f;
        retract_restart_extra = 0.0f;
        retract_feedrate = 45.0f;
        unretract_feedrate = 45.0f;
        retract_zlift = 0.0f;
        z_lifted = false;
        z_before_lift = 0.0f;
    }
...
public:
    ...
    static bool contains_reserved_tags(const std::string &gcode, unsigned int max_count,
...
public:
    ...
    struct TimeBlock
    {
        ...
        // Marlin and RRF can't execute segments faster than ~20-25ms due to:
        // - MIN_STEPS_PER_SEGMENT (minimum 6 steps before moving)
        // - min_segment_time_us (M205 B parameter, typically 20000µs)
        // Short segments (dense infill, arc linearization) get stretched to this minimum.
        static constexpr float MIN_SEGMENT_TIME_SEC = 0.016f; // 16ms

        float time() const
        {
            const float calculated = trapezoid.acceleration_time(feedrate_profile.entry, acceleration) +
                                     trapezoid.cruise_time() + trapezoid.deceleration_time(distance, acceleration);
            return std::max(calculated, MIN_SEGMENT_TIME_SEC);
        }
    };
...
private:
    struct TimeMachine
    {
        struct State
        {
            ...
            float dir_x{0.0f};         // normalized X direction component
            float dir_y{0.0f};         // normalized Y direction component
            bool has_direction{false}; // true if this move has valid XY direction
...
private:
    ...
    FirmwareRetractionConfig m_firmware_retraction;
...
public:
    ...
    // Streaming interface, for processing G-codes just generated by preFlight in a pipelined fashion.
    ...
    std::vector<std::pair<CustomGCode::Type, std::pair<float, float>>> get_custom_gcode_times(
...
private:
    ...
    bool process_slicer_tags(const std::string_view comment);
    ...
    void process_G1(const std::array<std::optional<double>, 4> &axes = {std::nullopt, std::nullopt, std::nullopt,
                                                                        std::nullopt},
    ...
    // Set firmware retraction parameters (RepRapFirmware)
    void process_M207(const GCodeReader::GCodeLine &line);

    // Parse individual RRF M-code field for time estimation
    ...
    void create_firmware_retract_time_block(EMoveType type, float distance, float feedrate, float acceleration,
                                            Axis axis // E for retraction, Z for lift
    );
    ...
    double extract_absolute_position_on_axis(Axis axis, const GCodeReader::GCodeLine &line,
                                             double area_filament_cross_section);
};

} /* namespace Slic3r */

#endif /* slic3r_preFlight_GCodeProcessor_hpp_ */
