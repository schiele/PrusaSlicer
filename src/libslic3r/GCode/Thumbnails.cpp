...
std::unique_ptr<CompressedImageBuffer> compress_thumbnail_png(const ThumbnailData &data)
{
    ...
    out->data = tdefl_write_image_to_png_file_in_memory_ex((const void *) data.pixels.data(), data.width, data.height,
                                                           4, &out->size, MZ_DEFAULT_LEVEL, 1);
    return out;
}
...
std::string get_error_string(const ThumbnailErrors &errors)
{
    ...
    if (errors.has(ThumbnailError::InvalidVal))
        error_str += "\n - " +
                     format("Invalid input format. Expected vector of dimensions in the following format: \"%1%\"",
                            "XxY/EXT, XxY/EXT, ...");
