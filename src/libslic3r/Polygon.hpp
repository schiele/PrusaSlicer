...
inline bool polygon_is_convex(const Polygon &poly)
{
...
inline bool has_duplicate_points(Polygon &&poly)
{
    return has_duplicate_points(std::move(poly.points));
}
inline bool has_duplicate_points(const Polygon &poly)
{
...
template<class I>
IntegerOnly<I, Polygons> reserve_polygons(I cap)
{
...
public:
    ...
    ColorPolygon(std::initializer_list<Point> points, std::initializer_list<Color> colors)
        : Polygon(points), colors(colors)
    {
    }
...
{
template<>
struct geometry_concept<Slic3r::Polygon>
{
...
template<>
struct geometry_concept<Slic3r::Polygons>
{
