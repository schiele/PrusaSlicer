...
std::vector<std::string> ConfigOptionDef::cli_args(const std::string &key) const
{
    ...
    if (this->cli != ConfigOptionDef::nocli)
    {
        ...
        // Neither '=' nor '!' is used in any of the cli parameters currently defined by preFlight.
...
void ConfigDef::finalize()
{
    ...
    for (std::pair<const t_config_option_key, ConfigOptionDef> &kvp : options)
    {
        ...
        else if (def.is_gui_type_enum_open())
        {
            ...
            assert(def.gui_type != ConfigOptionDef::GUIType::f_enum_open || def.type == coFloat ||
                   def.type == coPercent || def.type == coFloatOrPercent);
            assert(def.gui_type != ConfigOptionDef::GUIType::select_open || def.type == coString ||
                   def.type == coStrings);
        }
        else
        {
...
bool ConfigBase::set_deserialize_nothrow(const t_config_option_key &opt_key_src, const std::string &value_src,
                                         ConfigSubstitutionContext &substitutions_ctxt, bool append)
{
...
void ConfigBase::set_deserialize(const t_config_option_key &opt_key_src, const std::string &value_src,
                                 ConfigSubstitutionContext &substitutions_ctxt, bool append)
{
...
bool ConfigBase::set_deserialize_raw(const t_config_option_key &opt_key_src, const std::string &value,
                                     ConfigSubstitutionContext &substitutions_ctxt, bool append)
{
    t_config_option_key opt_key = opt_key_src;
    ...
    {
        //FIXME Special handling of vectors of bools, quick and not so dirty solution.
        bool nullable = opt->nullable();
        ConfigHelpers::DeserializationSubstitution default_value =
            ConfigHelpers::DeserializationSubstitution::DefaultsToFalse;
        ...
        auto result = nullable
                          ? static_cast<ConfigOptionBoolsNullable *>(opt)->deserialize_with_substitutions(value, append,
                                                                                                          default_value)
                          : static_cast<ConfigOptionBools *>(opt)->deserialize_with_substitutions(value, append,
                                                                                                  default_value);
    ...
    else
    {
            ...
            (optdef->type == coEnum || optdef->type == coBool) && ConfigHelpers::looks_like_enum_value(value))
        {
            ...
            assert(substitutions_ctxt.rule == ForwardCompatibilitySubstitutionRule::Enable ||
                   substitutions_ctxt.rule == ForwardCompatibilitySubstitutionRule::EnableSilent);
...
ConfigSubstitutions ConfigBase::load(const std::string &filename,
                                     ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    if (is_gcode_file(filename))
    {
        ...
        file_type = (is_valid_binary_gcode(*file, true, cs_buffer.data(), cs_buffer.size()) == EResult::Success)
                        ? EFileType::BinaryGCode
                        : EFileType::AsciiGCode;
        fclose(file);
    }
    else
...
ConfigSubstitutions ConfigBase::load_from_ini(const std::string &file,
                                              ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    try
    {
...
ConfigSubstitutions ConfigBase::load_from_ini_string(const std::string &data,
                                                     ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    boost::property_tree::ptree tree;
...
ConfigSubstitutions ConfigBase::load_from_ini_string_commented(std::string &&data,
                                                               ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    // Convert the "data" string into INI format by removing the semi-colons at the start of a line.
    // Also the "; generated by preFlight ..." comment line will be removed.
    size_t j = 0;
...
size_t ConfigBase::load_from_gcode_string_legacy(ConfigBase &config, const char *str,
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &filename,
                                                     ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    // Read a 64k block from the end of the G-code.
    boost::nowide::ifstream ifs(filename, std::ifstream::binary);
    // Look for Slic3r, PrusaSlicer, or preFlight header.
    ...
    bool has_delimiters = false;
    {
        ...
        static constexpr const char legacy_gcode_header[] = "; generated by PrusaSlicer ";
        static constexpr const char preflight_gcode_header[] = "; generated by preFlight ";
        std::string header;
        ...
        while (std::getline(ifs, header))
        {
            ...
            else if (strncmp(legacy_gcode_header, header.c_str(), strlen(legacy_gcode_header)) == 0)
            {
                // Parse PrusaSlicer version (legacy).
                size_t i = strlen(legacy_gcode_header);
            ...
            else if (strncmp(preflight_gcode_header, header.c_str(), strlen(preflight_gcode_header)) == 0)
            {
                // Parse preFlight version - all versions have delimiters
                has_delimiters = true;
                header_found = true;
                break;
            }
        ...
        if (!header_found)
            throw Slic3r::RuntimeError("Not a preFlight / PrusaSlicer / Slic3r PE generated g-code.");
    }
    ...
    if (has_delimiters)
    {
        // preFlight and PrusaSlicer 2.4.0+ delimit the config section stored into G-code with
        // ; preflight_config = begin (or prusaslicer_config for legacy files)
        // ...
        // ; preflight_config = end (or prusaslicer_config for legacy files)
        ...
        while (reader.getline(line))
            if (line == "; preflight_config = end" || line == "; prusaslicer_config = end")
            {
                end_found = true;
                break;
            }
        if (!end_found)
            throw Slic3r::RuntimeError(
                format("Configuration block closing tag \"; preflight_config = end\" not found when reading %1%",
                       filename));
        std::string key, value;
        while (reader.getline(line))
        {
            if (line == "; preflight_config = begin" || line == "; prusaslicer_config = begin")
            {
                begin_found = true;
                break;
            }
        ...
        if (!begin_found)
            throw Slic3r::RuntimeError(
                format("Configuration block opening tag \"; preflight_config = begin\" not found when reading %1%",
                       filename));
    }
    else
    {
        // Slic3r or older PrusaSlicer do not emit any delimiter.
...
ConfigSubstitutions ConfigBase::load_from_binary_gcode_file(const std::string &filename,
                                                            ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ConfigSubstitutionContext substitutions_ctxt(compatibility_rule);
    ...
    if (res != EResult::Success)
        throw Slic3r::RuntimeError(
            format("Error while reading file %1%: %2%", filename, std::string(translate_result(res))));

    // searches for config block
    BlockHeader block_header;
    res = read_next_block_header(*file.f, file_header, block_header, EBlockType::SlicerMetadata, cs_buffer.data(),
                                 cs_buffer.size());
    if (res != EResult::Success)
        throw Slic3r::RuntimeError(
            format("Error while reading file %1%: %2%", filename, std::string(translate_result(res))));
...
bool DynamicConfig::equals(const DynamicConfig &other) const
{
    return !dynamic_config_iterate(*this, other,
                                   [](const t_config_option_key & /* key */, const ConfigOption *l,
                                      const ConfigOption *r) { return *l != *r; });
}
...
CEREAL_REGISTER_POLYMORPHIC_RELATION(Slic3r::ConfigOptionVector<Slic3r::FloatOrPercent>,
                                     Slic3r::ConfigOptionFloatsOrPercentsNullable)
