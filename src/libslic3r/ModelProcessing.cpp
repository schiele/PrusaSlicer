...
// Originally cycled through extruders (1, 2, 1, 2...) when loading multi-part
// objects. This is presumptuous - instead assign all to extruder 1 (primary) and let
// the user select which extruder each part should use.
...
void convert_to_multipart_object(Model &model, unsigned int /*max_extruders*/)
{
    ...
    for (const ModelObject *o : model.objects)
        for (const ModelVolume *v : o->volumes)
        {
            ...
            int counter = 1;
            auto copy_volume = [o, &counter](ModelVolume *new_v)
            {
                assert(new_v != nullptr);
                new_v->name = (counter > 1) ? o->name + "_" + std::to_string(counter++) : o->name;
                new_v->config.set("extruder", 1); // Always assign to primary extruder
                return new_v;
            };
...
TriangleMeshStats get_object_mesh_stats(const ModelObject *object)
{
    ...
    for (ModelVolume *volume : object->volumes)
    {
        ...
        {
            Transform3d trans = object->instances.empty() ? volume->get_matrix()
...
int get_repaired_errors_count(const ModelVolume *volume)
{
    ...
    return errors.degenerate_facets + errors.edges_fixed + errors.facets_removed + errors.facets_reversed +
