...
using MapMatrixXfUnaligned =
    Eigen::Map<const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor | Eigen::DontAlign>>;
using MapMatrixXiUnaligned =
    Eigen::Map<const Eigen::Matrix<int, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor | Eigen::DontAlign>>;
...
TriangleMesh eigen_to_triangle_mesh(const EigenMesh &emesh)
{
    ...
    for (Eigen::Index i = 0; i < FC.rows(); ++i)
        its.indices.emplace_back(FC.row(i).cast<coord_t>());
...
{
    EigenMesh emesh;
    emesh.first = MapMatrixXfUnaligned(mesh.its.vertices.front().data(), Eigen::Index(mesh.its.vertices.size()), 3)
                      .cast<double>();

    emesh.second.resize(mesh.its.indices.size(), 3);
    for (size_t i = 0; i < mesh.its.indices.size(); ++i)
    {
        emesh.second.row(i) << int(mesh.its.indices[i](0)), int(mesh.its.indices[i](1)), int(mesh.its.indices[i](2));
    }
    return emesh;
}
...
void triangle_mesh_to_cgal(const std::vector<stl_vertex> &V, const std::vector<stl_triangle_vertex_indices> &F,
                           _Mesh &out)
{
    if (F.empty())
        return;
...
void _cgal_do(Op &&op, CGALMesh &A, CGALMesh &B)
{
    ...
    try
    {
        CGALMesh result;
        try_catch_signal(
            {SIGSEGV, SIGFPE}, [&success, &A, &B, &result, &op] { success = op(A, B, result); },
            [&] { hw_fail = true; });
...
{
    _cgal_do(_cgal_diff, A, B);
}
void plus(CGALMesh &A, CGALMesh &B)
{
    _cgal_do(_cgal_union, A, B);
}
void intersect(CGALMesh &A, CGALMesh &B)
{
    _cgal_do(_cgal_intersection, A, B);
}
