...
bool Model::is_fdm_support_painted() const
{
    return std::any_of(this->objects.cbegin(), this->objects.cend(),
                       [](const ModelObject *mo) { return mo->is_fdm_support_painted(); });
}

bool Model::is_seam_painted() const
{
    return std::any_of(this->objects.cbegin(), this->objects.cend(),
                       [](const ModelObject *mo) { return mo->is_seam_painted(); });
}

bool Model::is_mm_painted() const
{
    return std::any_of(this->objects.cbegin(), this->objects.cend(),
                       [](const ModelObject *mo) { return mo->is_mm_painted(); });
}
...
ModelObject &ModelObject::assign_copy(const ModelObject &rhs)
{
    ...
    this->brim_points_status = rhs.brim_points_status;
...
ModelObject &ModelObject::assign_copy(ModelObject &&rhs)
{
    ...
    this->brim_points_status = std::move(rhs.brim_points_status);
...
bool ModelObject::is_fdm_support_painted() const
{
    return std::any_of(this->volumes.cbegin(), this->volumes.cend(),
                       [](const ModelVolume *mv) { return mv->is_fdm_support_painted(); });
}

bool ModelObject::is_seam_painted() const
{
    return std::any_of(this->volumes.cbegin(), this->volumes.cend(),
                       [](const ModelVolume *mv) { return mv->is_seam_painted(); });
}
...
void ModelObject::sort_volumes(bool full_sort)
{
    ...
    if (full_sort)
        std::stable_sort(volumes.begin(), volumes.end(),
                         [](ModelVolume *vl, ModelVolume *vr) { return vl->type() < vr->type(); });
    // sort have to controll "place" of the support blockers/enforcers. But one of the model parts have to be on the first place.
    else
        std::stable_sort(volumes.begin(), volumes.end(),
                         [](ModelVolume *vl, ModelVolume *vr)
                         {
                             ModelVolumeType vl_type = vl->type() > ModelVolumeType::PARAMETER_MODIFIER
                                                           ? vl->type()
                                                           : ModelVolumeType::PARAMETER_MODIFIER;
                             ModelVolumeType vr_type = vr->type() > ModelVolumeType::PARAMETER_MODIFIER
                                                           ? vr->type()
                                                           : ModelVolumeType::PARAMETER_MODIFIER;
...
Polygon ModelObject::convex_hull_2d(const Transform3d &trafo_instance) const
{
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, volumes.size()),
                      [&](const tbb::blocked_range<size_t> &range)
                      {
                          for (size_t i = range.begin(); i < range.end(); ++i)
                          {
                              ...
                              if (v->is_model_part())
                                  chs.emplace_back(
                                      its_convex_hull_2d_above(v->mesh().its,
                                                               (trafo_instance * v->get_matrix()).cast<float>(), 0.0f));
                          }
                      });
...
void ModelObject::bake_xy_rotation_into_meshes(size_t instance_idx)
{
    ...
    bool uniform_scaling =
        std::abs(reference_trafo.get_scaling_factor().x() - reference_trafo.get_scaling_factor().y()) < EPSILON &&
    ...
    for (size_t i = 0; i < this->instances.size(); ++i)
    {
        ...
        model_instance.set_rotation(
            Vec3d(0., 0., Geometry::rotation_diff_z(reference_trafo.get_matrix(), model_instance.get_matrix())));
    ...
    for (ModelVolume *model_volume : this->volumes)
    {
        ...
        bool volume_uniform_scaling =
            std::abs(volume_trafo.get_scaling_factor().x() - volume_trafo.get_scaling_factor().y()) < EPSILON &&
        ...
        model_volume->set_scaling_factor(
            Vec3d(volume_new_scaling_factor, volume_new_scaling_factor, volume_new_scaling_factor));
...
unsigned int ModelObject::update_instances_print_volume_state(const BuildVolume &build_volume)
{
    ...
    for (ModelInstance *model_instance : this->instances)
    {
        ...
        for (const ModelVolume *vol : this->volumes)
            if (vol->is_model_part())
            {
                ...
                BuildVolume::ObjectState state = build_volume.object_state(vol->mesh().its, matrix.cast<float>(),
                                                                           true /* may be below print bed */,
                                                                           true /*ignore_bottom*/, &bed);
        ...
        model_instance->print_volume_state = inside_outside == (INSIDE | OUTSIDE) ? ModelInstancePVS_Partly_Outside
...
void ModelVolume::center_geometry_after_creation(bool update_source_offset)
{
    ...
    if (!shift.isApprox(Vec3d::Zero()))
    {
        ...
        if (m_convex_hull)
            const_cast<TriangleMesh *>(m_convex_hull.get())
                ->translate(-(float) shift(0), -(float) shift(1), -(float) shift(2));
        translate(shift);
    }
...
void ModelVolume::rotate(double angle, const Vec3d &axis)
{
    set_rotation(get_rotation() +
                 Geometry::extract_rotation(Eigen::Quaterniond(Eigen::AngleAxisd(angle, axis)).toRotationMatrix()));
}
...
std::vector<size_t> ModelVolume::get_extruders_from_multi_material_painting() const
{
    ...
    for (size_t state_idx = static_cast<size_t>(TriangleStateType::Extruder1); state_idx < data.used_states.size();
         ++state_idx)
    {
...
std::string FacetsAnnotation::get_triangle_as_string(int triangle_idx) const
{
    ...
    auto triangle_it = std::lower_bound(m_data.triangles_to_split.begin(), m_data.triangles_to_split.end(),
                                        triangle_idx,
                                        [](const TriangleSelector::TriangleBitStreamMapping &l, const int r)
                                        { return l.triangle_idx < r; });
    if (triangle_it != m_data.triangles_to_split.end() && triangle_it->triangle_idx == triangle_idx)
    {
        ...
        int end = ++triangle_it == m_data.triangles_to_split.end() ? int(m_data.bitstream.size())
                                                                   : triangle_it->bitstream_start_idx;
        while (offset < end)
        {
            int next_code = 0;
            for (int i = 3; i >= 0; --i)
                               ...
                               const ModelVolumeType type)
{
    return model_volume_list_changed(model_object_old, model_object_new,
                                     [type](const ModelVolumeType t) { return t == type; });
}
...
bool model_property_changed(const ModelObject &model_object_old, const ModelObject &model_object_new,
                            TypeFilterFn type_filter, CompareFn compare)
{
...
bool model_custom_seam_data_changed(const ModelObject &mo, const ModelObject &mo_new)
{
    return model_property_changed(
        mo, mo_new, [](const ModelVolumeType t) { return t == ModelVolumeType::MODEL_PART; },
        [](const ModelVolume &mv_old, const ModelVolume &mv_new)
        { return mv_old.seam_facets.timestamp_matches(mv_new.seam_facets); });
}
...
bool model_mmu_segmentation_data_changed(const ModelObject &mo, const ModelObject &mo_new)
{
    return model_property_changed(
        mo, mo_new, [](const ModelVolumeType t) { return t == ModelVolumeType::MODEL_PART; },
        [](const ModelVolume &mv_old, const ModelVolume &mv_new)
        { return mv_old.mm_segmentation_facets.timestamp_matches(mv_new.mm_segmentation_facets); });
}
