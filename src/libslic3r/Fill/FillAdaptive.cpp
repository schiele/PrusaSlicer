...
struct CubeProperties
{
    ...
    double
        line_xy_distance; // Defines maximal distance from a center of a cube on X and Y axis on which lines will be created
};
...
struct Octree
{
    ...
    void insert_triangle(const Vec3d &a, const Vec3d &b, const Vec3d &c, Cube *current_cube,
                         const BoundingBoxf3 &current_bbox, int depth);
};
...
std::pair<double, double> adaptive_fill_line_spacing(const PrintObject &print_object)
{
    ...
    for (size_t region_id = 0; region_id < print_object.num_printing_regions(); ++region_id)
    {
        ...
        double infill_extrusion_width = config.infill_extrusion_width.percent
                                            ? default_infill_extrusion_width * 0.01 * config.infill_extrusion_width
                                            : config.infill_extrusion_width;
        region_fill_data.push_back(
            RegionFillData({has_adaptive_infill ? Tristate::Maybe : Tristate::No,
                            has_support_infill ? Tristate::Maybe : Tristate::No, config.fill_density,
...
struct FillContext
{
        ...
        , sin_a(sin(direction_angles[direction_idx]))
    {
        ...
        temp_lines.assign((1 << octree.cubes_properties.size()) - 1,
                          Line(Point(unused, unused), Point(unused, unused)));
    }

    // Rotate the point, uses the same convention as Point::rotate().
    Vec2d rotate(const Vec2d &v)
    {
        return Vec2d(this->cos_a * v.x() - this->sin_a * v.y(), this->sin_a * v.x() + this->cos_a * v.y());
    }
...
static bool verify_traversal_order(FillContext &context, const Cube *cube, int depth, const Vec2d &line_from,
                                   const Vec2d &line_to)
{
    std::array<Vec3d, 8> c;
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        Vec3d cntr = to_world *
                     (cube->center_octree + (child_centers[j] * (context.cubes_properties[depth].edge_length / 4.)));
                                            ...
                                            int address, int depth)
{
    ...
    if (z_diff_abs < cubes_properties[depth].line_z_distance)
    {
        ...
        else if ((new_line.a - last_line.b).cwiseAbs().maxCoeff() > 1000)
...
struct Intersection
{
    ...
    Intersection(const Line &closest_line, const Line &intersect_line, Polyline *intersect_pl,
                 const Point &intersect_point, bool front)
    ...
    {
        return (this->intersect_point == intersect_line->a ? intersect_line->b - intersect_line->a
                                                           : intersect_line->a - intersect_line->b)
            .cast<double>();
    }
};

static inline Intersection *get_nearest_intersection(std::vector<std::pair<Intersection *, double>> &intersect_line,
...
{
    offset_line.translate((perp(intersection.closest_line->vector().cast<double>().normalized()) *
                           (intersection.left ? scaled_offset : -scaled_offset))
                              .cast<coord_t>());
...
static void add_hook(const Intersection &intersection, const double scaled_offset, const coordf_t hook_length,
                     double scaled_trim_distance, const rtree_t &rtree, const Lines &lines_src)
{
    ...
    [[maybe_unused]] bool intersection_found = intersection.intersect_line->intersection(
        create_offset_line(*intersection.closest_line, intersection, scaled_offset), &hook_start);
    ...
    auto max_hook_length = [hook_length, scaled_trim_distance,
                            &lines_src](const Vec2d &pt, const Vec2d &dir,
                                        ...
                                        bool self_intersection, const std::optional<Line> &self_intersection_line,
                                        const Point &self_intersection_point)
    {
    ...
    double hook_forward_max_length = max_hook_length(hook_startf, hook_vector_norm, hook_intersections,
                                                     self_intersection, other_hook, self_intersection_point);
    ...
    if (hook_forward_max_length < hook_length - SCALED_EPSILON)
    {
        ...
        hook_backward_max_length = max_hook_length(hook_startf, -hook_vector_norm, hook_intersections,
                                                   self_intersection, other_hook, self_intersection_point);
    }

    // Take the longer hook.
    Vec2d hook_dir = (hook_forward_max_length > hook_backward_max_length ? hook_forward_max_length
...
static Polylines connect_lines_using_hooks(Polylines &&lines, const ExPolygon &boundary, const double spacing,
                                           const coordf_t hook_length, const coordf_t hook_length_max)
{
    rtree_t rtree;
    size_t poly_idx = 0;
            ...
            if (&poly != lines.data())
            {
                                                    ...
                                                    const Polyline *polyline) -> std::pair<Polyline *, bool>
                {
                    ...
                    if (dist2_min < r2_close)
                    {
                        ...
                        double dist2_min_other =
                            std::min((other->points.front() - pt_other).cast<double>().squaredNorm(),
                ...
                assert(!collinear_front.first || !collinear_back.first ||
                       collinear_front.first != collinear_back.first);
        ...
        for (size_t line_idx = 0; line_idx < lines.size(); ++line_idx)
        {
            ...
            std::optional<size_t> tjoint_front, tjoint_back;
            {
                auto has_tjoint = [&closest, line_idx, &rtree, &lines, &lines_src](const Point &pt)
                {
                    ...
                    closest.clear();
                    rtree.query(bgi::nearest(mk_rtree_point(pt), 1) && bgi::satisfies(filter_t_joint),
                ...
                auto filter_end_point_connections =
                    [&lines_touching_at_endpoints, &lines, &line](std::optional<size_t> in)
                {
                    std::optional<size_t> out;
                    if (in)
                    {
            ...
            if (num_tjoints > 0)
            {
                ...
                else if (anchor)
                {
                    if (tjoint_front)
                    {
                        ...
                        intersections.emplace_back(lines_src[*tjoint_front], lines_src[line_idx], &line, front_point,
                                                   true);
                    ...
                    if (tjoint_back)
                    {
                        ...
                        intersections.emplace_back(lines_src[*tjoint_back], lines_src[line_idx], &line, back_point,
                                                   false);
                        assert(validate_intersection_t_joint(intersections.back()));
                    }
                }
                else
                {
                    if (tjoint_front)
                        ...
                        // Trim the front side.
                        front_point += ((scaled_trim_distance * 1.155) *
                                        (back_point - front_point).cast<double>().normalized())
                                           .cast<coord_t>();
                    if (tjoint_back)
                        // T joint at the front at a 60 degree angle, the line is very short.
                        // Trim the front side.
                        back_point += ((scaled_trim_distance * 1.155) *
                                       (front_point - back_point).cast<double>().normalized())
                                          .cast<coord_t>();
                }
            }
        }
    ...
    std::sort(intersections.begin(), intersections.end(),
              ...
              {
                  return (i1.closest_line == i2.closest_line) ? int(i1.left) < int(i2.left)
    ...
    for (size_t min_idx = 0; min_idx < intersections.size();)
    {
        ...
        // All the nearest points (T-joints) ending at the same line are projected onto this line. Because of it, it can easily find the nearest point.
        {
            ...
            for (; max_idx < intersections.size() &&
                 ...
                 ++max_idx)
                intersect_line.emplace_back(&intersections[max_idx],
                                            line_dir.dot(intersections[max_idx].intersect_point.cast<double>()));
            min_idx = max_idx;
            assert(intersect_line.size() > 0);
            // Sort the intersections along line_dir.
            std::sort(intersect_line.begin(), intersect_line.end(),
...
#ifdef ADAPTIVE_CUBIC_INFILL_DEBUG_OUTPUT
                export_infill_lines_to_svg(boundary, lines,
                                           debug_out_path("FillAdaptive-add_hook0-pre-%d-%d.svg", iRun, iStep),
        ...
        for (size_t first_idx = 0; first_idx < intersect_line.size(); ++first_idx)
        {
            ...
            Line offset_line = create_offset_line(Line(first_i.intersect_point, nearest_i.intersect_point), first_i,
                                                  scaled_offset);
            ...
            could_connect &= (nearest_i_point - first_i_point).cast<double>().squaredNorm() <=
            ...
            if (could_connect)
            {
                ...
                rtree.query(bgi::intersects(mk_rtree_seg(first_i_point, nearest_i_point)) &&
                                bgi::satisfies(
                                    [&first_i, &nearest_i, &lines_src](const auto &item)
                                    {
                                        return item.second !=
                                                   (long unsigned int) (first_i.intersect_line - lines_src.data()) &&
                                               item.second !=
                                                   (long unsigned int) (nearest_i.intersect_line - lines_src.data());
                                    }),
                            std::back_inserter(closest));
            ...
            if (could_connect)
            {
#ifdef ADAPTIVE_CUBIC_INFILL_DEBUG_OUTPUT
                export_infill_lines_to_svg(boundary, lines,
                                           debug_out_path("FillAdaptive-connecting-pre-%d-%d.svg", iRun, iStep),
                ...
                else
                {
                    ...
                    bool trim_start = first_i.intersect_pl->points.size() == 3 &&
                                      first_i.other_hook_intersects(l, pt_start);
                    bool trim_end = nearest_i.intersect_pl->points.size() == 3 &&
                                    nearest_i.other_hook_intersects(l, pt_end);
...
#ifdef ADAPTIVE_CUBIC_INFILL_DEBUG_OUTPUT
                export_infill_lines_to_svg(boundary, lines,
                                           debug_out_path("FillAdaptive-connecting-post-%d-%d.svg", iRun, iStep),
                                           {first_i.intersect_point, nearest_i.intersect_point});
#endif // ADAPTIVE_CUBIC_INFILL_DEBUG_OUTPUT
            }
            if (!connected)
...
#ifdef ADAPTIVE_CUBIC_INFILL_DEBUG_OUTPUT
                export_infill_lines_to_svg(boundary, lines,
                                           debug_out_path("FillAdaptive-add_hook-pre-%d-%d.svg", iRun, iStep),
                                           {first_i.intersect_point});
#endif // ADAPTIVE_CUBIC_INFILL_DEBUG_OUTPUT
                add_hook(first_i, scaled_offset, hook_length, scaled_trim_distance, rtree, lines_src);
#ifdef ADAPTIVE_CUBIC_INFILL_DEBUG_OUTPUT
                export_infill_lines_to_svg(boundary, lines,
                                           debug_out_path("FillAdaptive-add_hook-post-%d-%d.svg", iRun, iStep),
                                           {first_i.intersect_point});
#endif // ADAPTIVE_CUBIC_INFILL_DEBUG_OUTPUT
            }
#ifdef ADAPTIVE_CUBIC_INFILL_DEBUG_OUTPUT
    ...
    polylines_out.reserve(polylines_out.size() +
                          std::count_if(lines.begin(), lines.end(), [](const Polyline &pl) { return !pl.empty(); }));
...
bool has_no_collinear_lines(const Polylines &polylines)
{
    ...
    for (const Polyline &pl : polylines)
    {
        ...
        {
            std::vector<std::pair<const LineEnd *, double>> hits = closest_end_point_lookup.find_all(
                line_start.point());
...
void Filler::_fill_surface_single(const FillParams &params, unsigned int thickness_layers,
                                  ...
                                  Polylines &polylines_out)
{
    ...
    Polylines all_polylines;
    {
        ...
        std::array<FillContext, 3> contexts{FillContext{*adapt_fill_octree, this->z, 0},
        ...
        {
            generate_infill_lines_recursive(context, adapt_fill_octree->root_cube, 0,
                                            int(adapt_fill_octree->cubes_properties.size()) - 1);
...
//        assert(has_no_collinear_lines(all_polylines));
    ...
    const auto hook_length = coordf_t(
        std::min<float>(std::numeric_limits<coord_t>::max(), scale_(params.anchor_length)));
    ...
    {
        static int iRun = 0;
        export_infill_lines_to_svg(expolygon, all_polylines_with_hooks,
                                   debug_out_path("FillAdaptive-hooks-%d.svg", iRun++));
    }
#endif /* ADAPTIVE_CUBIC_INFILL_DEBUG_OUTPUT */
    ...
    if (cubes_properties.size() > 1)
    {
        ...
        auto process_triangle = [octree_ptr, max_depth, diag_half](const Vec3d &a, const Vec3d &b, const Vec3d &c)
        {
            octree_ptr->insert_triangle(a, b, c, octree_ptr->root_cube,
                                        BoundingBoxf3(octree_ptr->root_cube->center - diag_half,
                                                      octree_ptr->root_cube->center + diag_half),
                                        max_depth);
        };
...
void Octree::insert_triangle(const Vec3d &a, const Vec3d &b, const Vec3d &c, Cube *current_cube,
                             const BoundingBoxf3 &current_bbox, int depth)
{
    assert(current_cube);
    assert(depth > 0);
    ...
    for (size_t i = 0; i < 8; ++i)
    {
        ...
        Vec3d child_center = current_cube->center +
                             (child_center_dir * (this->cubes_properties[depth].edge_length / 2.));
