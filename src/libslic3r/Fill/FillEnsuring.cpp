...
static std::vector<Lines> filter_vibrating_extrusions(const std::vector<Lines> &lines_sections)
{
    ...
    for (size_t initial_line_section_idx = 0; initial_line_section_idx < line_nodes_sections.size();
         ++initial_line_section_idx)
    {
        ...
        for (size_t propagation_line_section_idx = initial_line_section_idx;
             propagation_line_section_idx < line_nodes_sections.size(); ++propagation_line_section_idx)
        {
            ...
            for (LineNode &propagation_line : line_nodes_sections[propagation_line_section_idx])
            {
                ...
                for (LineNode *neighbour_line : propagation_line.next_section_overlapping_lines)
                {
                    ...
                    const int neighbour_total_short_lines = propagation_line.state.total_short_lines +
                                                            int(is_short_line);
                    ...
                    if (neighbour_line->state.initialized)
                    {
                        ...
                        neighbour_line->state.min_skips_taken = std::max(neighbour_line->state.min_skips_taken,
                                                                         neighbour_total_short_lines);
                        neighbour_line->state.min_skips_taken = std::min(neighbour_line->state.min_skips_taken,
                                                                         neighbour_min_skips_taken);
                        ...
                        {
                            neighbour_line->state.initial_touches_long_lines =
                                propagation_line.state.initial_touches_long_lines;
                        }
                    }
                    else
                    {
...
ThickPolylines make_fill_polylines(const Fill *fill, const Surface *surface, const FillParams &params,
                                   bool stop_vibrations, bool fill_gaps, bool connect_extrusions)
{
    ...
    Polygons inner_area = stop_vibrations
                              ? intersection(filled_area, opening(filled_area, 2 * scaled_spacing, 3 * scaled_spacing))
                              : filled_area;
    ...
    if (!vertical_lines.empty())
    {
        ...
        vertical_lines.back().a = Point{coord_t(bb.min.x() + n_vlines * double(scaled_spacing) + scaled_spacing * 0.5),
                                        y_min};
    ...
    if (fill_gaps)
    {
            ...
            for (const auto &polygon_slice : polygon_sections_w_width)
            {
                ...
                for (TracedPoly &traced_poly : current_traced_polys)
                {
                    ...
                    for (auto candidate = candidates_begin; candidate != candidates_end && !segment_added; candidate++)
                    {
                        ...
                        if (connect_extrusions &&
                            (traced_poly.lows.back() - candidates_begin->a).cast<double>().squaredNorm() <
                        ...
                        else
                        {
                            ...
                            traced_poly.lows.push_back(candidates_begin->a - Point(scaled_spacing / 2, coord_t(0)));
                            traced_poly.lows.push_back(candidates_begin->a);
                        }

                        if (connect_extrusions &&
                            (traced_poly.highs.back() - candidates_begin->b).cast<double>().squaredNorm() <
                                squared_distance_limit_reconnection)
                        {
                        ...
                        else
                        {
                            traced_poly.highs.push_back(traced_poly.highs.back() +
                                                        Point(scaled_spacing / 2, coord_t(0)));
                            traced_poly.highs.push_back(candidates_begin->b - Point(scaled_spacing / 2, coord_t(0)));
                            traced_poly.highs.push_back(candidates_begin->b);
                        }
                        segment_added = true;
                    ...
                    if (!segment_added)
                    {
                        ...
                        traced_poly.lows.push_back(traced_poly.lows.back() + Point(scaled_spacing / 2, coord_t(0)));
                        traced_poly.highs.push_back(traced_poly.highs.back() + Point(scaled_spacing / 2, coord_t(0)));
                ...
                for (const auto &segment : polygon_slice)
                {
                    if (used_segments.find(&segment) == used_segments.end())
                    {
                        ...
                        new_tp.lows.push_back(segment.a - Point(scaled_spacing / 2, coord_t(0)));
                        new_tp.lows.push_back(segment.a);
                        new_tp.highs.push_back(segment.b - Point(scaled_spacing / 2, coord_t(0)));
        ...
        std::sort(thick_polylines.begin(), thick_polylines.end(),
                  [](const ThickPolyline &left, const ThickPolyline &right)
                  {
        ...
        for (size_t pl_idx = 0; pl_idx < thick_polylines.size(); pl_idx++)
        {
            ...
            connection_endpoints.push_back(
                {thick_polylines[pl_idx].first_point().cast<double>(), pl_idx, current_idx + 1, true});
            connection_endpoints.push_back(
                {thick_polylines[pl_idx].last_point().cast<double>(), pl_idx, current_idx, false});
        }
        ...
        auto coord_fn = [&connection_endpoints](size_t idx, size_t dim)
        {
            return connection_endpoints[idx].position[dim];
        };
        ...
        for (size_t ep_idx = 0; ep_idx < connection_endpoints.size(); ep_idx++)
        {
            ...
            if (!ep1.used)
            {
                std::vector<size_t> close_endpoints = find_nearby_points(endpoints_tree, ep1.position,
                                                                         double(scaled_spacing));
        ...
        thick_polylines.erase(std::remove_if(thick_polylines.begin(), thick_polylines.end(),
                                             [scaled_spacing](const ThickPolyline &tp)
                                             {
                                                 return tp.length() < scaled_spacing &&
                                                        std::all_of(
                                                            tp.width.begin(), tp.width.end(),
                                                            [scaled_spacing](double w) { return w < scaled_spacing; });
                                             }),
                              thick_polylines.end());
    }

    Algorithm::sort_paths(thick_polylines.begin(), thick_polylines.end(), bb.min, double(scaled_spacing) * 1.2,
    ...
    if (connect_extrusions)
    {
        ...
        if (!thick_polylines.empty())
        {
            ...
            for (size_t tp_idx = 1; tp_idx < thick_polylines.size(); tp_idx++)
            {
                ...
                if ((last - tp.last_point()).cast<double>().squaredNorm() <
                    (last - tp.first_point()).cast<double>().squaredNorm())
                {
                    tp.reverse();
                }
