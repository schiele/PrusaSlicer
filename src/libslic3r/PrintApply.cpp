...
static inline void model_volume_list_copy_configs(ModelObject &model_object_dst, const ModelObject &model_object_src,
                                                                                 ...
                                                                                 const Vec3d &shrinkage_compensation)
{
    ...
    for (ModelInstance *model_instance : model_object.instances)
        if (model_instance->is_printable())
        {
            ...
            trafo.trafo = model_instance_transformation.get_matrix_with_applied_shrinkage_compensation(
            ...
            const_cast<PrintObjectTrafoAndInstances &>(*it).instances.emplace_back(
                PrintInstance{nullptr, model_instance, shift});
        }
...
static bool layer_height_ranges_equal(const t_layer_config_ranges &lr1, const t_layer_config_ranges &lr2,
                                      bool check_layer_height)
{
    ...
    for (const auto &kvp1 : lr1)
    {
        ...
        if (std::abs(kvp1.first.first - kvp2.first.first) > EPSILON ||
            ...
            (check_layer_height && std::abs(kvp1.second.option("layer_height")->getFloat() -
                                            kvp2.second.option("layer_height")->getFloat()) > EPSILON))
            return false;
    }
    return true;
}
...
static bool custom_per_printz_gcodes_tool_changes_differ(const std::vector<CustomGCode::Item> &va,
                                               ...
                                               DynamicPrintConfig &filament_overrides)
{
    ...
    for (const t_config_option_key &opt_key : current_config.keys())
    {
        ...
        if (opt_new == nullptr)
            ...
            continue;
        const ConfigOption *opt_new_filament = std::binary_search(extruder_retract_keys.begin(),
                                                                  extruder_retract_keys.end(), opt_key)
...
public:
    struct LayerRange
    {
        ...
        {
            return this->layer_height_range < rhs.layer_height_range;
        }
    };
    ...
    {
        auto it = std::lower_bound(m_ranges.begin(), m_ranges.end(),
                                   LayerRange{{range.first - EPSILON, range.second - EPSILON}});
        // #ys_FIXME_COLOR
...
public:
    ...
    struct iterator_range : std::pair<const_iterator, const_iterator>
    {
        ...
        iterator_range(const std::pair<const_iterator, const_iterator> in)
            : std::pair<const_iterator, const_iterator>(in)
        {
        }
...
static PrintObjectRegions::BoundingBox transformed_its_bbox2d(const indexed_triangle_set &its, const Transform3f &m,
                                                              float offset)
{
...
static void transformed_its_bboxes_in_z_ranges(const indexed_triangle_set &its, const Transform3f &m,
...
void print_objects_regions_invalidate_keep_some_volumes(PrintObjectRegions &print_object_regions,
                                                        ModelVolumePtrs old_volumes, ModelVolumePtrs new_volumes)
{
    ...
    for (size_t i_new = 0; i_new < new_volumes.size(); ++i_new)
        if (model_volume_solid_or_modifier(*new_volumes[i_new]))
        {
            ...
            if (i_old != old_volumes.size() && old_volumes[i_old]->id() == new_volumes[i_new]->id())
            {
                ...
                {
                    // Reuse the volume.
                    for (; print_object_regions.cached_volume_ids[i_cached_volume] < old_volumes[i_old]->id();
                         ++i_cached_volume)
                    ...
                    print_object_regions.cached_volume_ids[last_cached_volume++] =
                        print_object_regions.cached_volume_ids[i_cached_volume++];
                }
                else
                {
                                                           ...
                                                           const ModelVolume &volume)
{
    auto it = lower_bound_by_predicate(layer_range.volumes.begin(), layer_range.volumes.end(),
                                       [&volume](const PrintObjectRegions::VolumeExtents &l)
                                       { return l.volume_id < volume.id(); });
                                                             ...
                                                             const int this_region_id)
{
    ...
    if (!this_region.model_volume->is_model_part())
        for (int parent_region_id = this_region.parent;;)
        {
            ...
            const PrintObjectRegions::BoundingBox *parent_extents = find_volume_extents(layer_range,
                                                                                        *parent_region.model_volume);
...
bool verify_update_print_object_regions(ModelVolumePtrs model_volumes, const PrintRegionConfig &default_region_config,
                                        size_t num_extruders, PrintObjectRegions &print_object_regions,
                                        const std::function<void(const PrintRegionConfig &, const PrintRegionConfig &,
                                                                 const t_config_option_keys &)> &callback_invalidate)
{
    ...
    for (PrintObjectRegions::LayerRangeRegions &layer_range : print_object_regions.layer_ranges)
    {
        ...
        for (PrintObjectRegions::VolumeRegion &region : layer_range.volume_regions)
            ...
            {
                auto it_model_volume = lower_bound_by_predicate(model_volumes.begin(), model_volumes.end(),
                                                                [&region](const ModelVolume *l)
                ...
                if (region.model_volume->is_modifier() && it_model_volume != it_model_volume_modifier_last)
                {
                    ...
                    const PrintObjectRegions::BoundingBox *bbox = find_volume_extents(layer_range,
                                                                                      *region.model_volume);
                    assert(bbox);
                    for (int parent_region_id = next_region_id - 1; parent_region_id >= 0; --parent_region_id)
                    {
                        const PrintObjectRegions::VolumeRegion &parent_region =
                            layer_range.volume_regions[parent_region_id];
                        ...
                        if (parent_region.model_volume->is_model_part() || parent_region.model_volume->is_modifier())
                        {
                                ...
                                layer_range.volume_regions[next_region_id].parent == parent_region_id)
                            {
                                ...
                                ++next_region_id;
                                         ...
                                         find_modifier_volume_extents(layer_range, parent_region_id);
                                ...
                                if (PrintRegionConfig config = region_config_from_model_volume(
                                        parent_region.region->config(), nullptr, **it_model_volume, num_extruders);
    ...
    for (const PrintObjectRegions::LayerRangeRegions &layer_range : print_object_regions.layer_ranges)
    {
        for (const PrintObjectRegions::FuzzySkinPaintedRegion &region : layer_range.fuzzy_skin_painted_regions)
        {
            ...
            // AllWalls triggers the "all perimeters" optimization, External does not
            cfg.fuzzy_skin.value = cfg.fuzzy_skin_painted_perimeters.value == FuzzySkinPaintedPerimeters::All
...
void update_volume_bboxes(std::vector<PrintObjectRegions::LayerRangeRegions> &layer_ranges,
                          std::vector<ObjectID> &cached_volume_ids, ModelVolumePtrs model_volumes,
                          const Transform3d &object_trafo, const float offset)
{
            ...
            if (model_volume_solid_or_modifier(*model_volume))
            {
                if (std::binary_search(cached_volume_ids.begin(), cached_volume_ids.end(), model_volume->id()))
                {
                    ...
                    layer_range.volumes.push_back(
                        {model_volume->id(),
                         transformed_its_bbox2d(model_volume->mesh().its,
    ...
    else
    {
        ...
        for (const ModelVolume *model_volume : model_volumes)
            if (model_volume_solid_or_modifier(*model_volume))
            {
                ...
                {
                    transformed_its_bboxes_in_z_ranges(model_volume->mesh().its,
                                                       trafo_for_bbox(object_trafo, model_volume->get_matrix()), ranges,
                                                       bboxes, offset);
                    for (PrintObjectRegions::LayerRangeRegions &layer_range : layer_ranges)
...
static PrintObjectRegions *generate_print_object_regions(
    PrintObjectRegions *print_object_regions_old, const ModelVolumePtrs &model_volumes,
    const LayerRanges &model_layer_ranges, const PrintRegionConfig &default_region_config, const Transform3d &trafo,
    size_t num_extruders, const float xy_size_compensation, const std::vector<unsigned int> &painting_extruders,
    const bool has_painted_fuzzy_skin)
{
    ...
    update_volume_bboxes(layer_ranges_regions, out->cached_volume_ids, model_volumes, out->trafo_bboxes,
                         is_mm_painted ? 0.f : std::max(0.f, xy_size_compensation));
    ...
    auto get_create_region = [&region_set, &all_regions](PrintRegionConfig &&config) -> PrintRegion *
    {
        ...
        auto it = Slic3r::lower_bound_by_predicate(
            region_set.begin(), region_set.end(), [&config, hash](const PrintRegion *l)
                ...
                if (const PrintObjectRegions::BoundingBox *bbox = find_volume_extents(layer_range, volume); bbox)
                {
                    if (volume.is_model_part())
                    {
                        ...
                        layer_range.volume_regions.push_back(
                            {&volume, -1,
                             get_create_region(region_config_from_model_volume(
                                 default_region_config, layer_range.config, volume, num_extruders)),
                             bbox});
                    }
                    ...
                    else
                    {
                        ...
                        for (int parent_region_id = int(layer_range.volume_regions.size()) - 1; parent_region_id >= 0;
                             --parent_region_id)
                        {
                            const PrintObjectRegions::VolumeRegion &parent_region =
                                layer_range.volume_regions[parent_region_id];
                            ...
                            if (parent_volume.is_model_part() || parent_volume.is_modifier())
                                if (PrintObjectRegions::BoundingBox parent_bbox =
                                        find_modifier_volume_extents(layer_range, parent_region_id);
                                    parent_bbox.intersects(*bbox))
                                {
                                    // Only create new region for a modifier, which actually modifies config of it's parent.
                                    if (PrintRegionConfig config = region_config_from_model_volume(
                                            parent_region.region->config(), nullptr, volume, num_extruders);
                                        config != parent_region.region->config())
                                    {
                                        added = true;
                                        layer_range.volume_regions.push_back(
                        ...
                        if (!added && parent_model_part_id >= 0)
                            ...
                            layer_range.volume_regions.push_back(
                                {&volume, parent_model_part_id, layer_range.volume_regions[parent_model_part_id].region,
                                 bbox});
                    }
                }
        }
    ...
    for (PrintObjectRegions::LayerRangeRegions &layer_range : layer_ranges_regions)
    {
        for (unsigned int painted_extruder_id : painting_extruders)
            for (int parent_region_id = 0; parent_region_id < int(layer_range.volume_regions.size());
                 ++parent_region_id)
                    ...
                    parent_region.model_volume->is_model_part() || parent_region.model_volume->is_modifier())
                {
                    ...
                    layer_range.painted_regions.push_back(
                        {painted_extruder_id, parent_region_id, get_create_region(std::move(cfg))});
                }
        ...
        std::sort(layer_range.painted_regions.begin(), layer_range.painted_regions.end(),
    ...
    if (has_painted_fuzzy_skin)
    {
        ...
        for (PrintObjectRegions::LayerRangeRegions &layer_range : layer_ranges_regions)
        {
            ...
            {
                if (const PrintObjectRegions::VolumeRegion &parent_volume_region =
                        layer_range.volume_regions[parent_volume_region_id];
                    parent_volume_region.model_volume->is_model_part() ||
                    parent_volume_region.model_volume->is_modifier())
                {
                    PrintRegionConfig cfg = parent_volume_region.region->config();
                    cfg.fuzzy_skin.value = cfg.fuzzy_skin_painted_perimeters.value == FuzzySkinPaintedPerimeters::All
                                               ? FuzzySkinType::AllWalls
                                               : FuzzySkinType::External;
                    layer_range.fuzzy_skin_painted_regions.push_back({FuzzySkinParentType::VolumeRegion,
                                                                      parent_volume_region_id,
                                                                      get_create_region(std::move(cfg))});
                }
            }
            ...
            {
                const PrintObjectRegions::PaintedRegion &parent_painted_region =
                    layer_range.painted_regions[parent_painted_regions_id];
            ...
            std::sort(layer_range.fuzzy_skin_painted_regions.begin(), layer_range.fuzzy_skin_painted_regions.end(),
                      [&layer_range](auto &l, auto &r)
                      {
                          return l.parent_print_object_region_id(layer_range) <
                                 r.parent_print_object_region_id(layer_range);
                      });
        }
    }
...
static void validate_print_config_change(const PrintConfig &old_config, const DynamicPrintConfig &new_config,
                                         std::vector<std::string> *warnings)
{
    ...
    if (old_config.bed_temperature_extruder > 0 &&
        old_config.bed_temperature_extruder == new_config.option("bed_temperature_extruder")->getInt())
    {
        ...
        if (old_config.bed_temperature.values != new_config.option("bed_temperature")->getInts() ||
            old_config.first_layer_bed_temperature.values !=
                new_config.option("first_layer_bed_temperature")->getInts())
        {
                                ...
                                std::vector<std::string> *warnings)
{
    ...
    {
        apply_status = std::max<unsigned int>(apply_status,
                                              invalidated ? APPLY_STATUS_INVALIDATED : APPLY_STATUS_CHANGED);
    };
    ...
    if (m_force_invalidation)
    {
        m_force_invalidation = false; // Reset flag
    ...
    if (!full_config_diff.empty())
    {
        ...
        m_placeholder_parser.set("physical_printer_preset",
        ...
        try
        {
            DynamicConfig custom_pars = parse_custom_parameters_to_dynamic_config(
                m_config.custom_parameters_print, m_config.custom_parameters_printer,
    ...
    else
    {
        if (m_model.custom_gcode_per_print_z() != model.custom_gcode_per_print_z())
        {
            ...
            const bool multi_extruder_differ = (current_mode == next_mode) &&
                                               (current_mode == CustomGCode::MultiExtruder ||
                                                next_mode == CustomGCode::MultiExtruder);
            ...
            const bool tool_change_differ = num_extruders > 1 &&
                                            custom_per_printz_gcodes_tool_changes_differ(
                                                m_model.custom_gcode_per_print_z().gcodes,
                                                model.custom_gcode_per_print_z().gcodes, CustomGCode::ToolChange);
            ...
            const bool color_change_differ = num_extruders > 1 && (next_mode == CustomGCode::MultiExtruder) &&
                                             custom_per_printz_gcodes_tool_changes_differ(
                                                 m_model.custom_gcode_per_print_z().gcodes,
                                                 model.custom_gcode_per_print_z().gcodes, CustomGCode::ColorChange);
        ...
        else
        {
            ...
            auto by_id_lower = [](const ModelObject *lhs, const ModelObject *rhs)
            {
    ...
    const std::initializer_list<ModelVolumeType> solid_or_modifier_types{ModelVolumeType::MODEL_PART,
                                                                         ModelVolumeType::NEGATIVE_VOLUME,
                                                                         ModelVolumeType::PARAMETER_MODIFIER};
    for (size_t idx_model_object = 0; idx_model_object < model.objects.size(); ++idx_model_object)
    {
        ModelObject &model_object = *m_model.objects[idx_model_object];
        ModelObjectStatus &model_object_status = const_cast<ModelObjectStatus &>(
            model_object_status_db.reuse(model_object));
        const ModelObject &model_object_new = *model.objects[idx_model_object];
        if (model_object_status.status == ModelObjectStatus::New)
            // PrintObject instances will be added in the next loop.
            continue;
        // Update the ModelObject instance, possibly invalidate the linked PrintObjects.
        assert(model_object_status.status == ModelObjectStatus::Old ||
               model_object_status.status == ModelObjectStatus::Moved);
        // Check whether a model part volume was added or removed, their transformations or order changed.
        // Only volume IDs, volume types, transformation matrices and their order are checked, configuration and other parameters are NOT checked.
        bool solid_or_modifier_differ = model_volume_list_changed(model_object, model_object_new,
        ...
        bool layer_height_ranges_differ = !layer_height_ranges_equal(model_object.layer_config_ranges,
            ...
            !model_object.layer_height_profile.timestamp_matches(model_object_new.layer_height_profile))
        {
            ...
            model_object_status.print_object_regions_status =
                model_object_status.print_object_regions == nullptr || model_origin_translation_differ ||
            ...
            if (model_object_status.print_object_regions_status ==
                ModelObjectStatus::PrintObjectRegionsStatus::PartiallyValid)
                // Drop everything from PrintObjectRegions but those VolumeExtents (of their particular ModelVolumes) that are still valid.
                print_objects_regions_invalidate_keep_some_volumes(*model_object_status.print_object_regions,
                                                                   model_object.volumes, model_object_new.volumes);
        ...
        else
        {
            ...
            if (brim_points_differ)
            {
                ...
                // Invalidate support material step to regenerate clean support_fills
                // Then invalidate skirt/brim step to regenerate brim and re-trim support with new brim geometry
                for (const PrintObjectStatus &print_object_status : print_objects_range)
                {
                    update_apply_status(print_object_status.print_object->invalidate_step(posSupportMaterial));
                }
        ...
        if (!solid_or_modifier_differ)
        {
            ...
            {
                PrintObjectConfig new_config = PrintObject::object_config_from_model_object(m_default_object_config,
                                                                                            model_object,
                                                                                            num_extruders);
                for (const PrintObjectStatus &print_object_status : print_object_status_db.get_range(model_object))
                {
                    ...
                    if (!diff.empty())
                    {
                        update_apply_status(print_object_status.print_object->invalidate_state_by_config_options(
                            print_object_status.print_object->config(), new_config, diff));
            ...
            model_volume_list_copy_configs(model_object /* dst */, model_object_new /* src */,
                                           ModelVolumeType::MODEL_PART);
            model_volume_list_copy_configs(model_object /* dst */, model_object_new /* src */,
                                           ModelVolumeType::PARAMETER_MODIFIER);
            layer_height_ranges_copy_configs(model_object.layer_config_ranges /* dst */,
                                             model_object_new.layer_config_ranges /* src */);
            ...
            if (model_object.instances.size() != model_object_new.instances.size() ||
                !std::equal(model_object.instances.begin(), model_object.instances.end(),
                            model_object_new.instances.begin(), [](auto l, auto r) { return l->id() == r->id(); }))
            {
            ...
            else if (!std::equal(model_object.instances.begin(), model_object.instances.end(),
                                 model_object_new.instances.begin(),
                                 [](auto l, auto r)
                                 {
                                     return l->print_volume_state == r->print_volume_state &&
                                            l->printable == r->printable &&
                                            l->get_transformation().get_matrix().isApprox(
                                                r->get_transformation().get_matrix());
                                 }))
            {
    ...
    // 4) Generate PrintObjects from ModelObjects and their instances.
    {
        ...
        for (ModelObject *model_object : m_model.objects)
        {
            ...
            auto print_object_apply_config =
                [this, &print_object_last, model_object, num_extruders](PrintObject *print_object)
            {
            ...
            std::sort(old.begin(), old.end(), [](const PrintObjectStatus *lhs, const PrintObjectStatus *rhs)
                      { return transform3d_lower(lhs->trafo, rhs->trafo); });
            ...
            for (PrintObjectTrafoAndInstances &new_instances : model_object_status.print_instances)
            {
                ...
                else
                {
                    ...
                    PrintBase::ApplyStatus status = (*it_old)->print_object->set_instances(
                        std::move(new_instances.instances));
        ...
        if (m_objects != print_objects_new)
        {
            ...
            if (new_objects || deleted_objects)
                update_apply_status(
                    this->invalidate_steps({psAlertWhenSupportsNeeded, psSkirtBrim, psWipeTower, psGCodeExport}));
            if (new_objects)
                update_apply_status(false);
            print_regions_reshuffled = true;
        }
    ...
    for (auto it_print_object = m_objects.begin(); it_print_object != m_objects.end();)
    {
        ...
        for (++it_print_object_end; it_print_object_end != m_objects.end() &&
             ...
             ++it_print_object_end)
            assert((*it_print_object_end)->m_shared_regions == nullptr ||
        ...
        if (const auto &volumes = print_object.model_object()->volumes;
            num_extruders > 1 && print_object.model_object()->is_mm_painted())
        {
            ...
            for (const ModelVolume *volume : volumes)
            {
                if (volume->is_mm_painted())
                {
                    ...
                    for (size_t state_idx = 1;
                         state_idx < std::min(volume_used_facet_states.size(), used_facet_states.size()); ++state_idx)
                    {
            ...
            for (size_t state_idx = static_cast<size_t>(TriangleStateType::Extruder1);
                 state_idx < used_facet_states.size(); ++state_idx)
            {
                    ...
                    print_object_regions->trafo_bboxes, model_object_status.print_instances.front().trafo))
            {
                ...
                print_regions_reshuffled = true;
                     ...
                     verify_update_print_object_regions(
                         print_object.model_object()->volumes, m_default_region_config, num_extruders,
                         *print_object_regions,
                                                ...
                                                const t_config_option_keys &diff_keys)
                         {
                             for (auto it = it_print_object; it != it_print_object_end; ++it)
                                 if ((*it)->m_shared_regions != nullptr)
                                     update_apply_status(
                                         (*it)->invalidate_state_by_config_options(old_config, new_config, diff_keys));
                         }))
            {
            ...
            model_object_status.print_object_regions_status != ModelObjectStatus::PrintObjectRegionsStatus::Valid)
        {
            ...
            print_object_regions = generate_print_object_regions(
                print_object_regions, print_object.model_object()->volumes,
                LayerRanges(print_object.model_object()->layer_config_ranges), m_default_region_config,
                model_object_status.print_instances.front().trafo, num_extruders,
...
void Print::cleanup()
{
    ...
    std::sort(all_objects.begin(), all_objects.end(),
              [](const PrintObject *l, const PrintObject *r) { return l->shared_regions() < r->shared_regions(); });
    for (auto it = all_objects.begin(); it != all_objects.end();)
    {
        ...
        for (; it != all_objects.end() && shared_regions == (*it)->shared_regions(); ++it)
            ...
            (*it)->cleanup();
        auto this_objects = SpanOfConstPtrs<PrintObject>(const_cast<const PrintObject *const *const>(&(*it_begin)),
                                                         it - it_begin);
...
bool Print::is_shared_print_object_step_valid_unguarded(SpanOfConstPtrs<PrintObject> print_objects,
                                                        PrintObjectStep print_object_step)
{
    return std::any_of(print_objects.begin(), print_objects.end(),
                       [print_object_step](auto po) { return po->is_step_done_unguarded(print_object_step); });
}
