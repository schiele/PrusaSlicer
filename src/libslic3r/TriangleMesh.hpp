...
template<>
struct IndexedTriangleSetType<AdditionalMeshInfo::None>
{
    using type = indexed_triangle_set;
};

template<>
struct IndexedTriangleSetType<AdditionalMeshInfo::Color>
{
...
struct RepairedMeshErrors
{
    ...
    bool repaired() const
    {
        return degenerate_facets > 0 || edges_fixed > 0 || facets_removed > 0 || facets_reversed > 0 ||
               backwards_edges > 0;
    }
};
...
std::vector<Vec3i> its_face_edge_ids(const indexed_triangle_set &its, std::vector<Vec3i> &face_neighbors,
                                     bool assign_unbound_edges = false, int *num_edges = nullptr);
...
inline Vec2i its_triangle_edge(const stl_triangle_vertex_indices &triangle_indices, int edge_idx)
{
    ...
    return {coord_t(triangle_indices[edge_idx]), coord_t(triangle_indices[next_edge_idx])};
}
...
inline TriangleMesh make_cube(double x, double y, double z)
{
    return TriangleMesh(its_make_cube(x, y, z));
}
inline TriangleMesh make_prism(float width, float length, float height)
{
    return TriangleMesh(its_make_prism(width, length, height));
}
inline TriangleMesh make_cylinder(double r, double h, double fa = (2 * PI / 360))
{
    return TriangleMesh{its_make_cylinder(r, h, fa)};
}
inline TriangleMesh make_cone(double r, double h, double fa = (2 * PI / 360))
{
    return TriangleMesh(its_make_cone(r, h, fa));
}
inline TriangleMesh make_pyramid(float base, float height)
{
    return TriangleMesh(its_make_pyramid(base, height));
}
inline TriangleMesh make_sphere(double rho, double fa = (2 * PI / 360))
{
    return TriangleMesh(its_make_sphere(rho, fa));
}
...
inline BoundingBoxf3 bounding_box(const TriangleMesh &m)
{
...
{
    archive.loadBinary(reinterpret_cast<char *>(const_cast<Slic3r::TriangleMeshStats *>(&mesh.stats())),
                       sizeof(Slic3r::TriangleMeshStats));
