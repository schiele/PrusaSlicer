...
static bool test_line_inside_sphere(const Vec3f &line_a, const Vec3f &line_b, const Vec3f &sphere_p,
...
static bool test_line_inside_cylinder(const Vec3f &line_a, const Vec3f &line_b, const Vec3f &cylinder_P,
                                      const Vec3f &cylinder_Q, const float cylinder_radius)
{
    ...
    const float t1 = (-eq_b - discr_sqrt) / eq_a;
    if (0.f <= t1 && t1 <= 1.f)
        if (const float cylinder_endcap_t1 = m_dot_d + t1 * n_dot_d;
            0.f <= cylinder_endcap_t1 && cylinder_endcap_t1 <= d_dot_d)
            return true;

    const float t2 = (-eq_b + discr_sqrt) / eq_a;
    if (0.f <= t2 && t2 <= 1.f)
        if (const float cylinder_endcap_t2 = (m_dot_d + t2 * n_dot_d);
            0.f <= cylinder_endcap_t2 && cylinder_endcap_t2 <= d_dot_d)
            return true;
...
static bool test_line_inside_capsule(const Vec3f &line_a, const Vec3f &line_b, const Vec3f &capsule_p,
                                     const Vec3f &capsule_q, const float capsule_radius)
{
...
inline bool is_point_inside_triangle(const Vec3f &pt, const Vec3f &p1, const Vec3f &p2, const Vec3f &p3)
{
    ...
    return std::all_of(begin(barycentric_cords), end(barycentric_cords),
                       [](float cord) { return 0.f <= cord && cord <= 1.0; });
}
...
int TriangleSelector::select_unsplit_triangle(const Vec3f &hit, int facet_idx, const Vec3i &neighbors) const
{
    ...
    if (num_of_children != 1)
    {
        for (int i = 0; i < num_of_children; ++i)
        {
            ...
            if (is_point_inside_triangle(hit, m_vertices[t_vert[0]].v, m_vertices[t_vert[1]].v,
                                         m_vertices[t_vert[2]].v))
...
void TriangleSelector::select_patch(int facet_start, std::unique_ptr<Cursor> &&cursor, TriangleStateType new_state,
                                    ...
                                    float highlight_by_angle_deg)
{
    ...
    std::vector<int> facets_to_check = m_cursor->get_facets_to_select(facet_start, m_vertices, m_triangles,
                                                                      m_orig_size_vertices, m_orig_size_indices);
...
void TriangleSelector::seed_fill_select_triangles(const Vec3f &hit, int facet_start,
                                                  const Transform3d &trafo_no_translate, const ClippingPlane &clp,
                                                  float seed_fill_angle, float seed_fill_gap_area,
                                                  float highlight_by_angle_deg, ForceReselection force_reselection)
{
    ...
    if (int start_facet_idx = select_unsplit_triangle(hit, facet_start);
        start_facet_idx >= 0 && m_triangles[start_facet_idx].is_selected_by_seed_fill() &&
        force_reselection == ForceReselection::NO && !clp.is_active())
        return;
    ...
    while (!facet_queue.empty())
    {
        ...
        if (!visited[current_facet] &&
            (highlight_by_angle_deg == 0.f || vec_down.dot(facet_normal) >= highlight_angle_limit))
        {
            if (m_triangles[current_facet].is_split())
            {
                for (int split_triangle_idx = 0;
                     split_triangle_idx <= m_triangles[current_facet].number_of_split_sides(); ++split_triangle_idx)
                {
            ...
            if (this->is_original_triangle(current_facet))
            {
                ...
                for (int neighbor_idx : m_neighbors[current_facet])
                {
                    ...
                    if (neighbor_idx >= 0 && !visited[neighbor_idx] && !is_facet_clipped(neighbor_idx, clp))
                    {
                        ...
                        else if (seed_fill_gap_area > 0. &&
                                 get_triangle_area(m_triangles[neighbor_idx]) <= seed_fill_gap_area)
                        {
...
void TriangleSelector::seed_fill_fill_gaps(const std::vector<int> &gap_fill_candidate_facets,
                                           const float seed_fill_gap_area)
{
    std::vector<bool> visited(m_triangles.size(), false);
    ...
    for (const int starting_facet_idx : gap_fill_candidate_facets)
    {
        ...
        while (!facet_queue.empty())
        {
            ...
            {
                for (int split_triangle_idx = 0; split_triangle_idx <= current_facet.number_of_split_sides();
                     ++split_triangle_idx)
                {
            ...
            if (this->is_original_triangle(current_facet_idx))
            {
                ...
                for (int neighbor_idx : m_neighbors[current_facet_idx])
                {
                    ...
                    if (neighbor_idx >= 0 && !visited[neighbor_idx] &&
                        !m_triangles[neighbor_idx].is_selected_by_seed_fill())
                        facet_queue.push(neighbor_idx);
                }
            }
...
void TriangleSelector::precompute_all_neighbors_recursive(const int facet_idx, const Vec3i &neighbors,
                                                          const Vec3i &neighbors_propagated,
                                                          std::vector<Vec3i> &neighbors_out,
                                                          std::vector<Vec3i> &neighbors_propagated_out) const
{
    assert(facet_idx < int(m_triangles.size()));
...
{
    std::vector<Vec3i> neighbors(m_triangles.size(), Vec3i(coord_t(-1), coord_t(-1), coord_t(-1)));
    std::vector<Vec3i> neighbors_propagated(m_triangles.size(), Vec3i(coord_t(-1), coord_t(-1), coord_t(-1)));
    for (int facet_idx = 0; facet_idx < m_orig_size_indices; ++facet_idx)
    {
        ...
        if (m_triangles[facet_idx].is_split())
            this->precompute_all_neighbors_recursive(facet_idx, neighbors[facet_idx], neighbors_propagated[facet_idx],
                                                     neighbors, neighbors_propagated);
    }
...
void TriangleSelector::append_touching_subtriangles(int itriangle, int vertexi, int vertexj,
                                                    std::vector<int> &touching_subtriangles_out) const
{
    if (itriangle == -1)
        return;

    auto process_subtriangle = [this, &itriangle, &vertexi, &vertexj,
                                &touching_subtriangles_out](const int subtriangle_idx, Partition partition) -> void
    {
...
void TriangleSelector::append_touching_edges(int itriangle, int vertexi, int vertexj,
                                             std::vector<Vec2i> &touching_edges_out) const
{
    if (itriangle == -1)
        return;
...
std::vector<int> TriangleSelector::get_all_touching_triangles(int facet_idx, const Vec3i &neighbors,
                                                              const Vec3i &neighbors_propagated) const
{
    assert(facet_idx != -1 && facet_idx < int(m_triangles.size()));
    assert(this->verify_triangle_neighbors(m_triangles[facet_idx], neighbors));

    const Vec3i vertices = {coord_t(m_triangles[facet_idx].verts_idxs[0]),
                            coord_t(m_triangles[facet_idx].verts_idxs[1]),
                            coord_t(m_triangles[facet_idx].verts_idxs[2])};
                                                    ...
                                                    BucketFillPropagate propagate, ForceReselection force_reselection)
{
    ...
    while (!facet_queue.empty())
    {
        ...
        if (!visited[current_facet])
        {
            ...
            std::vector<int> touching_triangles = this->get_all_touching_triangles(current_facet,
                                                                                   neighbors[current_facet],
    ...
    bucket_fill_fill_gaps(gap_fill_candidate_facets, bucket_fill_gap_area, start_facet_state, neighbors,
                          neighbors_propagated);
}

void TriangleSelector::bucket_fill_fill_gaps(const std::vector<int> &gap_fill_candidate_facets,
                                             const float bucket_fill_gap_area,
                                             const TriangleStateType start_facet_state,
                                             ...
                                             const std::vector<Vec3i> &neighbors_propagated)
{
    ...
    for (const int starting_facet_idx : gap_fill_candidate_facets)
    {
        ...
        while (!facet_queue.empty())
        {
            ...
            std::vector<int> touching_triangles = this->get_all_touching_triangles(
                current_facet_idx, neighbors[current_facet_idx], neighbors_propagated[current_facet_idx]);
            for (const int tr_idx : touching_triangles)
            {
                if (tr_idx < 0 || visited[tr_idx] || m_triangles[tr_idx].get_state() != start_facet_state ||
                    m_triangles[tr_idx].is_selected_by_seed_fill())
                    continue;
...
bool TriangleSelector::select_triangle(int facet_idx, TriangleStateType type, bool triangle_splitting)
{
    ...
    assert(m_invalid_triangles ==
           std::count_if(m_triangles.begin(), m_triangles.end(), [](const Triangle &tr) { return !tr.valid(); }));
...
int TriangleSelector::neighbor_child(const Triangle &tr, int vertexi, int vertexj, Partition partition) const
{
    ...
    if (tr.number_of_split_sides() == 1)
    {
        if (edge != next_idx_modulo(tr.special_side(), 3))
            ...
            return this->neighbor_child(m_triangles[tr.children[edge == tr.special_side() ? 0 : 1]], vertexi, vertexj,
                                        partition);
    ...
    else if (tr.number_of_split_sides() == 2)
    {
        ...
        child_idx = edge == tr.special_side() ? (partition == Partition::First ? 0 : 1)
...
std::pair<int, int> TriangleSelector::triangle_subtriangles(const Triangle &tr, int vertexi, int vertexj)
{
    ...
    if (tr.number_of_split_sides() == 1)
    {
        return edge == next_idx_modulo(tr.special_side(), 3)
                   ? std::make_pair(tr.children[0], tr.children[1])
...
int TriangleSelector::triangle_midpoint(const Triangle &tr, int vertexi, int vertexj) const
{
    ...
    if (tr.number_of_split_sides() == 1)
    {
        return edge == next_idx_modulo(tr.special_side(), 3)
                   ...
                   : this->triangle_midpoint(m_triangles[tr.children[edge == tr.special_side() ? 0 : 1]], vertexi,
                                             vertexj);
    }
    else if (tr.number_of_split_sides() == 2)
    {
...
Vec3i TriangleSelector::child_neighbors_propagated(const Triangle &tr, const Vec3i &neighbors_propagated, int child_idx,
                                                   const Vec3i &child_neighbors) const
{
    int i = tr.special_side();
    int j = next_idx_modulo(i, 3);
    int k = next_idx_modulo(j, 3);
...
bool TriangleSelector::select_triangle_recursive(int facet_idx, const Vec3i &neighbors, TriangleStateType type,
    ...
    else
    {
        ...
        if (num_of_children != 1)
        {
            for (int i = 0; i < num_of_children; ++i)
            {
                ...
                // Recursion, deep first search over the children of this triangle.
                // All children of this triangle were created by splitting a single source triangle of the original mesh.
                select_triangle_recursive(tr->children[i], this->child_neighbors(*tr, neighbors, i), type,
                                          triangle_splitting);
...
void TriangleSelector::split_triangle(int facet_idx, const Vec3i &neighbors)
{
    ...
    std::array<double, 3> sides = {(*pts[2] - *pts[1]).squaredNorm(), (*pts[0] - *pts[2]).squaredNorm(),
...
bool TriangleSelector::Cursor::is_facet_visible(const Cursor &cursor, int facet_idx,
...
inline std::array<Vec3f, 3> TriangleSelector::Cursor::transform_triangle(const Triangle &tr,
...
void TriangleSelector::perform_split(int facet_idx, const Vec3i &neighbors, TriangleStateType old_state)
{
    ...
    case 1:
        ...
        tr.children[ichild++] = push_triangle(verts_idxs[0], verts_idxs[1], verts_idxs[2], tr.source_triangle,
                                              old_state);
    ...
    case 2:
        ...
        tr.children[ichild++] = push_triangle(verts_idxs[0], verts_idxs[1], verts_idxs[4], tr.source_triangle,
                                              old_state);
        tr.children[ichild++] = push_triangle(verts_idxs[1], verts_idxs[2], verts_idxs[4], tr.source_triangle,
                                              old_state);
    ...
    case 3:
        ...
        tr.children[ichild++] = push_triangle(verts_idxs[0], verts_idxs[1], verts_idxs[5], tr.source_triangle,
                                              old_state);
        tr.children[ichild++] = push_triangle(verts_idxs[1], verts_idxs[2], verts_idxs[3], tr.source_triangle,
                                              old_state);
        tr.children[ichild++] = push_triangle(verts_idxs[3], verts_idxs[4], verts_idxs[5], tr.source_triangle,
                                              old_state);
...
typename IndexedTriangleSetType<facet_info>::type TriangleSelector::get_facets(
    const std::function<bool(const Triangle &)> &facet_filter) const
{
...
template<AdditionalMeshInfo facet_info>
typename IndexedTriangleSetType<facet_info>::type TriangleSelector::get_facets_strict(
    const std::function<bool(const Triangle &)> &facet_filter) const
{
    using IndexedTriangleSetType = typename IndexedTriangleSetType<facet_info>::type;
                                                   ...
                                                   std::vector<uint8_t> &out_colors) const
{
    ...
    {
        for (int i = 0; i <= tr.number_of_split_sides(); ++i)
            this->get_facets_strict_recursive<facet_info>(m_triangles[tr.children[i]],
                                                          this->child_neighbors(tr, neighbors, i), facet_filter,
                                                   ...
                                                   std::vector<uint8_t> &out_colors) const
{
    ...
    case 1:
    {
        ...
        this->get_facets_split_by_tjoints<facet_info>(
            ...
            {neighbors(i), this->neighbor_child(neighbors(j), vertices(k), vertices(j), Partition::Second), -1}, color,
            out_triangles, out_colors);
        this->get_facets_split_by_tjoints<facet_info>(
            {midpoints(j), vertices(k), vertices(i)},
            {this->neighbor_child(neighbors(j), vertices(k), vertices(j), Partition::First), neighbors(k), -1}, color,
            out_triangles, out_colors);
        break;
    }
    case 2:
    {
        ...
        this->get_facets_split_by_tjoints<facet_info>(
            ...
            {this->neighbor_child(neighbors(i), vertices(j), vertices(i), Partition::First), -1, -1}, color,
            out_triangles, out_colors);
        this->get_facets_split_by_tjoints<facet_info>(
            {vertices(j), vertices(k), midpoints(k)},
            {neighbors(j), this->neighbor_child(neighbors(k), vertices(i), vertices(k), Partition::Second), -1}, color,
            out_triangles, out_colors);
        break;
    }
    default:
...
void TriangleSelector::get_seed_fill_contour_recursive(const int facet_idx, const Vec3i &neighbors,
                                                       const Vec3i &neighbors_propagated,
                                                       std::vector<Vec2i> &edges_out) const
{
    ...
    {
        Vec3i vertices = {coord_t(m_triangles[facet_idx].verts_idxs[0]), coord_t(m_triangles[facet_idx].verts_idxs[1]),
                          coord_t(m_triangles[facet_idx].verts_idxs[2])};
        ...
        for (int idx = 0; idx < 3; ++idx)
            if (int neighbor_tr_idx = neighbors_propagated(idx);
                neighbor_tr_idx != -1 && !m_triangles[neighbor_tr_idx].is_split() &&
    ...
    struct Serializer
    {
        ...
        void serialize(int facet_idx)
        {
            ...
            if (split_sides)
            {
                ...
                // Serialized in reverse order for backwards compatibility.
            ...
            else
            {
                ...
                {
                    // Simple case, for backwards compatibility when storing paint on supports and seams.
                    // Store 2 bits of n.
...
void TriangleSelector::deserialize(const TriangleSplittingData &data, bool needs_reset)
{
    ...
    {
        int facet_id = 0;
        Vec3i neighbors{coord_t(-1), coord_t(-1), coord_t(-1)};
...
TriangleSelector::Cursor::Cursor(const Vec3f &source_, float radius_world, const Transform3d &trafo_,
...
TriangleSelector::SinglePointCursor::SinglePointCursor(const Vec3f &center_, const Vec3f &source_, float radius_world,
...
TriangleSelector::DoublePointCursor::DoublePointCursor(const Vec3f &first_center_, const Vec3f &second_center_,
                                                       const Vec3f &source_, float radius_world,
                                                       const Transform3d &trafo_, const ClippingPlane &clipping_plane_)
...
bool TriangleSelector::Capsule2D::is_mesh_point_inside(const Vec3f &point) const
{
    ...
    if (first_center_diff_projected.dot(centers_diff_projected) <= 0.f &&
        second_center_diff_projected.dot(centers_diff_projected) >= 0.f)
    {
        ...
        if ((rectangle_a - transformed_point).dot(rectangle_da_dir) <= 0.f &&
            (rectangle_d - transformed_point).dot(rectangle_da_dir) >= 0.f)
            return is_mesh_point_not_clipped(point, clipping_plane);
    }
...
static bool is_circle_pointer_inside_triangle(const Vec3f &p1_, const Vec3f &p2_, const Vec3f &p3_, const Vec3f &center,
                                              const Vec3f &dir, const bool use_world_coordinates,
...
// p1, p2, p3 are in mesh coords!
bool TriangleSelector::SinglePointCursor::is_pointer_in_triangle(const Vec3f &p1_, const Vec3f &p2_,
...
// p1, p2, p3 are in mesh coords!
bool TriangleSelector::DoublePointCursor::is_pointer_in_triangle(const Vec3f &p1_, const Vec3f &p2_,
                                                                 const Vec3f &p3_) const
{
...
bool line_plane_intersection(const Vec3f &line_a, const Vec3f &line_b, const Vec3f &plane_origin,
                             const Vec3f &plane_normal, Vec3f &out_intersection)
{
...
bool TriangleSelector::Capsule3D::is_any_edge_inside_cursor(const Triangle &tr,
                                                            const std::vector<Vertex> &vertices) const
{
    const std::array<Vec3f, 3> pts = this->transform_triangle(tr, vertices);
    for (int side = 0; side < 3; ++side)
...
// Is edge inside cursor?
bool TriangleSelector::Capsule2D::is_any_edge_inside_cursor(const Triangle &tr,
                                                            const std::vector<Vertex> &vertices) const
{
    const Vec3f centers_diff = this->second_center - this->first_center;
    // Vector in the direction of line |AD| of the rectangle that intersects the circle with the center in first_center.
...
TriangleSelector::HeightRange::HeightRange(const Vec3f &mesh_hit, const BoundingBoxf3 &mesh_bbox, float z_range,
                                           const Transform3d &trafo, const ClippingPlane &clipping_plane)
...
bool TriangleSelector::HeightRange::is_any_edge_inside_cursor(const Triangle &tr,
                                                              const std::vector<Vertex> &vertices) const
{
    const std::array<Vec3f, 3> pts = this->transform_triangle(tr, vertices);
...
std::vector<int> TriangleSelector::HeightRange::get_facets_to_select(const int facet_idx,
                                                                     const std::vector<Vertex> &vertices,
                                                                     const std::vector<Triangle> &triangles,
                                                                     const int orig_size_vertices,
                                                                     const int orig_size_indices) const
{
