...
HealedExPolygons text2shapes(
    FontFileWithCache &font, const char *text, const FontProp &font_prop,
    const std::function<bool()> &was_canceled = []() { return false; });
ExPolygonsWithIds text2vshapes(
    FontFileWithCache &font, const std::wstring &text, const FontProp &font_prop,
    const std::function<bool()> &was_canceled = []() { return false; });
...
void apply_transformation(const std::optional<float> &angle, const std::optional<float> &distance,
...
std::string create_range_text(const std::string &text, const FontFile &font, unsigned int font_index,
...
double get_align_y_offset_in_mm(FontProp::VerticalAlign align, unsigned count_lines, const FontFile &ff,
                                const FontProp &fp);
...
std::vector<double> calculate_angles(const BoundingBoxes &glyph_sizes, const PolygonPoints &polygon_points,
