...
//    data << "<!-- Created with preFlight (https://preflight3d.com/) -->\n";
//
...
Point::coord_type to_coor(float val, double scale)
{
    return static_cast<Point::coord_type>(std::round(val * scale));
}
...
bool is_line(const float *p, float precision)
{
    ...
    return is_approx(p[2], p[0] + dx_3, precision) && is_approx(p[4], p[6] - dx_3, precision) &&
           is_approx(p[3], p[1] + dy_3, precision) && is_approx(p[5], p[7] - dy_3, precision);
}
...
void flatten_cubic_bez(Points &points, float tessTol, const Vec2f &p1, const Vec2f &p2, const Vec2f &p3,
                       const Vec2f &p4, int level)
{
...
LinesPath linearize_path(NSVGpath *first_path, const NSVGLineParams &param)
{
    ...
    for (NSVGpath *path = first_path; path != NULL; path = path->next)
    {
        ...
        for (size_t i = 0; i < path_size; i += 3)
        {
            ...
            Vec2f p3(p[4], p[5]);
            Vec2f p4(p[6], p[7]);
            flatten_cubic_bez(points, param.tesselation_tolerance, p1 * param.scale, p2 * param.scale, p3 * param.scale,
                              p4 * param.scale, param.max_level);
        }
        assert(!points.empty());
        if (points.empty())
            continue;
...
HealedExPolygons stroke_to_expolygons(const LinesPath &lines_path, const NSVGshape &shape, const NSVGLineParams &param)
{
    ...
    case NSVGlineJoin::NSVG_JOIN_BEVEL:
        ...
        break;
    case NSVGlineJoin::NSVG_JOIN_MITER:
    ...
    {
    case NSVGlineCap::NSVG_CAP_BUTT:
        end_type = EndType::Butt;
        break;
    case NSVGlineCap::NSVG_CAP_ROUND:
        end_type = EndType::Round;
        break;
