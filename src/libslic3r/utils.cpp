...
static std::error_code real_path_from_handle(HANDLE H, std::wstring &buffer)
{
    ...
    DWORD CountChars = ::GetFinalPathNameByHandleW(H, (LPWSTR) buffer.data(), (DWORD) buffer.size() - 1,
    ...
    if (CountChars > buffer.size())
    {
        ...
        CountChars = ::GetFinalPathNameByHandleW(H, (LPWSTR) buffer.data(), (DWORD) buffer.size() - 1,
                                                 FILE_NAME_NORMALIZED);
    }
    if (CountChars == 0)
...
std::error_code rename(const std::string &from, const std::string &to)
{
    ...
    for (unsigned retry = 0; retry != 200; ++retry)
    {
        ...
        ScopedFileHandle to_handle(::CreateFileW((LPCWSTR) wide_to.data(), GENERIC_READ | DELETE,
                                                 ...
                                                 OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_DELETE_ON_CLOSE,
                                                 NULL));
        if (!to_handle)
        {
...
bool copy_file_linux(const boost::filesystem::path &from, const boost::filesystem::path &to,
                     boost::system::error_code &ec)
{
    ...
    if (to_mode != from_mode && ::fchmod(outfile.fd, from_mode) != 0)
    {
        if (platform_flavor() == PlatformFlavor::LinuxOnChromium)
        {
            ...
            BOOST_LOG_TRIVIAL(info) << "copy_file_linux() failed to fchmod() the output file \"" << to.string()
                                    << "\" to " << from_mode << ": " << ec.message()
        ...
        else
        {
            ...
            BOOST_LOG_TRIVIAL(error) << "copy_file_linux() failed to fchmod() the output file \"" << to.string()
                                     << "\" to " << from_mode << ": " << ec.message();
        }
    }
...
CopyFileResult copy_file_inner(const std::string &from, const std::string &to, std::string &error_message)
{
    ...
    static const auto perms = boost::filesystem::owner_read | boost::filesystem::owner_write |
                              boost::filesystem::group_read | boost::filesystem::others_read; // aka 644
...
CopyFileResult copy_file(const std::string &from, const std::string &to, std::string &error_message,
...
{
    return boost::iends_with(path, ".gcode") || boost::iends_with(path, ".gco") || boost::iends_with(path, ".g") ||
           boost::iends_with(path, ".ngc") || boost::iends_with(path, ".bgcode") || boost::iends_with(path, ".bgc");
}
...
{
    return std::string("generated by preFlight " SLIC3R_VERSION " on ") + Utils::utc_timestamp();
}
