...
const Glyph *get_glyph(int unicode, const FontFile &font, const FontProp &font_prop, Glyphs &cache,
...
void remove_bad(Polygons &polygons)
{
    polygons.erase(std::remove_if(polygons.begin(), polygons.end(), [](const Polygon &p) { return p.size() < 3; }),
                   polygons.end());
}
...
HealedExPolygons Emboss::heal_polygons(const Polygons &shape, bool is_non_zero, unsigned int max_iteration)
{
    ...
    Clipper2Lib::FillRule fill_type = is_non_zero ? Clipper2Lib::FillRule::NonZero : Clipper2Lib::FillRule::EvenOdd;
    ...
    Clipper2Lib::Paths64 paths = Clipper2Lib::SimplifyPaths(Slic3rPolygons_to_ClipperPaths(shape), 2.0);
    Polygons polygons = ClipperPaths_to_Slic3rPolygons(paths);
    polygons.erase(std::remove_if(polygons.begin(), polygons.end(), [](const Polygon &p) { return p.size() < 3; }),
                   polygons.end());
...
Points get_unique_intersections(const Slic3r::IntersectionsLines &intersections)
{
    ...
    std::transform(intersections.begin(), intersections.end(), std::back_inserter(result),
                   [](const Slic3r::IntersectionLines &i)
...
std::optional<Glyph> get_glyph(const stbtt_fontinfo &font_info, int unicode_letter, float flatness)
{
    ...
    stbtt__point *points = stbtt_FlattenCurves(vertices, num_verts, flatness, &contour_lengths, &num_countour_int,
                                               font_info.userdata);
    if (!points)
        return glyph; // no valid flattening
    ScopeGuard sg2(
        [&contour_lengths, &points]()
        {
...
{
    return {boost::nowide::narrow(name.c_str()), boost::nowide::narrow(path.c_str()), EmbossStyle::Type::file_path,
            FontProp()};
}
} // namespace
...
EmbossStyles Emboss::get_font_list_by_register()
{
    ...
    do
    {
        ...
        result = RegEnumValue(hKey, valueIndex, font_name, &font_name_size, 0, &valueType, fileTTF_name,
                              &fileTTF_name_size);
        valueIndex++;
        if (result != ERROR_SUCCESS || valueType != REG_SZ)
            continue;
...
namespace
{
ExPolygons letter2shapes(wchar_t letter, Point &cursor, const FontFile &font, Glyphs &cache, const FontProp &font_prop,
...
HealedExPolygons Emboss::text2shapes(FontFileWithCache &font_with_cache, const char *text, const FontProp &font_prop,
...
ExPolygonsWithIds Emboss::text2vshapes(FontFileWithCache &font_with_cache, const std::wstring &text,
                                       const FontProp &font_prop, const std::function<bool()> &was_canceled)
{
...
void Emboss::apply_transformation(const std::optional<float> &angle, const std::optional<float> &distance,
...
bool Emboss::is_italic(const FontFile &font, unsigned int font_index)
{
    ...
    int name_id = 2;
    int length;
    const char *value = stbtt_GetFontNameString(info, &length, STBTT_PLATFORM_ID_MICROSOFT, STBTT_MS_EID_UNICODE_BMP,
...
std::string Emboss::create_range_text(const std::string &text, const FontFile &font, unsigned int font_index,
                                      bool *exist_unknown)
{
    if (!is_valid(font, font_index))
        return {};
...
indexed_triangle_set polygons2model_unique(const ExPolygons &shape2d, const IProjection &projection,
                                           const Points &points)
{
    ...
    result.vertices.insert(result.vertices.end(), std::make_move_iterator(back_points.begin()),
...
indexed_triangle_set polygons2model_duplicit(const ExPolygons &shape2d, const IProjection &projection,
                                             const Points &points, const Points &duplicits)
{
    // CW order of triangle indices
...
bool point_in_distance(const Coord2 &distance_sq, PolygonPoint &polygon_point, const size_t &i,
                       const Slic3r::Polygon &polygon, bool is_first, bool is_reverse = false)
{
...
std::vector<double> Emboss::calculate_angles(const BoundingBoxes &glyph_sizes, const PolygonPoints &polygon_points,
...
double Emboss::get_align_y_offset_in_mm(FontProp::VerticalAlign align, unsigned count_lines, const FontFile &ff,
                                        const FontProp &fp)
{
