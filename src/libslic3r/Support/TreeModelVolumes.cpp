...
static Polygons calculateMachineBorderCollision(Polygon machine_border)
{
    ...
    machine_border
        .reverse(); // Makes the polygon negative so that we subtract the actual volume from the collision area.
    out.emplace_back(std::move(machine_border));
    return out;
#endif
}

TreeModelVolumes::TreeModelVolumes(const PrintObject &print_object, const BuildVolume &build_volume,
...
#else
    {
        ...
        tbb::parallel_for(
            tbb::blocked_range<size_t>(
                num_raft_layers, num_layers,
                std::min<size_t>(1, std::max<size_t>(16, num_layers / (8 * tbb::this_task_arena::max_concurrency())))),
            [&](const tbb::blocked_range<size_t> &range)
            {
...
void TreeModelVolumes::precalculate(const PrintObject &print_object, const coord_t max_layer,
                                    std::function<void()> throw_on_cancel)
{
...

    {
        ...
        for (size_t distance_to_top = 0; distance_to_top <= config.tip_layers; ++distance_to_top)
        {
            ...
            possible_tip_radiis.emplace_back(
                ceilRadius(config.getRadius(distance_to_top) + m_current_min_xy_dist_delta));
        }
        ...
        for (coord_t radius_eval = m_radius_0; radius_eval <= config.branch_radius;
             radius_eval = ceilRadius(radius_eval + 1))
    ...
    // Calculate the relevant avoidances in parallel as far as possible
    {
        ...
        task_group.run(
            [this, relevant_avoidance_radiis, throw_on_cancel]
        ...
        task_group.run([this, relevant_avoidance_radiis, throw_on_cancel]
                       { calculateWallRestrictions(relevant_avoidance_radiis, throw_on_cancel); });
        task_group.wait();
    }
    ...
    //    m_precalculated = true;
    BOOST_LOG_TRIVIAL(info) << "Precalculating collision took" << dur_col << " ms. Precalculating avoidance took "
                            << dur_avo << " ms.";
...
const Polygons &TreeModelVolumes::getCollision(const coord_t orig_radius, LayerIndex layer_idx, bool min_xy_dist) const
{
    ...
    if (std::optional<std::reference_wrapper<const Polygons>> result = m_collision_cache.getArea({radius, layer_idx});
        result)
        return (*result).get();
    if (m_precalculated)
    {
        BOOST_LOG_TRIVIAL(error_level_not_in_cache)
            << "Had to calculate collision at radius " << radius << " and layer " << layer_idx
...
std::optional<std::pair<coord_t, std::reference_wrapper<const Polygons>>>
TreeModelVolumes::get_collision_lower_bound_area(LayerIndex layer_id, coord_t max_radius) const
{
...
const Polygons &TreeModelVolumes::getCollisionHolefree(coord_t radius, LayerIndex layer_idx) const
{
    ...
    if (std::optional<std::reference_wrapper<const Polygons>> result = m_collision_cache_holefree.getArea(
            {radius, layer_idx});
        result)
        return (*result).get();
    if (m_precalculated)
    {
        BOOST_LOG_TRIVIAL(error_level_not_in_cache)
            << "Had to calculate collision holefree at radius " << radius << " and layer " << layer_idx
            << ", but precalculate was called. Performance may suffer!";
...
const Polygons &TreeModelVolumes::getAvoidance(const coord_t orig_radius, LayerIndex layer_idx, AvoidanceType type,
                                               bool to_model, bool min_xy_dist) const
{
    ...
    if (m_precalculated)
    {
        ...
        {
            BOOST_LOG_TRIVIAL(error_level_not_in_cache)
                << "Had to calculate Avoidance to model at radius " << radius << " and layer " << layer_idx
                << ", but precalculate was called. Performance may suffer!";
        ...
        {
            BOOST_LOG_TRIVIAL(error_level_not_in_cache)
                << "Had to calculate Avoidance at radius " << radius << " and layer " << layer_idx
                << ", but precalculate was called. Performance may suffer!";
...
const Polygons &TreeModelVolumes::getPlaceableAreas(const coord_t orig_radius, LayerIndex layer_idx,
                                                    std::function<void()> throw_on_cancel) const
{
    const coord_t radius = ceilRadius(orig_radius);
    if (std::optional<std::reference_wrapper<const Polygons>> result = m_placeable_areas_cache.getArea(
            {radius, layer_idx});
        result)
        return (*result).get();
    if (m_precalculated)
    {
        BOOST_LOG_TRIVIAL(error_level_not_in_cache)
            << "Had to calculate Placeable Areas at radius " << radius << " and layer " << layer_idx
            << ", but precalculate was called. Performance may suffer!";
        tree_supports_show_error(format("Not precalculated Placeable areas requested, radius %1%, layer %2%", radius,
                                        layer_idx),
                                 false);
    }
    if (orig_radius == 0)
...
const Polygons &TreeModelVolumes::getWallRestriction(const coord_t orig_radius, LayerIndex layer_idx,
                                                     bool min_xy_dist) const
{
    assert(layer_idx > 0);
    if (layer_idx == 0)
    ...
    {
        BOOST_LOG_TRIVIAL(error_level_not_in_cache)
            << "Had to calculate Wall restricions at radius " << radius << " and layer " << layer_idx
            << ", but precalculate was called. Performance may suffer!";
    ...
    return getWallRestriction(
        orig_radius, layer_idx,
                                          ...
                                          std::function<void()> throw_on_cancel)
{
    ...
    std::sort(layer_outline_indices.begin(), layer_outline_indices.end(), [this](size_t i, size_t j)
              { return m_layer_outlines[i].second.size() < m_layer_outlines[j].second.size(); });
    ...
    for (size_t outline_idx : layer_outline_indices)
        if (const std::vector<Polygons> &outlines = m_layer_outlines[outline_idx].second; !outlines.empty())
        {
            ...
            const int z_distance_bottom_layers = int(
                round(double(settings.support_bottom_distance) / double(layer_height)));
            ...
            tbb::parallel_for(tbb::blocked_range<LayerIndex>(data.begin(), data.end()),
                              [&collision_areas_offsetted, &outlines, &machine_border = m_machine_border,
                               &anti_overhang = m_anti_overhang, radius, xy_distance, z_distance_bottom_layers,
                               z_distance_top_layers, min_resolution = m_min_resolution, &data, processing_last_mesh,
                               &throw_on_cancel](const tbb::blocked_range<LayerIndex> &range)
                              {
                                  for (LayerIndex layer_idx = range.begin(); layer_idx != range.end(); ++layer_idx)
                                  {
                                      ...
                                      for (int i = 1; i <= z_distance_top_layers; ++i)
                                          if (int j = layer_idx + i; j < int(outlines.size()))
                                          {
                                              ...
                                              coord_t required_range_x =
                                                  (xy_distance - ((i - (z_distance_top_layers == 1 ? 0.5 : 0)) *
                                              ...
                                              append(collisions,
                                                     offset(union_ex(collision_areas_original),
                                                            radius + required_range_x, JoinType::Miter, 1.2));
                                          }
                                      collisions = processing_last_mesh && layer_idx < int(anti_overhang.size())
                                                       ? union_(collisions, offset(union_ex(anti_overhang[layer_idx]),
                                                                                   radius, JoinType::Miter, 1.2))
                                                       : union_(collisions);
            ...
            if (calculate_placable)
            {
                ...
                tbb::parallel_for(
                    tbb::blocked_range<LayerIndex>(std::max(z_distance_bottom_layers + 1, data.begin()), data.end()),
...
#ifdef SLIC3R_TREESUPPORTS_PROGRESS
    {
        ...
        if (m_precalculated && m_precalculation_progress < TREE_PROGRESS_PRECALC_COLL)
        {
            ...
            Progress::messageProgress(Progress::Stage::SUPPORT,
                                      m_precalculation_progress * m_progress_multiplier + m_progress_offset,
                                      TREE_PROGRESS_TOTAL);
        }
    }
#endif
                                                  ...
                                                  std::function<void()> throw_on_cancel)
{
    ...
    tbb::parallel_for(
        tbb::blocked_range<LayerIndex>(0, max_layer + 1, keys.size()),
                ...
                for (RadiusLayerPair key : keys)
                    if (layer_idx <= key.second)
                    {
                        ...
                        data.emplace_back(RadiusLayerPair(radius, layer_idx),
                                          polygons_simplify(offset(union_ex(this->getCollision(m_increase_until_radius,
                                                                                               layer_idx, false)),
                                                                   5 - increase_radius_ceil, JoinType::Round,
                                                                   m_min_resolution),
                                          ...
                                          std::function<void()> throw_on_cancel)
{
    ...
    for (int iter_idx = 0; iter_idx < 2 * int(keys.size()) * int(AvoidanceType::Count); ++iter_idx)
    {
        ...
        task.start_layer =
            std::max<LayerIndex>(1, 1 + avoidance_cache(task.type, task.to_model).getMaxCalculatedLayer(task.radius));
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, avoidance_tasks.size(), 1),
        [this, &avoidance_tasks, &throw_on_cancel](const tbb::blocked_range<size_t> &range)
        {
            for (size_t task_idx = range.begin(); task_idx < range.end(); ++task_idx)
            {
                ...
                Polygons latest_avoidance = getAvoidance(task.radius, task.start_layer - 1, task.type, task.to_model,
                                                         true);
                std::vector<std::pair<RadiusLayerPair, Polygons>> data;
                ...
                for (LayerIndex layer_idx = task.start_layer; layer_idx <= task.max_required_layer; ++layer_idx)
                {
                    ...
                    const Polygons &current_layer_collisions = collision_holefree
                                                                   ? getCollisionHolefree(task.radius, layer_idx)
                    ...
                    for (int istep = 0; istep < move_steps; ++istep)
                        latest_avoidance = union_(current_layer_collisions,
                                                  offset(latest_avoidance,
                                                         ...
                                                         JoinType::Round, m_min_resolution));
                    if (task.to_model)
                        latest_avoidance = diff(latest_avoidance,
                                                getPlaceableAreas(task.radius, layer_idx, throw_on_cancel));
...
#ifdef SLIC3R_TREESUPPORTS_PROGRESS
                {
                    std::lock_guard<std::mutex> critical_section(*m_critical_progress);
                    if (m_precalculated &&
                        m_precalculation_progress < TREE_PROGRESS_PRECALC_COLL + TREE_PROGRESS_PRECALC_AVO)
                    {
                        m_precalculation_progress += to_model ? 0.4 * TREE_PROGRESS_PRECALC_AVO / (keys.size() * 3)
                                                     : m_support_rests_on_model
                                                         ? 0.4
                                           ...
                                           std::function<void()> throw_on_cancel)
{
    ...
    tbb::parallel_for(
        tbb::blocked_range<LayerIndex>(std::max(1, start_layer), max_required_layer + 1),
                                   ...
                                   &throw_on_cancel](const tbb::blocked_range<LayerIndex> &range)
                                  {
                                      for (LayerIndex layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
                                      {
                                          data[layer_idx - min_layer_bottom] = polygons_simplify(
                                              ...
                                              intersection(getCollision(0, layer_idx, false),
                                                           getCollision(radius, layer_idx - 1, true)),
                                              m_min_resolution, polygons_strictly_simple);
                                          if (!data_min.empty())
                                              data_min[layer_idx - min_layer_bottom] = polygons_simplify(
                                                  intersection(getCollision(0, layer_idx, true),
                                                               getCollision(radius, layer_idx - 1, true)),
                                                  m_min_resolution, polygons_strictly_simple);
                                          throw_on_cancel();
                                      }
...
coord_t TreeModelVolumes::ceilRadius(const coord_t radius) const
{
    ...
    if (radius > m_radius_0)
    {
        ...
        auto ignore = [this](coord_t r)
        {
            return std::binary_search(m_ignorable_radii.begin(), m_ignorable_radii.end(), r);
        };
    ...
    assert(std::is_sorted(out.begin(), out.end(),
                          [](auto &l, auto &r)
                          {
                              return l.first.second < r.first.second ||
                                     (l.first.second == r.first.second) && l.first.first < r.first.first;
                          }));
    return out;
}
