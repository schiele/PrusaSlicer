                           ...
                           const std::vector<Polygons> &excluded_areas)
{
    ...
    // When interface_layers > 0, tips should stay in regular polygons (InterfacePlacer handles interfaces)
    const bool separate_tips = !interface_placer.support_parameters.has_top_contacts;
    ...
    tbb::parallel_for(
        ...
        [&trees, &volumes, &config, &slicing_params, &move_bounds, &mesh_slicing_params, &throw_on_cancel,
         separate_tips](const tbb::blocked_range<size_t> &range)
        {
    ...
    tbb::parallel_for(
        ...
        [&print_object, &config, &slices, &bottom_contacts, &top_contacts, &intermediate_layers, &layer_storage,
         &throw_on_cancel, &excluded_areas, route_tips_to_top_contacts](const tbb::blocked_range<size_t> &range)
        {
