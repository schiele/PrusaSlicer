...
struct ColorProjectionRange
{
    ...
    ColorProjectionRange(double from_t, double from_distance, double to_t, double to_distance,
                         ColorPolygon::Color color)
    ...
    double distance_at(const double t) const
    {
        ...
        return (t - this->from_t) / (this->to_t - this->from_t) * (this->to_distance - this->from_distance) +
               this->from_distance;
    }
    ...
    friend bool operator==(const ColorProjectionRange &lhs, const ColorProjectionRange &rhs)
    {
        return lhs.from_t == rhs.from_t && lhs.from_distance == rhs.from_distance && lhs.to_t == rhs.to_t &&
               lhs.to_distance == rhs.to_distance && lhs.color == rhs.color;
    }
};
...
struct ColorProjectionLineWrapper
{
    ...
    explicit ColorProjectionLineWrapper(ColorProjectionLine *const color_projection_line)
        : a(color_projection_line->a), b(color_projection_line->b), color_projection_line(color_projection_line) {};
...
[[maybe_unused]] static void export_graph_to_svg(const std::string &path, const Voronoi::VD &vd,
                                                 const std::vector<ColoredLines> &colored_polygons)
{
...
[[maybe_unused]] static void export_regions_to_svg(const std::string &path, const std::vector<ExPolygons> &regions,
...
[[maybe_unused]] void export_processed_input_expolygons_to_svg(const std::string &path, const LayerRegionPtrs &regions,
...
[[maybe_unused]] static void export_color_polygons_points_to_svg(const std::string &path,
                                                                 const std::vector<ColorPoints> &color_polygons_points,
                                                                 const ExPolygons &lslices)
{
    const std::vector<std::string> colors = {"aqua", "black", "blue",   "fuchsia", "gray",   "green", "lime",  "maroon",
    ...
    for (const ColorPoints &color_polygon_points : color_polygons_points)
    {
        ...
        svg.draw(Line(color_polygon_points.back().p, color_polygon_points.front().p),
                 colors[color_polygon_points.back().color_next], stroke_width);
    }
}

[[maybe_unused]] static void export_color_polygons_to_svg(const std::string &path, const ColorPolygons &color_polygons,
                                                          const ExPolygons &lslices)
{
    const std::vector<std::string> colors = {"blue",   "cyan",    "red",    "orange", "pink",
                                             "yellow", "magenta", "purple", "black"};
...
[[maybe_unused]] static void export_color_polygons_lines_to_svg(const std::string &path,
                                                                const std::vector<ColorLines> &color_polygons_lines,
                                                                const ExPolygons &lslices)
{
    const std::vector<std::string> colors = {"blue",   "cyan",    "red",    "orange", "pink",
                                             "yellow", "magenta", "purple", "black"};
    ...
    for (const ColorLines &color_polygon_lines : color_polygons_lines)
    {
        ...
        {
            svg.draw(Line(color_line.a, color_line.b),
                     (color_line.color < colors.size() ? colors[color_line.color] : default_color), stroke_width);
        }
    }
}

[[maybe_unused]] static void export_color_projection_lines_color_ranges_to_svg(
    const std::string &path, const ColorProjectionExPolygon &color_projection_expolygon, const ExPolygons &lslices)
{
    const std::vector<std::string> colors = {"blue",   "cyan",    "red",    "orange", "pink",
                                             "yellow", "magenta", "purple", "black"};
    const std::string default_color = "black";
        ...
        for (const ColorProjectionLine &color_projection_line : color_projection_lines)
        {
            ...
            for (const ColorProjectionRange &range : color_projection_line.color_projection_ranges)
            {
                const Vec2d color_projection_line_vec =
                    (color_projection_line.b - color_projection_line.a).cast<double>();
                const Point from_pt = (range.from_t * color_projection_line_vec).cast<coord_t>() +
                                      color_projection_line.a;
                const Point to_pt = (range.to_t * color_projection_line_vec).cast<coord_t>() + color_projection_line.a;

                svg.draw(Line(from_pt, to_pt), (range.color < colors.size() ? colors[range.color] : default_color),
                         stroke_width);
            }
        }
    };
...
inline OutputIterator douglas_peucker(ColorPoints::const_iterator begin, ColorPoints::const_iterator end,
                                      OutputIterator out, const double tolerance,
                                      const double max_different_color_length)
{
    ...
    auto take_floater_predicate = [&tolerance_sq,
                                   &max_different_color_length_sq](ColorPoints::const_iterator anchor_it,
                                                                   ColorPoints::const_iterator floater_it,
                                                                   const int64_t max_dist_sq) -> bool
    {
...
static inline bool points_inside(const Line &contour_first, const Line &contour_second, const Point &new_point)
{
    ...
    auto three_points_inward_normal = [](const Point &left, const Point &middle, const Point &right) -> Vec2d
    {
        assert(left != middle);
        assert(middle != right);
        return (perp(Point(middle - left)).cast<double>().normalized() +
                perp(Point(right - middle)).cast<double>().normalized())
            .normalized();
    };
...
static typename std::enable_if<std::is_same<CellRange, Geometry::SegmentCellRange<Point>>::value ||
                                   std::is_same<CellRange, Geometry::PointCellRange<Point>>::value,
...
cell_range_to_polygon(const CellRange &cell_range)
{
    ...
    cell_polygon.points.emplace_back(
        Geometry::VoronoiUtils::to_point(cell_range.edge_begin->vertex0()).template cast<coord_t>());
...
static std::vector<ExPolygons> extract_colored_segments(const std::vector<ColoredLines> &colored_polygons,
                                                        const size_t num_facets_states, const size_t layer_idx)
{
    ...
    for (const Voronoi::VD::cell_type &cell : vd.cells())
    {
        ...
        if (const Geometry::SegmentCellRange<Point> cell_range =
                Geometry::VoronoiUtils::compute_segment_cell_range(cell, colored_lines.begin(), colored_lines.end());
            cell_range.is_valid())
    ...
    for (const Voronoi::VD::vertex_type &vertex : vd.vertices())
    {
        ...
        if (!Geometry::VoronoiUtils::is_in_range<coord_t>(vertex) ||
            !bbox.contains(Geometry::VoronoiUtils::to_point(vertex).cast<coord_t>()))
    ...
    for (const Voronoi::VD::cell_type &cell : vd.cells())
    {
        ...
        if (const Geometry::SegmentCellRange<Point> cell_range =
            ...
            cell_range.is_valid())
        {
            const ColoredLine &current_line = Geometry::VoronoiUtils::get_source_segment(cell, colored_lines.begin(),
                                                                                         colored_lines.end());
            ...
            do
            {
                ...
                if (!points_inside(current_line.line, next_line.line,
                                   Geometry::VoronoiUtils::to_point(edge->vertex1()).cast<coord_t>()))
                {
                    edge->color(VD_ANNOTATION::DELETED);
                    edge->twin()->color(VD_ANNOTATION::DELETED);
                    delete_vertex_deep(*edge->vertex1());
    ...
    for (const Voronoi::VD::cell_type &cell : vd.cells())
    {
        ...
        const ColoredLine &curr_line = Geometry::VoronoiUtils::get_source_segment(cell, colored_lines.begin(),
                                                                                  colored_lines.end());
    ...
    for (const Voronoi::VD::cell_type &cell : vd.cells())
    {
            ...
            cell_range.is_valid())
        {
            ...
            if (non_deleted_edge_count(cell_begin_vertex) == 1)
            {
                ...
                assert(point_cell_begin_edge->color() != VD_ANNOTATION::DELETED &&
                       point_cell_end_edge->color() != VD_ANNOTATION::DELETED);
                assert(point_cell_begin_edge->vertex0() == point_cell_end_edge->vertex1());

                const Geometry::PointCellRange<Point> point_cell_range(
                    Geometry::VoronoiUtils::to_point(point_cell_begin_edge->vertex0()).cast<coord_t>(),
                    point_cell_begin_edge, point_cell_end_edge);
                ...
                const ColoredLine &next_line = get_next_contour_line(curr_line);
                std::pair<Polygon, Polygon> point_cell_polygons = split_convex_polygon_to_two_polygons_with_equal_area(
                    *point_cell_polygon);
...
static void cut_segmented_layers(const std::vector<ExPolygons> &input_expolygons,
                                 std::vector<std::vector<ExPolygons>> &segmented_regions, const float cut_width,
                                 const float interlocking_depth, const std::function<void()> &throw_on_cancel_callback,
                                 const bool preserve_holes = false)
{
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, segmented_regions.size()),
        [&segmented_regions, &input_expolygons, &cut_width, &interlocking_cut_width, &throw_on_cancel_callback,
         &preserve_holes](const tbb::blocked_range<size_t> &range)
        {
            for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
            {
                throw_on_cancel_callback();
                const float region_cut_width = (layer_idx % 2 == 0 && interlocking_cut_width > 0.f)
                                                   ? interlocking_cut_width
                                                   : cut_width;
                ...
                if (region_cut_width > 0.f)
                {
                    ...
                    for (size_t extruder_idx = 0; extruder_idx < num_extruders_plus_one; ++extruder_idx)
                        if (const ExPolygons &ex_polygons = segmented_regions[layer_idx][extruder_idx];
                            !ex_polygons.empty())
                        {
                            // For fuzzy skin, we need perimeter bands around BOTH outer contours AND holes.
                            // The challenge: standard offset expands holes which "eats into" painted outer areas.
                            // Solution: compute outer band and hole bands SEPARATELY, then union them.
                            //
                            // Outer band: between original outer and (outer shrunk by delta) - shrink outer
                            // Hole bands: between original hole and (hole expanded by delta) - expand holes INTO material

                            ExPolygons result;
                            if (preserve_holes)
                            {
                                // Step 1: Compute outer band (shrink outer contour only, no holes)
                                ExPolygons outer_band;
                                for (const ExPolygon &ep : ex_polygons)
                                {
                                    // Get just the outer contour as a solid shape
                                    ExPolygon outer_only(ep.contour);
                                    // Shrink it
                                    ExPolygons shrunk_outer = offset_ex(outer_only, -region_cut_width);
                                    // Outer band = original outer - shrunk outer
                                    ExPolygons band = diff_ex(ExPolygons{outer_only}, shrunk_outer);
                                    append(outer_band, band);
                                }

                                // Step 2: Compute hole bands (expand holes into material)
                                ExPolygons hole_bands;
                                for (const ExPolygon &ep : ex_polygons)
                                {
                                    for (const Polygon &hole : ep.holes)
                                    {
                                        // Convert hole to a positive polygon (CCW)
                                        Polygon hole_ccw = hole;
                                        hole_ccw.make_counter_clockwise();
                                        ExPolygon hole_shape(hole_ccw);

                                        // Expand hole outward (into material) - use POSITIVE offset
                                        ExPolygons hole_expanded = offset_ex(hole_shape, region_cut_width);

                                        // Hole band = expanded hole - original hole (the ring of material)
                                        ExPolygons band = diff_ex(hole_expanded, ExPolygons{hole_shape});
                                        append(hole_bands, band);
                                    }
                                }

                                // Step 3: Union outer and hole bands, clip to painted region
                                ExPolygons all_bands = union_ex(outer_band, hole_bands);
                                result = intersection_ex(all_bands, ex_polygons);
                            }
                            else
                            {
                                ExPolygons shrunk = offset_ex(input_expolygons[layer_idx], -region_cut_width);
                                result = diff_ex(ex_polygons, shrunk);
                            }
                            segmented_regions_cuts[extruder_idx] = std::move(result);
                        }
...
static inline ExPolygons trim_by_top_or_bottom_layer(
    ExPolygons expolygons_to_trim, const size_t top_or_bottom_layer_idx,
    const std::vector<std::vector<Polygons>> &top_or_bottom_raw_by_extruder)
{
...
static inline std::vector<std::vector<ExPolygons>> segmentation_top_and_bottom_layers(
    const PrintObject &print_object, const std::vector<ExPolygons> &input_expolygons,
    const std::function<ModelVolumeFacetsInfo(const ModelVolume &)> &extract_facets_info,
    const size_t num_facets_states, const std::function<void()> &throw_on_cancel_callback)
{
    ...
    for (size_t i = 0; i < print_object.num_printing_regions(); ++i)
    {
        ...
        granularity = std::max(granularity,
                               std::max(config.top_solid_layers.value, config.bottom_solid_layers.value) - 1);
    }
    ...
    if (max_top_layers > 0 || max_bottom_layers > 0)
    {
        for (const ModelVolume *mv : print_object.model_object()->volumes)
            if (mv->is_model_part())
            {
                ...
                {
                    const indexed_triangle_set painted = extract_facets_info(*mv).facets_annotation.get_facets_strict(
                        *mv, TriangleStateType(extruder_idx));
                    ...
                    if (!painted.indices.empty())
                    {
                        ...
                        if (!zs.empty() && is_volume_sinking(painted, volume_trafo))
                        {
                            ...
                            slice_mesh_slabs(painted, zs_sinking, volume_trafo, max_top_layers > 0 ? &top : nullptr,
                                             max_bottom_layers > 0 ? &bottom : nullptr, throw_on_cancel_callback);
    ...
    for (size_t extruder_idx = 0; extruder_idx < num_facets_states; ++extruder_idx)
    {
        for (size_t layer_idx = 0; layer_idx < num_layers; ++layer_idx)
        {
            ...
            const bool has_bottom_surface = !bottom_raw[extruder_idx].empty() &&
                                            !bottom_raw[extruder_idx][layer_idx].empty();

            if (has_top_surface && layer_idx < (num_layers - 1))
            {
                top_raw[extruder_idx][layer_idx] = diff(top_raw[extruder_idx][layer_idx],
                                                        input_expolygons[layer_idx + 1]);
            }

            if (has_bottom_surface && layer_idx > 0)
            {
                bottom_raw[extruder_idx][layer_idx] = diff(bottom_raw[extruder_idx][layer_idx],
                                                           input_expolygons[layer_idx - 1]);
            }
        }
    }
    ...
    if constexpr (MM_SEGMENTATION_DEBUG_TOP_BOTTOM)
    {
        const std::vector<std::string> colors = {"aqua",   "black", "blue",   "fuchsia", "gray",
            ...
            for (size_t extruder_idx = 0; extruder_idx < num_facets_states; ++extruder_idx)
            {
                if (!top_raw[extruder_idx].empty() && !top_raw[extruder_idx][layer_id].empty())
                {
                    if (ExPolygons expoly = union_ex(top_raw[extruder_idx][layer_id]); !expoly.empty())
                    {
                        ...
                        svg.emplace_back(expoly,
                                         SVG::ExPolygonAttributes{format("top%d", extruder_idx), color, color, color});
                    }
                }
                ...
                if (!bottom_raw[extruder_idx].empty() && !bottom_raw[extruder_idx][layer_id].empty())
                {
                    if (ExPolygons expoly = union_ex(bottom_raw[extruder_idx][layer_id]); !expoly.empty())
                    {
                        ...
                        svg.emplace_back(expoly, SVG::ExPolygonAttributes{format("bottom%d", extruder_idx), color,
                                                                          color, color});
                    }
                }
            }

            SVG::export_expolygons(debug_out_path("mm-segmentation-top-bottom-%d-%lf.svg", layer_id, zs[layer_id]),
                                   svg);
        }
    }
    ...
    auto layer_color_stat = [&layers = std::as_const(layers)](const size_t layer_idx,
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, num_layers, granularity),
        [&granularity, &num_layers, &num_facets_states, &layer_color_stat, &top_raw, &triangles_by_color_top,
         &throw_on_cancel_callback, &input_expolygons, &bottom_raw,
         &triangles_by_color_bottom](const tbb::blocked_range<size_t> &range)
        {
            ...
            for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
            {
                for (size_t color_idx = 0; color_idx < num_facets_states; ++color_idx)
                {
                        ...
                        if (ExPolygons top_ex = union_ex(top[layer_idx]); !top_ex.empty())
                        {
                            ...
                            if (!top_ex.empty())
                            {
                                ...
                                for (int last_idx = int(layer_idx) - 1;
                                     last_idx >= std::max(int(layer_idx - stat.top_solid_layers), int(0)); --last_idx)
                                {
                                    ...
                                    append(triangles_by_color_top[color_idx][last_idx + layer_idx_offset],
                                           std::move(last));
                                }
                            }
                        }
                    ...
                    if (std::vector<Polygons> &bottom = bottom_raw[color_idx];
                        !bottom.empty() && !bottom[layer_idx].empty())
                    {
                        if (ExPolygons bottom_ex = union_ex(bottom[layer_idx]); !bottom_ex.empty())
                        {
                            ...
                            if (!bottom_ex.empty())
                            {
                                ...
                                float offset = 0.f;
                                ExPolygons layer_slices_trimmed = input_expolygons[layer_idx];
                                for (size_t last_idx = layer_idx + 1;
                                     last_idx < std::min(layer_idx + stat.bottom_solid_layers, num_layers); ++last_idx)
                                {
                                    offset -= stat.extrusion_width;
                                    layer_slices_trimmed = intersection_ex(layer_slices_trimmed,
                                                                           input_expolygons[last_idx]);
                                    ExPolygons last = intersection_ex(bottom_ex,
                                    ...
                                    append(triangles_by_color_bottom[color_idx][last_idx + layer_idx_offset],
                                           std::move(last));
                                }
                            }
                        }
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, num_layers),
                      [&triangles_by_color_merged, &triangles_by_color_bottom, &triangles_by_color_top, &num_layers,
                          ...
                          for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
                          {
                              ...
                              for (size_t color_idx = 1; color_idx < triangles_by_color_merged.size(); ++color_idx)
                                  triangles_by_color_merged[color_idx][layer_idx] =
                                      diff_ex(triangles_by_color_merged[color_idx][layer_idx],
...
static std::vector<std::vector<ExPolygons>> merge_segmented_layers(
    ...
    std::vector<std::vector<ExPolygons>> &&top_and_bottom_layers, const size_t num_facets_states,
                ...
                for (size_t extruder_id = 1; extruder_id < num_facets_states; ++extruder_id)
                {
                    ...
                    if (!segmented_regions[layer_idx][extruder_id].empty())
                    {
                        ...
                        if (!top_and_bottom_layers.empty())
                        {
                            ...
                            {
                                if (!top_and_bottom_by_extruder[layer_idx].empty() &&
                                    !segmented_regions_trimmed.empty())
                                {
                                    segmented_regions_trimmed = diff_ex(segmented_regions_trimmed,
                    ...
                    if (!top_and_bottom_layers.empty() && !top_and_bottom_layers[extruder_id][layer_idx].empty())
                    {
                        ...
                        append(segmented_regions_merged[layer_idx][extruder_id - 1],
                               top_and_bottom_layers[extruder_id][layer_idx]);
                        ...
                        if (!was_top_and_bottom_empty)
                            segmented_regions_merged[layer_idx][extruder_id - 1] =
                                offset2_ex(union_ex(segmented_regions_merged[layer_idx][extruder_id - 1]),
...
bool filter_out_small_color_polygons(ColorPolygons &color_polygons, const double filter_min_area,
                                     const float filter_offset)
{
    ...
    {
        if (std::abs(color_polygon.area()) >= filter_min_area &&
            (filter_offset <= 0. || !offset(Polygon(color_polygon.points), filter_offset).empty()))
        {
            if (const size_t color_polygon_idx = &color_polygon - color_polygons.data();
                first_free_idx < color_polygon_idx)
            {
...
std::vector<ColoredLines> color_points_to_colored_lines(const std::vector<ColorPoints> &color_polygons_points)
{
    ...
    for (const ColorPoints &color_polygon_points : color_polygons_points)
    {
        ...
        for (size_t cpt_idx = 0; cpt_idx < color_polygon_points.size() - 1; ++cpt_idx)
        {
            ...
            colored_lines.push_back({Line(curr_color_point.p, next_color_point.p), curr_color_point.color_next,
                                     int(color_polygon_idx), int(cpt_idx)});
        }

        colored_lines.push_back({Line(color_polygon_points.back().p, color_polygon_points.front().p),
                                 color_polygon_points.back().color_next, int(color_polygon_idx),
                                 int(color_polygon_points.size() - 1)});
    }
...
{
    const size_t total_color_lines_count = std::accumulate(color_polygons_lines.begin(), color_polygons_lines.end(),
                                                           size_t(0),
...
void static filter_color_of_small_segments(ColorPoints &color_polygon_points, const double max_different_color_length)
{
    ...
    for (size_t from_segment_idx = 0; from_segment_idx < color_segments.size();)
    {
        ...
        {
            for (size_t pt_idx = color_segments[curr_segment_idx].color_pt_begin_idx;
                 pt_idx != color_segments[curr_segment_idx].color_pt_end_idx;
                 pt_idx = (pt_idx < color_polygon_points.size() - 1) ? pt_idx + 1 : 0)
            {
...
static std::vector<ColorProjectionLineWrapper> create_color_projection_lines_mapping(
    ColorProjectionExPolygons &color_projection_expolygons)
{
    ...
    const auto append_color_projection_lines =
        [&color_projection_lines_mapping](ColorProjectionLines &color_polygon_projection_lines)
    {
...
static ColorPolygon::Color get_color_of_first_polygon_line(const ColorProjectionLines &color_polygon_projection_lines)
{
    ...
    else if (const ColorProjectionLine &first_line = color_polygon_projection_lines.front();
             !first_line.color_changes.empty() && first_line.color_changes.front().t == 0.f)
    {
        return first_line.color_changes.front().color_next;
    }

    auto last_projection_line_it = std::find_if(color_polygon_projection_lines.rbegin(),
                                                color_polygon_projection_lines.rend(),
                                                [](const ColorProjectionLine &line)
...
static void filter_projected_color_points_on_polygon(ColorProjectionLines &color_polygon_projection_lines)
{
    for (ColorProjectionLine &color_line : color_polygon_projection_lines)
    {
        ...
        {
            if (const double endpoint_dist = color_change.t * line_length;
                endpoint_dist < MM_SEGMENTATION_MAX_SNAP_DISTANCE_SCALED)
            {
        ...
        for (auto cr_it = color_line.color_changes.rbegin(); cr_it != color_line.color_changes.rend(); ++cr_it)
        {
            ...
            if (const double endpoint_dist = (1. - color_change.t) * line_length;
                endpoint_dist < MM_SEGMENTATION_MAX_SNAP_DISTANCE_SCALED)
            {
                snap_candidates.emplace_back(&color_change);
            }
        ...
        if (color_line.color_changes.size() > 1)
        {
            ...
            for (auto cr_it = std::next(color_line.color_changes.begin()); cr_it != color_line.color_changes.end();
                 ++cr_it)
            {
                ColorChange &color_change = *cr_it;
                ...
                else if (const double t_diff = (color_change.t - color_changes_filtered.back().t);
...
static double calc_three_color_points_angle(const ColorPoint &prev_pt, const ColorPoint &curr_pt,
                                            const ColorPoint &next_pt)
{
...
static std::optional<SnapAngle> find_nearest_snap_angle(const ColorPoints &color_polygon_points, const size_t begin_idx,
                                                        const double max_distance, const double angle_threshold)
{
    assert(begin_idx < color_polygon_points.size() && color_polygon_points.size() >= 3);

    const auto calc_three_points_angle = [&color_polygon_points](const size_t prev_idx, const size_t curr_idx,
                                                                 const size_t next_idx) -> double
    {
        return calc_three_color_points_angle(color_polygon_points[prev_idx], color_polygon_points[curr_idx],
                                             color_polygon_points[next_idx]);
    };
...
static void snap_projected_color_points_to_nearest_angles(std::vector<ColorPoints> &color_polygons_points)
{
    for (ColorPoints &color_polygon_points : color_polygons_points)
    {
        ...
        for (size_t curr_idx = 0; curr_idx < color_polygon_points.size(); ++curr_idx)
        {
            ...
            if (prev_pt.color_next == curr_pt.color_next ||
                calc_three_color_points_angle(prev_pt, curr_pt, next_pt) > MM_SEGMENTATION_SNAP_ANGLE_THRESHOLD)
            {
                continue;
            }

            const std::optional<SnapAngle> snap_angle =
                find_nearest_snap_angle(color_polygon_points, curr_idx, MM_SEGMENTATION_SNAP_ANGLE_MAX_DISTANCE_SCALED,
                                        MM_SEGMENTATION_SNAP_ANGLE_THRESHOLD);
            ...
            if (snap_angle->direction == SnapAngle::Direction::Forward)
            {
                ...
                for (size_t modify_idx = curr_idx; modify_idx != snap_angle->pt_index;
                     modify_idx = next_index(modify_idx))
                {
            ...
            else
            {
                ...
                for (size_t modify_idx = snap_angle->pt_index; modify_idx != curr_idx;
                     modify_idx = next_index(modify_idx))
                {
...
static ColorPoints convert_color_polygon_projection_lines_to_color_points(
    const ColorProjectionLines &color_polygon_projection_lines)
{
    if (color_polygon_projection_lines.empty())
        return {};
    ...
    douglas_peucker(color_polygon_points.begin(), color_polygon_points.end(),
                    std::back_inserter(color_polygon_points_filtered), INPUT_POLYGONS_FILTER_TOLERANCE_SCALED,
                    POLYGON_COLOR_FILTER_DISTANCE_SCALED);
...
static std::vector<ColorPoints> convert_color_expolygon_projection_lines_to_color_points(
    const ColorProjectionExPolygon &color_projection_expolygon)
{
    std::vector<ColorPoints> color_expolygon_points;
    color_expolygon_points.reserve(color_projection_expolygon.polygons_count());

    ColorPoints contour_color_points = convert_color_polygon_projection_lines_to_color_points(
        color_projection_expolygon.contour);
    ...
    for (const ColorProjectionLines &hole_color_projection_lines : color_projection_expolygon.holes)
    {
        ColorPoints hole_color_points = convert_color_polygon_projection_lines_to_color_points(
            hole_color_projection_lines);
...
static std::optional<ColorProjectionRange> project_color_line_on_projection_line(
    const ColorLine &color_line, const ColorProjectionLine &projection_line,
    const double max_projection_distance_scaled)
{
    ...
    auto distance_to_color_line = [&projection_line_vec, &projection_line,
                                   &color_line](const double t_raw, const double t_clamped,
                                                const Vec2d &color_line_vec_pt) -> double
    {
        if (0. <= t_raw && t_raw <= 1.)
        {
    ...
    ColorProjectionRange range = t_a_clamped < t_b_clamped
                                     ? ColorProjectionRange{t_a_clamped, color_line_a_dist, t_b_clamped,
                                                            color_line_b_dist, color_line.color}
                                     : ColorProjectionRange{t_b_clamped, color_line_b_dist, t_a_clamped,
                                                            color_line_a_dist, color_line.color};
    ...
    const double t_max = (max_projection_distance_scaled - range.from_distance) /
                             (range.to_distance - range.from_distance) * (range.to_t - range.from_t) +
                         range.from_t;
...
static std::vector<ColorPolygons> slice_model_volume_with_color(
    const ModelVolume &model_volume,
    const std::function<ModelVolumeFacetsInfo(const ModelVolume &)> &extract_facets_info,
    const std::vector<float> &layer_zs, const PrintObject &print_object, const size_t num_facets_states)
{
    ...
    {
        if (const int volume_extruder_id = model_volume.extruder_id();
            facets_info.replace_default_extruder && !facets_info.is_painted && volume_extruder_id >= 0)
        {
        ...
        facets_info.replace_default_extruder && facets_info.is_painted && volume_extruder_id > 0)
    {
        for (ColorPolygons &color_polygons : color_polygons_per_layer)
        {
            for (ColorPolygon &color_polygon : color_polygons)
            {
                std::replace(color_polygon.colors.begin(), color_polygon.colors.end(),
                             static_cast<uint8_t>(TriangleStateType::NONE), static_cast<uint8_t>(volume_extruder_id));
            }
        }
    }
...
static void project_color_projection_lines_on_color_lines(
    ColorProjectionLines &color_projection_lines, const AABBTreeLines::LinesDistancer<ColorLine> &color_lines_distancer)
{
    for (ColorProjectionLine &projection_line : color_projection_lines)
    {
        std::vector<size_t> nearest_color_line_indices;
        Slic3r::append(nearest_color_line_indices,
                       color_lines_distancer.all_lines_in_radius(projection_line.a,
                                                                 MM_SEGMENTATION_MAX_PROJECTION_DISTANCE_SCALED));
        Slic3r::append(nearest_color_line_indices,
                       color_lines_distancer.all_lines_in_radius(projection_line.b,
                                                                 MM_SEGMENTATION_MAX_PROJECTION_DISTANCE_SCALED));
        ...
        for (size_t nearest_color_line_idx : nearest_color_line_indices)
        {
            ...
            std::optional<ColorProjectionRange> projection = project_color_line_on_projection_line(
                color_line, projection_line, MM_SEGMENTATION_MAX_PROJECTION_DISTANCE_SCALED);
...
// For each ColorProjectionLine, find the nearest ColorLines and project them on the ColorProjectionLine.
static void project_color_projection_expolygon_on_color_lines(
    ColorProjectionExPolygon &color_projection_expolygon,
    const AABBTreeLines::LinesDistancer<ColorLine> &color_lines_distancer)
{
...
// For each ColorProjectionLine, find the nearest ColorLines and project them on the ColorProjectionLine.
static void project_color_projection_expolygons_on_color_lines(
    ColorProjectionExPolygons &color_projection_expolygons,
    const AABBTreeLines::LinesDistancer<ColorLine> &color_lines_distancer)
{
    for (ColorProjectionExPolygon &color_projection_expolygon : color_projection_expolygons)
...
static void project_color_lines_on_color_projection_lines(
    std::vector<ColorLines> &color_polygons_lines,
    const AABBTreeLines::LinesDistancer<ColorProjectionLineWrapper> &color_projection_lines_distancer)
{
    for (const ColorLines &color_polygon : color_polygons_lines)
    {
        for (const ColorLine &color_line : color_polygon)
        {
            ...
            Slic3r::append(nearest_projection_line_indices,
                           color_projection_lines_distancer.all_lines_in_radius(
                               color_line.a, MM_SEGMENTATION_MAX_PROJECTION_DISTANCE_SCALED));
            Slic3r::append(nearest_projection_line_indices,
                           color_projection_lines_distancer.all_lines_in_radius(
            ...
            {
                ColorProjectionLine &color_projection_line =
                    *color_projection_lines_distancer.get_line(nearest_projection_line_idx).color_projection_line;
                std::optional<ColorProjectionRange> projection = project_color_line_on_projection_line(
                    color_line, color_projection_line, MM_SEGMENTATION_MAX_PROJECTION_DISTANCE_SCALED);
                if (projection.has_value())
                {
...
std::vector<std::vector<ExPolygons>> segmentation_by_painting(
    ...
    const std::function<ModelVolumeFacetsInfo(const ModelVolume &)> &extract_facets_info,
    const size_t num_facets_states, const float segmentation_max_width, const float segmentation_interlocking_depth,
    const bool segmentation_interlocking_beam, const IncludeTopAndBottomLayers include_top_and_bottom_layers,
    const std::function<void()> &throw_on_cancel_callback, const bool preserve_holes)
{
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, num_layers),
        [&layers, &input_expolygons, &input_expolygons_projection_lines_layers,
         &throw_on_cancel_callback](const tbb::blocked_range<size_t> &range)
        {
            for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
            {
                ...
                input_expolygons[layer_idx] = remove_duplicates(
                    expolygons_simplify(offset_ex(ex_polygons, -10.f * float(SCALED_EPSILON)), 5 * SCALED_EPSILON),
                    scaled<coord_t>(0.01), PI / 6);
                input_expolygons_projection_lines_layers[layer_idx] = create_color_projection_expolygons(
                    input_expolygons[layer_idx]);

                if constexpr (MM_SEGMENTATION_DEBUG_INPUT)
                {
                    export_processed_input_expolygons_to_svg(debug_out_path("mm-input-%d.svg", layer_idx),
    ...
    for (const ModelVolume *mv : print_object.model_object()->volumes)
    {
        std::vector<ColorPolygons> color_polygons_per_layer = slice_model_volume_with_color(*mv, extract_facets_info,
                                                                                            layer_zs, print_object,
                                                                                            num_facets_states);

        tbb::parallel_for(tbb::blocked_range<size_t>(0, num_layers),
                          [&color_polygons_per_layer, &color_polygons_lines_layers,
                           &throw_on_cancel_callback](const tbb::blocked_range<size_t> &range)
                          {
                              for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
                              {
                                  ...
                                  filter_out_small_color_polygons(raw_color_polygons, POLYGON_FILTER_MIN_AREA_SCALED,
                                                                  POLYGON_FILTER_MIN_OFFSET_SCALED);
                                  ...
                                  color_polygons_lines_layers[layer_idx].reserve(
                                      color_polygons_lines_layers[layer_idx].size() + raw_color_polygons.size());
                                  for (const ColorPoints &color_polygon_points :
                                       color_polygons_to_color_points(raw_color_polygons))
                                  {
                                      ColorPoints color_polygon_points_filtered;
                                      color_polygon_points_filtered.reserve(color_polygon_points.size());

                                      douglas_peucker(color_polygon_points.begin(), color_polygon_points.end(),
                                                      std::back_inserter(color_polygon_points_filtered),
                                                      POLYGON_COLOR_FILTER_TOLERANCE_SCALED,
                                                      POLYGON_COLOR_FILTER_DISTANCE_SCALED);

                                      if (color_polygon_points_filtered.size() < 3)
                                          continue;

                                      filter_color_of_small_segments(color_polygon_points_filtered,
                                                                     POLYGON_COLOR_FILTER_DISTANCE_SCALED);
                                      assert(is_valid_color_polygon_points(color_polygon_points_filtered));

                                      color_polygons_lines_layers[layer_idx].emplace_back(
                                          color_points_to_color_lines(color_polygon_points_filtered));
                                  }
                              }
                          }); // end of parallel_for
    }
    ...
    if constexpr (MM_SEGMENTATION_DEBUG_FILTERED_COLOR_LINES)
    {
        ...
        {
            export_color_polygons_lines_to_svg(debug_out_path("mm-filtered-color-line-%d.svg", layer_idx),
                                               color_polygons_lines_layers[layer_idx], input_expolygons[layer_idx]);
        }
    }
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, num_layers),
        [&color_polygons_lines_layers, &input_expolygons_projection_lines_layers,
         &throw_on_cancel_callback](const tbb::blocked_range<size_t> &range)
        {
            for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
            {
                ...
                // For each ColorLine, find the nearest ColorProjectionLines and project the ColorLine on each ColorProjectionLine.
                const AABBTreeLines::LinesDistancer<ColorProjectionLineWrapper> color_projection_lines_distancer{
                    create_color_projection_lines_mapping(input_expolygons_projection_lines_layers[layer_idx])};
                project_color_lines_on_color_projection_lines(color_polygons_lines_layers[layer_idx],
                                                              color_projection_lines_distancer);

                // For each ColorProjectionLine, find the nearest ColorLines and project them on the ColorProjectionLine.
                const AABBTreeLines::LinesDistancer<ColorLine> color_lines_distancer{
                    flatten_color_lines(color_polygons_lines_layers[layer_idx])};
                project_color_projection_expolygons_on_color_lines(input_expolygons_projection_lines_layers[layer_idx],
                                                                   color_lines_distancer);
            }
        }); // end of parallel_for
    ...
    tbb::parallel_for(
        tbb::blocked_range<size_t>(0, num_layers),
        [&input_expolygons_projection_lines_layers, &segmented_regions, &input_expolygons, &num_facets_states,
         &throw_on_cancel_callback](const tbb::blocked_range<size_t> &range)
        {
            for (size_t layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx)
            {
                ...
                ColorProjectionExPolygons &input_expolygons_projection_lines =
                    input_expolygons_projection_lines_layers[layer_idx];
                for (ColorProjectionExPolygon &input_expolygon_projection_lines : input_expolygons_projection_lines)
                {
                    const size_t expolygon_idx = &input_expolygon_projection_lines -
                                                 input_expolygons_projection_lines.data();

                    if constexpr (MM_SEGMENTATION_DEBUG_COLOR_RANGES)
                    {
                        export_color_projection_lines_color_ranges_to_svg(debug_out_path("mm-color-ranges-%d-%d.svg",
                                                                                         layer_idx, expolygon_idx),
                                                                          input_expolygon_projection_lines,
                                                                          input_expolygons[layer_idx]);
                    }
                    ...
                    std::vector<ColorPoints> color_polygons_points =
                        convert_color_expolygon_projection_lines_to_color_points(input_expolygon_projection_lines);
                    ...
                    {
                        export_color_polygons_points_to_svg(debug_out_path("mm-projected-color_polygon-%d-%d.svg",
                                                                           layer_idx, expolygon_idx),
                                                            color_polygons_points, input_expolygons[layer_idx]);
                    }

                    const std::vector<ColoredLines> colored_polygons = color_points_to_colored_lines(
                        color_polygons_points);
                    assert(!colored_polygons.empty());
                    if (has_polygons_only_one_color(colored_polygons))
                    {
                        ...
                        assert(!colored_polygons.front().empty());
                        segmented_regions[layer_idx][size_t(colored_polygons.front().front().color)].emplace_back(
                            input_expolygons[layer_idx][expolygon_idx]);
                    }
                    else
                    {
                        std::vector<ExPolygons> colored_segments_by_states = extract_colored_segments(colored_polygons,
                                                                                                      num_facets_states,
                                                                                                      layer_idx);
                        for (size_t state_idx = 0; state_idx < num_facets_states; ++state_idx)
                        {
                            ...
                            Slic3r::append(segmented_regions[layer_idx][state_idx],
                                           std::move(colored_segments_by_states[state_idx]));
                        }
                    }
                }
                ...
                {
                    export_regions_to_svg(debug_out_path("mm-regions-non-merged-%d.svg", layer_idx),
                                          segmented_regions[layer_idx], input_expolygons[layer_idx]);
                }
            }
        }); // end of parallel_for
    ...
    {
        top_and_bottom_layers = segmentation_top_and_bottom_layers(print_object, input_expolygons, extract_facets_info,
    ...
    {
        cut_segmented_layers(input_expolygons, segmented_regions, scaled<float>(segmentation_max_width),
                             scaled<float>(segmentation_interlocking_depth), throw_on_cancel_callback, preserve_holes);
        throw_on_cancel_callback();
    }

    std::vector<std::vector<ExPolygons>> segmented_regions_merged = merge_segmented_layers(
        segmented_regions, std::move(top_and_bottom_layers), num_facets_states, throw_on_cancel_callback);
    throw_on_cancel_callback();
    ...
    if constexpr (MM_SEGMENTATION_DEBUG_REGIONS)
    {
        for (size_t layer_idx = 0; layer_idx < print_object.layers().size(); ++layer_idx)
        {
            export_regions_to_svg(debug_out_path("mm-regions-merged-%d.svg", layer_idx),
                                  segmented_regions_merged[layer_idx], input_expolygons[layer_idx]);
        }
    }
    ...
    const PrintObject &print_object, const std::function<void()> &throw_on_cancel_callback)
{
    ...
    return segmentation_by_painting(print_object, extract_facets_info, num_facets_states, max_width, interlocking_depth,
                                    interlocking_beam, IncludeTopAndBottomLayers::Yes, throw_on_cancel_callback);
}
    ...
    const PrintObject &print_object, const std::function<void()> &throw_on_cancel_callback)
{
    ...
    // NOTE: Unlike MMU painted regions, painted fuzzy skin does NOT use "geometry theft".
    // The painted areas are stored in Layer::fuzzy_skin_painted_areas and used to create
    // PerimeterRegions in make_perimeters(). This applies fuzzy skin to perimeters via
    // polygon_segmentation() without modifying the underlying slice geometry.
    //
    // IMPORTANT: We use preserve_holes=true for fuzzy skin to prevent holes from expanding
    // during the negative offset in cut_segmented_layers(). Without this, holes "eat into"
    // painted regions on the outer surface when using large cut widths ("All perimeters" mode).

    int max_perimeter_depth = 0;
    for (size_t region_idx = 0; region_idx < print_object.num_printing_regions(); ++region_idx)
    {
        ...
        switch (region.config().fuzzy_skin_painted_perimeters.value)
        {
        case FuzzySkinPaintedPerimeters::External:
            max_perimeter_depth = std::max(max_perimeter_depth, 0);
            break;
        case FuzzySkinPaintedPerimeters::ExternalPlus1:
            max_perimeter_depth = std::max(max_perimeter_depth, 1);
            break;
        case FuzzySkinPaintedPerimeters::ExternalPlus2:
            max_perimeter_depth = std::max(max_perimeter_depth, 2);
            break;
        case FuzzySkinPaintedPerimeters::ExternalPlus3:
            max_perimeter_depth = std::max(max_perimeter_depth, 3);
            break;
        case FuzzySkinPaintedPerimeters::All:
            max_perimeter_depth = std::max(max_perimeter_depth, std::max(0, region.config().perimeters.value - 1));
            break;
        }
    }
    ...
    for (size_t region_idx = 0; region_idx < print_object.num_printing_regions(); ++region_idx)
    {
        ...
        const float external_perimeter_width = region.flow(print_object, frExternalPerimeter, layer_height).width();
        const float perimeter_width = region.flow(print_object, frPerimeter, layer_height).width();

        if (max_perimeter_depth > 0)
        {
            // External + N perimeters: calculate depth for specified perimeter count
            const float total_depth = external_perimeter_width + max_perimeter_depth * perimeter_width;
            max_depth = std::max(max_depth, total_depth);
        }
        else
        {
            // External only: just external perimeter width
            max_depth = std::max(max_depth, external_perimeter_width);
        }
    }

    return segmentation_by_painting(print_object, extract_facets_info, num_facets_states, max_depth, 0.f, false,
                                    IncludeTopAndBottomLayers::No, throw_on_cancel_callback, true /*preserve_holes*/);
}
