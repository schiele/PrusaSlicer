...
struct ItsNeighborsWrapper
{
    ...
    ItsNeighborsWrapper(const indexed_triangle_set &its, IndexT &&aindex)
        : its{its}, index_ref{index}, index(std::move(aindex))
    {
    }
...
{
    meshsplit_detail::NeighborVisitor visitor(meshsplit_detail::ItsWithNeighborsIndex_<Its>::get_its(m),
                                              meshsplit_detail::ItsWithNeighborsIndex_<Its>::get_index(m));
    bool has_some = false;
...
std::vector<Vec3i> create_face_neighbors_index(ExPolicy &&ex, const indexed_triangle_set &its)
{
    ...
    std::vector<Vec3i> neighbors(indices.size(), Vec3i(coord_t(no_value), coord_t(no_value), coord_t(no_value)));

    //for (const stl_triangle_vertex_indices& triangle_indices : indices) {
    execution::for_each(
        ex, size_t(0), indices.size(),
