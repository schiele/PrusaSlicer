...
// Spherical harmonics coefficients for a neutral studio environment.
// 9 RGB coefficients (order-2 SH) pre-computed offline. These give a soft,
// warm top light with cool fill from below - typical product photography.
...
void main()
{
    ...
    // Top light: diffuse + Blinn-Phong specular
    ...
    // Spherical harmonics environment reflection
...
#endif
        out_color = vec4(min(vec3(specular + rim) + reflection + color.rgb * diffuse, vec3(1.0)), color.a);
}
